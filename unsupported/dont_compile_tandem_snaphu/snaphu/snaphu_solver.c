...
long TreeSolve(nodeT **nodes, nodeT *ground, nodeT *source, 
  ...
  InitTreeNodes(nrow,ncol,nodes,ground);
  InitArcs(nrow,ncol,apexes,iscandidate);
  InitTree(source,nodes,ground,ngroundarcs,bkts,
	   nflow,costs,incrcosts,nrow,ncol,params);
  apexlistlen=INITARRSIZE;
  apexlist=MAlloc(apexlistlen*sizeof(nodeT *));
  groupcounter=2;
  ipivots=0;
  inondegen=0;
  nnodes=(nrow-1)*(ncol-1)+1;
  nnewnodes=0;
  maxnewnodes=ceil(nrow*ncol*params->maxnewnodeconst);
  treesize=1;
  fprintf(sp2,"treesize: %-8ld pivots: %-10ld improvements: %-8ld",
  ...
  while(treesize<nnodes){
    ...
    while(nnewnodes<maxnewnodes && treesize<nnodes){
      ...
      GetArc(from,to,&arcrow,&arccol,&arcdir,nrow,ncol);
      to->group=1;
	...
	to=NeighborNode(from,++arcnum,nodes,ground,
	...
	if(to->group>0){
	  if(to!=from->pred){
	    ...
	    CheckArcReducedCost(from,to,cycleapex,arcrow,arccol,arcdir,nflow,
				...
				&candidatebagsize,costs,incrcosts,
				iscandidate,nrow,ncol,params);
	  }else{
	...
	if((outcostto < to->outcost) || (cyclecost < 0)){
	  ...
	  if(cyclecost<0){
	    ...
	    /* edges change iff apex is ground (assuming ground is root) */
	    /* if in set-edges mode, skip this iteration if apex is ground */
	    /*
  	    if(params->setedges && apexes[arcrow][arccol]==ground){
 	      continue;
  	    }
	    */
	  ...
	  if(cyclecost<0){
	    ...
	    while(arcnum<upperarcnum){
	      tempnode1=NeighborNode(cycleapex,++arcnum,nodes,ground,
				     &arcrow,&arccol,&arcdir,nrow,ncol);
	    ...
	    if(firsttonode!=NULL){
	      NonDegenUpdateChildren(cycleapex,leavingparent,firsttonode,0,
				     ...
				     apexes,costs,incrcosts,nrow,ncol,params); 
	    }
	    if(firstfromnode!=NULL){
	  ...
	  if(leavingchild==NULL){
		...
		upperarcnum=ngroundarcs-1;
	      ...
	      while(arcnum<upperarcnum){
		node2=NeighborNode(node1,++arcnum,nodes,ground,
				   &arcrow,&arccol,&arcdir,nrow,ncol);
	...
	candidatebag[candidatebagnext++]=candidatelist[i];
      ...
      */
      ...
      /* display status */
      fprintf(sp2,"\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b"
	      "\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b"
	      "treesize: %-8ld pivots: %-10ld improvements: %-8ld",
	      treesize,ipivots,inondegen);
      fflush(sp2);
...
void CheckArcReducedCost(nodeT *from, nodeT *to, nodeT *apex, 
  ...
  /* illegal corner arcs have iscandidate==TRUE set in InitArcs() */
...
void InitTree(nodeT *source, nodeT **nodes, nodeT *ground, 
  ...
  while(arcnum<upperarcnum){
    ...
    to=NeighborNode(source,++arcnum,nodes,ground,
		    &arcrow,&arccol,&arcdir,nrow,ncol);
...
void GetArc(nodeT *from, nodeT *to, long *arcrow, long *arccol, 
  ...
  if(fromcol==tocol-1){           /* normal arcs (neither endpoint ground) */
    ...
    *arcdir=-1;
    ...
    fprintf(sp0,"WARNING: Illegal arc passed to GetArc()\n");
  }
...
void InitNetwork(short ***rowflowptr, short ***colflowptr, short **flows, 
  ...
  if(*nodesptr==NULL){
    *nodesptr=(nodeT **)Get2DMem(nrow-1,ncol-1,sizeof(nodeT *),sizeof(nodeT));
    InitNodeNums(nrow,ncol,*nodesptr,ground);
  }

  /* take care of ambiguous flows to ground at corners */
  *rowflowptr=flows;
  *colflowptr=&(flows[nrow-1]);
  ...
  *nflowdoneptr=0;
  *mostflowptr=0;
  Short2DRowColAbsMax(flows,nrow,ncol,mostflowptr);
  if(*mostflowptr*params->nshortcycle>LARGESHORT){
    fprintf(sp1,"Maximum flow on network: %ld\n",*mostflowptr);
    fprintf(sp0,"(maximum_flow * nshortcycle) too large.  Abort.\n");
    exit(ABNORMAL_EXIT);
  }
  *nflowptr=1;
  ...
  *apexesptr=(nodeT ***)Get2DRowColMem(nrow,ncol,sizeof(nodeT **),
  ...
  if(params->maxnflowcycles==USEMAXCYCLEFRACTION){
    params->maxnflowcycles=(long )(params->maxcyclefraction*nrow*ncol);
  }

  /* set up buckets for TreeSolve (MSTInitFlows() has local set of buckets) */
  *bktsptr=MAlloc(sizeof(bucketT));
  (*bktsptr)->minind=-FRound((params->maxcost+1)*(nrow+ncol)
			     *NEGBUCKETFRACTION);
  (*bktsptr)->maxind=FRound((params->maxcost+1)*(nrow+ncol)
			    *POSBUCKETFRACTION);
  ...
  *iincrcostfileptr=0;
  incrcosts[POSINCR]=(short **)Get2DRowColMem(nrow,ncol,
					      sizeof(short *),sizeof(short));
  incrcosts[NEGINCR]=(short **)Get2DRowColMem(nrow,ncol,
					      sizeof(short *),sizeof(short));
...
short GetCost(short ***incrcosts, long arcrow, long arccol, 
  ...
  if(arcdir>0){
    return(incrcosts[POSINCR][arcrow][arccol]);
  }else{
    return(incrcosts[NEGINCR][arcrow][arccol]);
  }
}
...
long ReCalcCost(void **costs, short ***incrcosts, long flow, 
  ...
  if(poscost>LARGESHORT){
    incrcosts[POSINCR][arcrow][arccol]=LARGESHORT;
    iclipped++;
  }else{
    if(poscost<-LARGESHORT){
      incrcosts[POSINCR][arcrow][arccol]=-LARGESHORT;
      ...
      incrcosts[POSINCR][arcrow][arccol]=poscost;
    }
  }
  if(negcost>LARGESHORT){
    incrcosts[NEGINCR][arcrow][arccol]=LARGESHORT;
    iclipped++;
  }else{
    if(negcost<-LARGESHORT){
      incrcosts[NEGINCR][arcrow][arccol]=-LARGESHORT;
      ...
      incrcosts[NEGINCR][arcrow][arccol]=negcost;
    }
  }
...
void SetupIncrFlowCosts(void **costs, short ***incrcosts, short **flows,
  ...
  long arcrow, arccol, maxcol, iclipped;
  ...
  for(arcrow=0;arcrow<2*nrow-1;arcrow++){
    ...
    for(arccol=0;arccol<maxcol;arccol++){
      ...
      if(params->setedges && 
	 ((arcrow<nrow-1 && (arccol==0 || arccol==ncol-1))
	  || arcrow==nrow-1 || arcrow==2*nrow-2)){
  ...
  if(iclipped){
    fprintf(sp0,"%ld incremental costs clipped to avoid overflow\n",iclipped);
  }
}
 ...
 * -----------------------------
 * Computes the total cost of the flow array and prints it out.
 */
long long EvaluateTotalCost(void **costs, short **flows, 
			    long nrow, long ncol, paramT *params){

  long long rowcost, totalcost;
  long row, col, maxcol;
...
void MSTInitFlows(float **wrappedphase, short ***flowsptr, 
  ...
  *nodesptr=(nodeT **)Get2DMem(nrow-1,ncol-1,sizeof(nodeT *),sizeof(nodeT));
  ...
  bkts->size=FRound((maxcost+1)*(nrow+ncol+1));
  ...
  while(TRUE){
    ...
    InitNodes(nrow,ncol,*nodesptr,ground);
	...
	if(row<nrow-1){
	  if(col!=0){
	    ...
	    if(tempcharge>MAXRES || tempcharge<MINRES){
	      fprintf(sp0,"Overflow of residue data type.  Abort.\n");
	      exit(ABNORMAL_EXIT);
	    }
