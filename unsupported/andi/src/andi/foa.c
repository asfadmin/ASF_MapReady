...
/*Report(lvFrame,"cFrameCount = %d, myFrame.frameNo = %d", 
	...
	*/
	if ( cFrameCount != myFrame.frameNo ) {
	...
	else if ( uFormCount != GET_SAT_USHORT(myFrame.formCycler) ) {
	    /* frame missed */
...
if (bCyclicLastFormatInitalized) {
	if ( uCyclicLastFormat != (unsigned short)(uFormCount-1) ) {
	    g_stat.format.luMissCount+= uFormCount - uCyclicLastFormat - 1;
	    Report(lvFormat,"Missing %ld formats during format scan at file offset %lu, uFormCount %u, uCyclic %u",
			(int)uFormCount - (int)uCyclicLastFormat - 1, luFileOffset,uFormCount,uCyclicLastFormat);
		}
	}
else {
