...
/*==============================================================================
Filename:	dtkm_get_antenna_dtk_padding_time.c
...
==============================================================================*/
#pragma ident	"@(#)dtkm_get_antenna_dtk_padding_time.c	5.1 98/01/08 APS/ASF"
#pragma ident	"@(#) /home/aps/r2.1.2/src/lib_dtkm/SCCS/s.dtkm_get_antenna_dtk_padding_time.c"


/*==============================================================================
Function:       dtkm_get_antenna_dtk_padding_time.c

Description:    gets the antenna padding time in seconds for the data-take, 
				based on station_id and antenna_id.  This padding time 
				is used when checking for conflicts between data-takes of 
...
Creation Date:  Wed Jul 17 16:41:47 PDT 1996
	...
	int 		*antenna_dtk_padding_time_sec ) 
{
	...
	*/
	*antenna_dtk_padding_time_sec = -100000 ;

	if ( antenna_list == NULL )
	{
		/* 
		-- retain this small relation in memory for 
		-- efficiency.  
		*/
	...
	/* check each record, looking for the right station, antenna:  */
	for (
		...
		)
	{
		   ...
		   )
		{
			...
			--
			-- the padding time required is the sum of all 
			-- of the individual times.  This is the delta time 
			-- required between 2 data-takes of DIFFERENT SATELLITES 
			-- ON THE SAME ANTENNA.  
			*/
			*antenna_dtk_padding_time_sec = 
				CAST_ANTENNA_PRE_PASS_TIME_SEC 
					antenna_rec[ANTENNA_PRE_PASS_TIME_SEC] +
				CAST_ANTENNA_PRE_DTK_TRACK_PAD_SEC 
					antenna_rec[ANTENNA_PRE_DTK_TRACK_PAD_SEC] +
				CAST_ANTENNA_POST_DTK_TRACK_PAD_SEC 
					antenna_rec[ANTENNA_POST_DTK_TRACK_PAD_SEC] +
				CAST_ANTENNA_POST_PASS_TIME_SEC 
					antenna_rec[ANTENNA_POST_PASS_TIME_SEC] ;
			return TRUE ;
		}
	}
	return DTKM_ERROR_ANTENNA_REC_NOT_FOUND_FOR_DTK_PROPOSAL ;
}
