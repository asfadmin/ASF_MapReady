...
/*==============================================================================
Filename:       mu_verify_single_perms.c
...
==============================================================================*/
#pragma ident   "@(#)mu_verify_single_perms.c	5.1 98/01/08 APS/ASF"
#pragma ident   "@(#) /home/aps/r2.1.2/src/lib_multiUser/SCCS/s.mu_verify_single_perms.c"


/*==============================================================================
Function:       mu_verify_single_perms()
    ...
    */
        ...
        )
    {
        ...
        sys_count = mu_count_sysprocesses_khph( dbproc,
            CAST_ACTIVE_SINGLE_ACTIVITIES_KPID 
        ...
        if( sys_count < 1 )
        {
            ...
            -- delete this single activity from the database 
            -- and the perm_list.  
            -- Be SURE to delete only this one record, by 
            -- putting in enough fields in the where_clause:
            */
...
"where %s = %ld and %s = '%s' and %s = '%s' and %s = '%s' and %s = %ld and %s = '%s'",
            ...
            if( n_deleted_recs == 0 )
                return MU_ERROR_DELETED_NO_RECS_FROM_ACTIVE_SINGLE_ACTIVITIES ;
            if( n_deleted_recs > 1 )
