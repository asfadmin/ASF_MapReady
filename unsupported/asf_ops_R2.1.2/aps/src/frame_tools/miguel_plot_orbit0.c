...
{     
	...
	char	string2[20] = "xxxxxxxxxxxxxxxxxxx" ;
	char	string3[20] = "xxxxxxxxxxxxxxxxxxx" ;
...
"Use -U sybase_userid or setenv APS_SYBASE_USERID.\n",
    ...
    */
	...
	fgets (buffer, 150, ascii_file_ptr) ; /* skip past header */
										        /* FOR ORBIT0 PLOTS  */
	...
	{
		/* FOR ORBIT0 PLOTS
		fscanf (ascii_file_ptr, "%s %s %s %f %f %f %f %f %f %f %f %f %f",
			&string1, &string2, &string3, &float1, &float2,
			&float3, &float4, &float5,
			&float6, &float7, &float8,
			&float9, &float10) ;
		   FOR VANCE'S FILE PLOTS
