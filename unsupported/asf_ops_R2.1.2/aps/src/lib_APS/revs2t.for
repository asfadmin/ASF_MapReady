...
*  $Date$ $Revision$ $Author$
*********************************************************************/
      SUBROUTINE REVS2T(dbproc, IBATCH,SAT,IREV1,IREV2,
     ?					ET1,ASFT1,ET2,ASFT2,IER)
      character*100 SccsFileID
     -/'@(#)revs2t.for	5.1 98/01/08 APS/ASF\0'/
      ...
      character*22 ASFT
      IF(IBATCH .EQ. 1)
     ?WRITE(*,*) ' @REVS2T:  SAT=', SAT(1:2),'  IREV1 = ', IREV1
      IF(IBATCH .EQ. 1)
     ?WRITE(*,*) ' @REVS2T:  IREV2 = ', IREV2
	...
	RETURN
      ENDIF
      IER = 0
      CALL REV2TIME(dbproc,SAT,IREV1,ET1,ASFT1,ET,ASFT,IER)
      IF(IER .NE. 0) RETURN 
      CALL REV2TIME(dbproc,SAT,IREV2,ET,ASFT,ET2,ASFT2,IER)
      IF(IER .NE. 0) RETURN 
      IF(IBATCH .EQ. 1)
     ?WRITE(*,*) ' $REVS2T:  ET1 = ', ET1,' ASFT1 = ', ASFT1(1:21)
      IF(IBATCH .EQ. 1)
     ?WRITE(*,*) ' $REVS2T:  ET2 = ', ET2,' ASFT2 = ', ASFT2(1:21)
      IF(IBATCH .EQ. 1)
     ?WRITE(*,*) ' $REVS2T:  IER = ', IER
      RETURN
      END
