...
*  $Logfile:   ACS003:[BLD.MPS.LIB.SRC]QNPNT.FOV  $
...
*  Output Parameters:
*  Name         Type    Definition
*  NLAT		REAL*8	LATITUDE OF NORTHERNMOST POINT IN QUAD
*  Variables:
*  Locals :
*  Externals :
...
*                                                                   
*********************************************************************/
      SUBROUTINE QNPNT (NLAT,Q1,Q2,Q3,Q4)
      character*100 SccsFileID
     -/'@(#)qnpnt.for	5.1 98/01/08 APS/ASF\0'/

      IMPLICIT NONE
      REAL*8 Q1(3), Q2(3), Q3(3), Q4(3), NLAT
      REAL*8 P(3), PC(3), XLL(2), D
      INTEGER IFLAG
      NLAT = 90.0D0
C---	CHECK FOR N POLE INSIDE THE QUAD.
      P(1) = 0
      P(2) = 0
      P(3) = 1
      CALL GCPINQ(IFLAG,Q1,Q2,Q3,Q4,P)
      IF(IFLAG .GE. 0) GO TO 9999
C---	N POLE IS OUTSIDE THE QUAD.  THE CLOSEST POINT TO POLE IS 
C---	IS ON ONE OF THE FOUR SIDES.  
      NLAT = -100.0D0
C---	GET LAT OF CLOSEST POINT ON EACH SIDE IN QUAD.
      CALL GCSEGP(D,PC,Q1,Q2,P)
      CALL XYZ2LL(PC,XLL)
      IF(XLL(1) .GT. NLAT) NLAT = XLL(1)
