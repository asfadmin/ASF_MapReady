...
==============================================================================*/
#pragma ident   "@(#)timeconvc.c	5.1 98/01/08 APS/ASF"
#pragma ident   "@(#) /home/aps/r2.1.2/src/lib_timec/SCCS/s.timeconvc.c"
...
Returns:     	TRUE if asftime is a valid ASF time string
				TC_VALID_TIME_ERR	- if the time string format is incorrect
				TC_VALID_YEAR_ERR	- if the year is an invalid value
				TC_VALID_DOY_ERR	- if the day of year is an invalid value
				TC_VALID_HOUR_ERR	- if the hour is an invalid value
				TC_VALID_MIN_ERR	- if the minutes are an invalid value
				TC_VALID_SEC_ERR	- if the seconds are an invalid value
...
==============================================================================*/
int
tc_validate_asf_datetime(asftime)
	char *asftime ;
{
	char *ptr ;
	char *ASF_datetime ;
	...
	/*
	-- this routine was allowing non-numerics in the numeric fields.
	-- atoi() does not catch this.
	*/
	...
	/*
	-- NOTE:  year, dayofyear, etc are pointers into the malloc'ed area. 
	...
	*/
	...
	/* check for valid values in the fields */

	if (iyear < 1990 || iyear > 2100)
		return (TC_VALID_YEAR_ERR) ;

	daysinyear = 365 + tc_leapyr(iyear) ;
	if (idayofyear < 1 || idayofyear > daysinyear)
		return (TC_VALID_DOY_ERR) ;

	if (ihour < 0 || ihour > 23)
		return (TC_VALID_HOUR_ERR) ;

	if (imin < 0 || imin > 59)
		return (TC_VALID_MIN_ERR) ;

	if (isec < 0 || isec > 59)
		return (TC_VALID_SEC_ERR) ;
...
********************************************************************/
int tc_parse_asftime(asftime, year, decade, day, hour, min, sec, msec)
	char *asftime ;
...
*  $Date$ $Revision$ $Author$
*********************************************************************/
int tc_asf2csa(asftime, csatime)
...
*  $Date$ $Revision$ $Author$
*********************************************************************/
int tc_csa2asf(csatime, asftime)
...
*  $Date$ $Revision$ $Author$
*********************************************************************/
int tc_asf2esa(asftime, esatime)
...
*  $Date$ $Revision$ $Author$
*********************************************************************/
int tc_esa2asf(esatime, asftime)
	char *esatime ; 	
	char *asftime ;  
{
	char 
	...
	if (!gen_field_is_numeric(eday, strlen(eday))
	||  !gen_field_is_numeric(emon, strlen(emon))
	||  !gen_field_is_numeric(eyear, strlen(eyear))
	||  !gen_field_is_numeric(ehour, strlen(ehour))
	||  !gen_field_is_numeric(emin, strlen(emin))
	||  !gen_field_is_numeric(esec, strlen(esec))		)
		return (FALSE) ;
	...
	*/
	if (!tc_cal2doy(year, mon, day, &doy)) 
		return(FALSE) ;
	...
	char *asftime)
{
	...
	||  !gen_field_is_numeric(emin, strlen(emin))
	||  !gen_field_is_numeric(esec, strlen(esec))
	||  !gen_field_is_numeric(etsec, strlen(etsec))	)
		return (FALSE) ;
...
*  $Date$ $Revision$ $Author$
*********************************************************************/
int tc_asf2yyyymmdd (asftime, datestr)
	...
	char *datestr ;
{
	...
	if (!gen_field_is_numeric(buf, strlen(buf)) )
		return (FALSE) ;
	year = atoi(buf) ;
	...
	if (month == 2)
	{
		if ((tc_leapyr(year)  &&   day > 29) 
		||  (!tc_leapyr(year)  &&   day > 28))
			return (FALSE) ;
	}
	...
	char	*asftime_plusndays ) /*  asf time + ndays */
{
	...
	/* check for valid pointers */
	if (asftime == NULL
	||  asftime_plusndays == NULL )
		return (FALSE) ;

	/*  Convert asftime to et */

	if (!tc_asf2et(asftime, &et)) 
		return (FALSE) ;
...
*           08 Feb 1995                                            FAIST  $
*********************************************************************/
int tc_systime2yr_date(year, date)
	char *year, *date ;
{
...
*
*********************************************************************/
int tc_systime2yyyycddd(datestr)
	char *datestr ;
{
	char asftime[22] ;
	...
	char	*asftime )
{
	...
	return(tc_esa2asf(esatime, asftime)) ;
}
...
==============================================================================*/
int tc_cal2julian(int y, int m, int d)
{
...
Notes:		
==============================================================================*/
int tc_et_ASF_datetime_diff(strttime, stoptime, deltatime)
	char *strttime ; 
	char *stoptime ;
	double *deltatime ;
{
	...
	if (!tc_asf2et(strttime, &et_start))
		return(FALSE) ;

	if (!tc_asf2et(stoptime, &et_stop))
		return(FALSE) ;
...
#ifdef DEBUG
	(void) printf( "in tc_odl2asf: odltime %s asftime %s\n", odltime, asftime );
#endif
	...
	char *padded_stoptime ) 
{
	...
	if (strttime == NULL
	||	stoptime == NULL
	||	padded_strttime == NULL
	||  padded_stoptime == NULL )
		return (FALSE) ;

	/* check for valid times */

	if (tc_validate_asf_datetime(strttime) != TRUE 
	||  tc_validate_asf_datetime(stoptime) != TRUE)
		return (FALSE) ;
	...
	if ((!tc_asf_add_ndays(strttime, -1 * et_days, padded_strttime))
	|| (!tc_asf_add_ndays(stoptime,       et_days, padded_stoptime)))
		return(FALSE) ;
	...
	char *argv[] ;
{
	...
	char odltime[] = "yyyy:ddd:mm:hh:ss.ccc" ;
...
#define PRINT_STATUS(status) if (status) printf("STATUS: OK\n\n") ; else printf("STATUS: ERR\n\n") ;
	...
	printf("tc_parse_asftime(char *asftime, \
		\n\t&year, &decade &doy, &mon,  &hour, &min, &sec)\n") ;
	status = tc_parse_asftime(asftime1, 
		&year, &decade, &doy, &mon,  &hour, &min, &sec) ;
	...
	printf("char * tc_id_asftime(char *id)") ;
	sv_filename = tc_id_asftime(sv) ;
	if (!sv_filename)
	{
	...
	printf("tc_asf2csa(asftime1, csatime)\n") ;
	status = tc_asf2csa(asftime1, csatime) ;
	printf("%s --> %s\n", asftime1, csatime) ;
	PRINT_STATUS(status) ;


	printf("tc_csa2asf(csatime, asftime1)\n") ;
	status = tc_csa2asf(csatime, asftime1) ;
	printf("%s --> %s\n", csatime, asftime1) ;
	PRINT_STATUS(status) ;


	printf("tc_asf2yyyymmdd(asftime1, j1date)\n") ;
	status = tc_asf2yyyymmdd(asftime1, j1date) ;
	printf("%s --> %s\n", asftime1, j1date) ;
	PRINT_STATUS(status) ;


	printf("tc_yyyymmdd2asf(j1date, asftime1)\n") ;
	status = tc_yyyymmdd2asf(j1date, asftime1) ;
	printf("%s --> %s\n", j1date, asftime1) ;
	PRINT_STATUS(status) ;


	printf("tc_asf_add_ndays(asftime1, 5.5, asftime2)\n") ;
	status = tc_asf_add_ndays(asftime1, 5.5, asftime2) ;
	printf("%s --> %s\n", asftime1, asftime2) ;
	PRINT_STATUS(status) ;


	printf("tc_esa2asf(esatime, asftime1)\n") ;
	status = tc_esa2asf(esatime, asftime1) ;
	printf("%s --> %s\n", esatime, asftime1) ;
	PRINT_STATUS(status) ;

	printf("tc_esadaytime2asf(esadaytime, asftime1)\n") ;
	status = tc_esadaytime2asf(esadaytime, asftime1) ;
	printf("%s --> %s\n", esadaytime, asftime1) ;
	PRINT_STATUS(status) ;

	printf("tc_systime2yr_date(cyear, cdate)\n") ;
	status = tc_systime2yr_date(cyear, cdate) ;
	printf("SYSTIME --> %s %s\n",  cyear, cdate) ;
	PRINT_STATUS(status) ;


	printf("tc_systime2asf(asftime1)\n") ;
	status = tc_systime2asf(asftime1) ;
	printf("SYSTIME --> %s\n",  asftime1) ;
	PRINT_STATUS(status) ;
	...
	printf("tc_yyyymmdd_check(j1date)\n") ;
	status = tc_yyyymmdd_check(j1date) ;
	printf("%s --> %s\n", j1date, j1date) ;
	PRINT_STATUS(status) ;

	printf("tc_yyyymmdd_check(j1date2)\n") ;
	status = tc_yyyymmdd_check(j1date2) ;
	printf("%s --> %s\n", j1date2, j1date2) ;
	PRINT_STATUS(status) ;


	printf("tc_yyyymmdd_hhmmss2asf(j1date, j1time, asftime)\n") ;
	status = tc_yyyymmdd_hhmmss2asf(j1date, j1time, asftime1) ;
	printf("%s  %s --> %s\n", j1date, j1time, asftime1) ;
	PRINT_STATUS(status) ;

	printf("tc_leapyr(1992)\n") ;
	status = tc_leapyr(1992) ;
	PRINT_STATUS(status) ;


	printf("tc_leapyr(1993)\n") ;
	status = tc_leapyr(1993) ;
	PRINT_STATUS(status) ;

	printf("tc_doy2cal(1993, 60, &year, &mday )\n") ;
	status = tc_doy2cal(1993, 60, &year, &mday) ;
	printf("YEAR: %d DOY: %d --> month: %d MDay: %d\n", 1992, 60, year, mday) ;
	PRINT_STATUS(status) ;


	printf("tc_cal2doy(1992, 3, 01, &doy)\n") ;
	status = tc_cal2doy(1992, 3, 01, &doy) ;
	printf("DOY: %d\n", doy) ;
	PRINT_STATUS(status) ;

	printf("tc_cal2doy(1993, 2, 29, &doy)\n") ;
	status = tc_cal2doy(1993, 2, 29, &doy) ;
	printf("DOY: %d\n", doy) ;
	PRINT_STATUS(status) ;


	printf("tc_doy2cal(1993, 60, &mon, &day)\n") ;
	status = tc_doy2cal(1993, 60, &mon, &day) ;
	printf("Month: %d Day:%d\n", mon, day) ;
	PRINT_STATUS(status) ;


	printf("tc_doy2cal(1992, 60, &mon, &day)\n") ;
	status = tc_doy2cal(1992, 60, &mon, &day) ;
	printf("Month: %d Day:%d\n", mon, day) ;
	PRINT_STATUS(status) ;

	printf("tc_asf2et(asftime2, &et)\n") ;
	status = tc_asf2et(asftime2, &et) ;
	printf("ASFT: %s ET: %.8f \n", asftime2, et) ;
	PRINT_STATUS(status) ;
	...
	printf("tc_et2asf(et, asftime2)\n") ;
	status = tc_et2asf(et, asftime2) ;
	printf("ASFT: %s ET: %.8f \n", asftime2, et) ;
	PRINT_STATUS(status) ;
	...
	asftime2[0] = NULL ;
	printf("tc_julian2asf(&et, asftime2)\n") ;
	status = tc_julian2asf(&et, asftime2) ;
	printf("ASFT: %s julian: %.8f \n", asftime2, et) ;
	PRINT_STATUS(status) ;

	printf("tc_et_ASF_datetime_diff(asftimed1, asftimed2)\n") ;
	status = tc_et_ASF_datetime_diff(asftimed1, asftimed2, &et) ;
	printf("ASFT: %s - %s: %.8f \n", asftimed1, asftimed2, et) ;
	PRINT_STATUS(status) ;

	printf("tc_time_pad(asftime1, asftime2, 63, asftime4, asftime5)\n") ;
	printf("ASFT: %s - %s \n ASFT: %s - %s \n ", 
		asftime1, asftime2, asftime4, asftime5) ;
	status = tc_time_pad(asftime1, asftime2, 63, asftime4, asftime5) ;
	printf("ASFT: %s - %s \n ASFT: %s - %s \n ", 
		asftime1, asftime2, asftime4, asftime5) ;
	PRINT_STATUS(status) ;


	printf("tc_asf2odl(asftime1, odltime)\n") ;
	status = tc_asf2odl(asftime1, odltime) ;
	printf("%s --> %s\n", asftime1, odltime) ;
	PRINT_STATUS(status) ;
}
#endif
