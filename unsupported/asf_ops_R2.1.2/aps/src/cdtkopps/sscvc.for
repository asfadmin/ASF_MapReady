...
*  $Logfile:   QDISK:[BLD.MPS.LIB.SRC]SSCVC.QFV  $
*  Purpose:	RUN SPECIFIC SITE COVERAGE FOR A CIRCLE SITE.  
*  Functions called:
*  VECTOR LIBRARY:  
*  Input Parameters:
...
*  RADIUSKM	REAL*8	RADIUS OF THE CIRCLE (KM)
*  CLAT		REAL*8	LATITUDE IN DEGREES CENTER
*  CLON		REAL*8	LONGITUDE IN DEGREES OF CENTER
...
*  Date			 Rev  Author
*  18 Jul 1990 16:49:32  2.0  DBMAN  
*  11 Mar 1993 13:37:20  2.1  IBBOTT  
*  $Date$ $Revision$ $Author$
*********************************************************************/
      SUBROUTINE SSCVC (dbproc, IBATCH,DARID,SITENAME,RADIUSKM,
     ?      CLAT,CLON,
     ?      SAT,SENSOR,ASCDSC,TIME1,TIME2,TREV1,TREV2,NRECS,ISTAT)
      character*100 SccsFileID
     -/'@(#)sscvc.for	5.1 98/01/08 APS/ASF\0'/

      IMPLICIT NONE
      INCLUDE 'APS_HOME:include/local/timeconv.inc'
      INTEGER DARID, NRECS, ISTAT, IBATCH
      REAL*8 TIME1, TIME2
      CHARACTER*33 SITENAME
      CHARACTER*3 SAT
      CHARACTER*4 SENSOR
      CHARACTER*2 ASCDSC
      CHARACTER*2 OLDASCDSC, CASCDSC, I_ASCDSC
      CHARACTER*22 ASFTIME1, ASFTIME2
      REAL*8 RADIUSKM,R,CLAT,CLON,CENTER(3),XLL(2),CLL(2),ANGRAD
      ...
      INTEGER IREV, OLDREV, COVRECS, J, TREV1, TREV2
      REAL*8 PDIST, SWATH, BUF(3)
      REAL*8 RADSEARCH
      INTEGER NPOINT,ILOOK,INORML,IEOF, IBRKUP
      REAL*8 POINT(10,99)
...
C*			IF NO COVERAGE FOR SITE, ALL VALUES ARE 0.0D00 
	...
	print 8001,dbproc, IBATCH, DARID, SITENAME(1:lastc(SITENAME)), 
     ...
     ?		TREV1, TREV2
 8001	format(' sscvc.for:  dbproc = ', Z8,
     ?		/13x,' IBATCH = ', I,  
     ?		/13x,' DARID = ', I, /13x,' SITENAME = ', A, 
     ?		/13x,' RADIUSKM  = ', F9.3,/13x,'CLAT = ', F10.3, 
     ?		/13x,'CLON = ', F10.3,/13x,'SAT = ', A
     ?		/13x,'SENSOR = ', A /13x,'ASCDSC = ', A
...
C---	This call is EITHER ascending OR descending:  
 	...
 	print *,'sscvc.for:  ASCDSC parameter must be "A" or "D".'
...
C---	CONVERT THE RADIUS OF THE CIRCLE FROM KM TO RADIANS ON A UNIT SPHERE.
      R = RADIUSKM / ERAD
      ISTAT = 0
...
C---	ADD RADIUS OF CIRCLE AND SWATH WIDTH.  
      RADSEARCH = PDIST + RADIUSKM + SWATH
C---	NOW CONVERT TO DEGREES.  
      RADSEARCH = (RADSEARCH / 1.852D0) / 60.0D0
C---	NOW BUILD TARGET AREA.  MIN/MAX LAT/LON
      CALL SSCVCB(CLL,RADSEARCH,NLAT,SLAT,ELON,WLON,INORML)
...
C---	MUST PROCESS THE GROUP AND SEE IF THIS COVERAGE PERIOD COVERS THE SITE.
      CALL SSCOVC(CENTER,R,POINT,NPOINT,START,STOP,I_ASCDSC)
C---	NOW RESET THE POINTER; THIS GROUP IS FINISHED.  
      NPOINT = 0
      IF(START(1).EQ.0.0D0) GO TO 7000
C---	NOW WRITE THE SSCRVG RECORD FROM START AND STOP TO A FILE.  
...
C---	LEFT LOOKING SWATH.  SET ILOOK AND SHUFFLE THE PREVIOUS SWATH POINTS.
      ...
      if(print_flag .eq. 1) THEN
          print *,' '
          print *,' '
          print *,'sscvc.for:  SWATH INDICATES LEFT-LOOKING SENSOR'
          print *,' '
          print *,' '
      ENDIF
      BUF(1) = POINT(8,1)
...
C---	NORMAL END.  
 ...
 9999 CONTINUE
      if(print_flag .eq. 1) 
     ?    print *,'sscvc.for:  NRECS = ', NRECS, '  ISTAT = ', ISTAT
      if(print_flag .eq. 1 .AND. ILOOK .EQ. 2)
     ?    print *,'sscvc.for:  SWATH INDICATED LEFT-LOOKING SENSOR'
      RETURN
      END
