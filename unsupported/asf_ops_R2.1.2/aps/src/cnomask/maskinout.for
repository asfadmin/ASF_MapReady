...
C-- ==========================================================================
C--
C--  Fortran Filename:  maskinout.for
C--
C--  Description:   
C-- 
C--  Notes:
...
c  Purpose: 
c      Generates maskinout datbase records.  
c
c  Input Parameters:
...
c
c  Local variables:
c  ISTEP   = 1     int    minutes between coverage points:    1.   
c  IQUICK  = 0     int    quick coverage flag:  =1 means write to ascii file 
c                         but do not write to db.        :    0
c  IFILE   = 0     int    file coverage flag:  = 1 means write to ascii file.
c  REPFLAG = Y     char   "Y" means use ground track repeat cycle for 
...
c       brev                    int     Rev number to start maskinout
c       erev                    int     Rev number to end maskinout
...
c  Output Parameters:
c  Name         Type            Description
c  Output is done in MaskFormat
c
...
c         dummy data for the sensor, and then suppresses the database 
c         writes into the coverage relation, while leaving in the 
...
c  
      ...
      INTEGER FUNCTION MASKINOUT(progname, dbproc, 
     ?  s_start_time,s_end_time, s_sat )

      character*100 SccsFileID
     -/'@(#)maskinout.for	5.1 98/01/08 APS/ASF\0'/
...
C
      INTEGER IREV, IER, NRECS_FOUND, NRECSM, NM
...
c---    DECLARE C-routine calls to the compiler:
      ...
      external init_vec_lib             !$pragma c(init_vec_lib)
          integer init_vec_lib                  !$pragma c(init_vec_lib)
      external init_vec_lib_exit        !$pragma c(init_vec_lib_exit)
      external aps_chmod          !$pragma c(aps_chmod)
      external aps_access         !$pragma c(aps_access)
      external maskinout_metadata !$pragma c(maskinout_metadata)
      external delete_maskinout   !$pragma c(delete_maskinout)
...
c
      ...
      MASKID = 'GBL'
c     Use a DUMMY VALUE for sensor:
      sensor = 'DUM'

C---    fabricate DUMMY SATSENSOR DATA 

      IMODE = 1
      CMODE = 'CENTER POINTING'
      BEAMW = 10.0
      ALOOK =  5.0
...
C---    run is shortened.  The end time is changed to the end of the phase.
      run_shortened = 0
      NRECSM = 0
      rcode = system('banner START MASKINOUT')
      call sleep (1)
      print *,'maskinout:  satellite = ', sat
      print *,'maskinout:  begin_time = ', start_time
      print *,'maskinout:  end_time = ', end_time
      print *,' '
...
C---    initialize the vector library; EXIT with a message if an error  
      rcode = init_vec_lib()
      if(rcode .ne. 0) THEN
          CALL APS_LOG_MSG_FORTRAN( PROGNAME, APS_CRITICAL,
     ?        'stoic file not readable', DO_SYSLOG, DO_PRINT )
          CALL APS_LOG_MSG_FORTRAN( PROGNAME, APS_INFO,
     ?        'Program terminated abnormally.', DO_SYSLOG, DO_PRINT )
          call init_vec_lib_exit(%VAL(rcode))
      ENDIF

      WRITE(*,*) ' MASKINOUT:  start_time =',start_time 
C---      CALL ASF2ET(start_time, start_et,IER)
      IER = tc_asf2et(start_time, start_et )
      if(IER .ne. 1) then
          print *,' '
          print *,'maskinout.for:  ERROR in start time'
          print *,'               start time = ', start_time
          print *,'               terminating the run.'
          CALL APS_LOG_MSG_FORTRAN( PROGNAME, APS_ERROR,
     ?            'start time value',
     ?            DO_SYSLOG, DO_PRINT )
          maskinout = APS_EXIT_ERROR
          return
      endif
      WRITE(*,*) ' MASKINOUT:  start et = ', start_et
      WRITE(*,*) ' MASKINOUT:  end_time =',end_time 
C---      CALL ASF2ET(end_time, end_et,IER)
      IER = tc_asf2et(end_time, end_et)
      if(IER .ne. 1) then
          print *,' '
          print *,'maskinout.for:  ERROR in end time, near column',
     ?                IER
     ...
     ?            DO_SYSLOG, DO_PRINT )
      ...
      WRITE(*,*) ' MASKINOUT;  end_et = ', end_et
      WRITE(*,*) ' MASKINOUT:  sat =',sat 
...
c---    pass dbproc by value to C routines:  
     ...
     ?     XINCL,ARGPER, ANTARCTIC_MODE, IER)
      print *,'maskinout:  from get_phase_t:  PHASE = ', PHASE, 
     ?  ' CYDAYS = ', CYDAYS, 
     ?     ' CYREVS = ', CYREVS 
...
C---          CALL ET2ASF(end_et, end_time)
          ...
          PRINT *, 'MASKINOUT:  This run had a start time in phase ',
     ...
     ?    ' phase.  The run '
      ...
      if(end_et - start_et .gt. 0.5) then
          print *,' '
          print *,'maskinout:  the time period for the run is: ' 
 ...
 9911     format(I3) 
     ...
     ?         // bufdays(1:lastc(bufdays)) // ' DAYS"' // char(0)
          ...
          if(end_et - start_et .gt. 200.5) then
              print *,'maskinout:  this run is for over 200 days; '
...
C---  file name only, without path.  for metadata...
      ...
      WRITE(*,*)'MASKINOUT:  ephemeris file =',fname(1:lastc(fname))
...
c---  translate the EPHEMERIS file name.  
      ibuf = fname
      call tfname(ibuf, filename80)
      lc = lastc(filename80)
      print *,'maskinout:  ephemeris file name = ', filename80(1:lc)
     ...
     ?        DO_SYSLOG, DO_PRINT )
      ...
      if(ier .ne. 0) then
          ...
          print*,' '
          print*,
     ?     'maskinout:  ERROR:  access to file denied.  r permission ',
     ?                         'needed for file:'
          rcode = system('ls -l ' // 
     ?                  filename80(1:lastc(filename80)) // char(0) )
          print *,'maskinout:  terminating the run.'
...
C---  FIRST WORD IS THE EPOCH.
      REWIND(EUNIT)
      READ(EUNIT)epoch
      WRITE(*,*) ' MASKINOUT:  epoch =',epoch 
C---      CALL ET2ASF(epoch,epochasf)
      IER = tc_et2asf(%VAL(epoch), epochasf)
      WRITE(*,*) ' MASKINOUT:  epochasf    =', epochasf
      WRITE(*,*) ' MASKINOUT:  phase_start =', phase_start
...
C---     of the Stoic File containing time coversion data.  
     ...
     ?        DO_SYSLOG, DO_PRINT )
      ...
      WRITE(*,*) 'MASKINOUT:  phase relation and ephemeris file agree.'

c---  Insert the MASKINOUT_MDAT record (maskinout metadata) into the database.
c---  NOTE:
c---  pass dbproc by value to C routines:  
      call maskinout_metadata(%VAL(dbproc), nowtime, 
     ?     SAT, start_time, 
     ?     end_time, 
...
C---      REPFLAG .EQ. 'Y' MEANS USE REPLICATION TO READ THE EPHM FILE.
     ...
     ?                start_et,end_et,TNODE,x,nodeflag)
          IF (FIRSTGETX) THEN
              FIRSTGETX = .FALSE.
              WRITE(*,*)' MASKINOUT:  FIRST RECORD FROM EPHEMERIS FILE:'
              WRITE(*,*)'          NODEFLAG = ', NODEFLAG
              WRITE(*,*)'          X = ', X
          ENDIF
      ENDDO
      IREV = X(8)
      WRITE(*,*)' MASKINOUT:  NOW GETTING STATION DATA'
C---  ASF station data:
...
C---      disable the ASF mask code via a non-zero, tiny radius:
      ...
      WRITE(*,*)' MASKINOUT: ASFLAT, ASFLON, ASFRAD=',
...
C---      disable the MCM mask code via a zero radius:
      ...
      WRITE(*,*)' MASKINOUT: MCMLAT, MCMLON, MCMRAD=',
...
c
      ...
      if(end_et - start_et .gt. 0.5) then
          ...
          print *,' '
          print *,' '
          print *,'This maskinout run is for:'
          rcode = system(bufdays)
      endif
      rcode = system('date')
      print *,'maskinout.for:  starting to generate maskinout records ',
     ?        'for ', sat, '.'
...
C--                   station_mask flag:
                      amask = 'A'
                      CALL MaskFormat(dbproc,'IN',Y, amask, sat, NM)
                      NRECSM = NRECSM + NM
                  ENDIF
...
C--                   station_mask flag:
                      amask = 'A'
                      CALL MaskFormat(dbproc,'OUT',Y, amask, sat, NM)
                      NRECSM = NRECSM + NM
                  ENDIF
...
C---          IF THIS IS A NORMAL MASKINOUT RUN OR IF THE USER
C---          HAS REQUESTED A "NORMAL" TIME STEP, THEN 
C---          PRINT THIS RECORD IN THE MASKINOUT FILE.
...
C--     -         CALL MaskFormat(dbproc, '-',x, 
C--  ?  -             xnlat, xnlon, xflat, xflon,
C--  ?  -             xc,marker,nodeflag,amask,sat, sensor, NM)
C--     -         NRECSM = NRECSM + NM
...
C--     -         CALL MaskFormat(dbproc, '-',x,
C--  ?  -               xnlat,xnlon,xflat,xflon,
C--  &  -                        xc, marker, nodeflag, amask, sat, 
C--  &  -                        sensor, NM)
C--     -         NRECSM = NRECSM + NM
...
c
      CLOSE (UNIT = EUNIT)
      WRITE(*,*)' MASKINOUT:  # maskinout RECS APPENDED = ', NRECSM
     ...
     ?             ' phase.  The run '
      ...
      print *,'maskinout.for:  completed generating maskinout records ',
     ?        'for ', sat, '.'
...
c---      WRITE(*,*)' MASKINOUT:  PROGRAM TERMINATING NORMALLY.'
      print *,' ' 
      rcode = system('date')
      maskinout = APS_EXIT_OK
      return
      END
...
C---      CALL ET2ASF(ETIME,ASFTIME)
      ...
      WRITE(*,*) 
     ?' REVIEW THE MASKINOUT FILE AT THE ABOVE TIME.  THE POINT SHOULD'
      WRITE(*,*) 
     ...
     ?' IFLAG = 2 MEANS THAT NEITHER POINTS ARE INSIDE THE CIRCLE.'
      WRITE(*,*) 
     ?' (WE ARE TALKING ABOUT THE NON-INTERPOLATED MASKINOUT RECORDS.)'
      WRITE(*,*) 
