...
C-- ==========================================================================
     ...
     -/'@(#)easap.for	5.1 98/01/08 APS/ASF\0'/
...
****************************************************************
*  ASAP CHANGE LOG
*  BY:  MARTIN LO               DATE: 1/24/89
****************************************************************
*
*  CHANGES:  
*
*  CHANGE ASAP INPUT OF THE START/STOP AND REFERENCE TIMES
*  AND THE INITIAL STATE VECTOR.  FORMERLY TINT, TFIN, TREF, ORB, PM WERE
*  READ IN FROM FILE 5.  THEY ARE STILL READ IN FROM FILE 5, BUT THE
*  VALUES ARE NOT USED.  INSTEAD, TID,TFD,TRD,X, AND PM ARE READ IN DIRECTLY 
*  FROM FILE 9 (ASAP.VEC) WHICH IS GENERATED BY THE EPHM PROGRAM. IN
*  THIS WAY, THE STRUCTURE OF FILE 5 NEED NOT CHANGE.
*
*  AFFECTED VARIABLES:
*
*  IORB  : HAS BEEN SET TO 0 AND THE COORDINATE TRANSFORMATION SECTION
*          OF ASAP HAS BEEN DISCONNECTED.
*  TID   : TIME OF INITIAL CONDITIONS IN JULIAN DAYS.
*  TSTART: START TIME OF EPHEMERIS OUTPUT.
*  TFD   : STOP  TIME IN JULIAN DAYS.
*  TRD   : REFERENCE EPOCH IN JULIAN DAYS.
*  X     : THE OSCULATING CARTESIAN STATE VECTOR (R,V) IN KM AND
*          KM/SEC, IN MEAN EQUATOR EQUINOX OF EPOCH COORDINATES AT 
*          TIME TID.
*  PM    : EARTH PRIME MERIDIAN ANGLE AT TIME TRD
*  DISCARDED VARIABLES: TINT,TFIN,TREF,ORB
*
****************************************************************
C
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)   
c---port file_name declaration
      character*100 file_name_5, file_name_9
      DIMENSION ORB(6),X(6)
      DIMENSION TINT(2),TFIN(2),TREF(2)
C--------------------- SPECIAL COMMON FOR MPS
...
c---port	tfname was created to translate the file name by decoding
c---port	environment variables.  
      call tfname('MPS_ASAP9', file_name_9)
      print *, 'easap.for:  MPS_ASAP9:  filename_9 = ', file_name_9
c---port      OPEN(9,FILE=file_name_9,READONLY,STATUS='OLD')
      OPEN(9,FILE=file_name_9,STATUS='OLD')
**********
c---port       OPEN(5,FILE='MPS_ASAP5',READONLY,STATUS='OLD')
      call tfname('MPS_ASAP5', file_name_5)
      print *, 'easap.for: MPS_ASAP5 file_name5 = ', file_name_5
c---port      OPEN(5,FILE=file_name_5,READONLY,STATUS='OLD')
      OPEN(5,FILE=file_name_5,STATUS='OLD')
...
C*********
C
C---	READ EPHEMERIS START TIME, FINAL TIME, EPOCH/REFERENCE TIME, 
C---	PRIME MERIDIAN DEGREES (AT EPOCH), AND R, V IN 
C---	CARTESIAN COORDS.
      IORB=0  ! INPUT VALUES ARE OSCULATING R,V
C---	TSTART IS IN COMMON BLOCK.
      READ(9,*) TSTART
      READ(9,*) TFD
      READ(9,*) TRD
      READ(9,*) PM
      READ(9,*) (X(I), I=1, 6)
C---	INITIAL CONDITIONS (X) ARE AT EPOCH TIME.
      TID = TRD
C*********
C
C  BEGIN OUTPUT INPUT DATA
C
      WRITE(*,4000)L,M,IRES,ISUN,IMOON,IEPHEM,IDRAG,IDENS,ISRP,IORB
     1  ,IPRINT,INODE,IPLOT
      WRITE(*,3000)(ORB(I),I=1,6),RELERR,ABSERR,STEP
      WRITE(*,3000)TINT,TFIN,TREF
      WRITE(*,3000)GE,RE,RATE,PM,ELLIP,RATM
      WRITE(*,3000)RDENS,RHT,SHT,ALTMAX,WT
      WRITE(*,3000)AREAD,AREAS,SCMASS,CDRAG,CSRP
      WRITE(*,3000)GS,(ES(I),I=1,7)
      WRITE(*,3000)GM,(EM(I),I=1,7)
C
...
C
****************************************************************
*  JULIAN DATES READ IN FROM FILE 9 'ASAP.VEC'
****************************************************************
...
****************************************************************
   ...
   60 CONTINUE
      ...
      TR=TRD*DTS
****************************************************************
*   OSCULATING CARTESIAN STATE VECTORS R,V ARE READ FROM FILE 9
...
C
      IF (IPLOT.EQ.1) OPEN (8,FILE='8.DAT',STATUS='NEW')
C
...
C
 ...
 6000 FORMAT(1P,1H1,/
     1      ,5X,'RUN INITIALIZES ON JULIAN DATE        = ',D25.16,/
