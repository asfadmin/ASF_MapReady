...
static char sccsid_qimagefile_c[] =
    "@(#)qimagefile.c	1.21 96/12/30 15:27:16";
...
read_ceos_file()
{
   ...
   if (! _average_in_file) {
     /* If there's no average file */
     ceos_file_descriptor = open(_filename, O_RDONLY);
   }
   else { 
      /* If we've got an average file to work with... */
     ceos_file_descriptor = open(_average_in_file, O_RDONLY);
   }
    ...
    * _bottom_border_lines, _suffix_pixels), whose values are found in
    * the first record of the file.  Return the _format_type with
    * (char *).  */
...
{ 
   ...
   if (! strncmp(_format_type, "UNSIGNED INTEGER", 16)) {
      if (_pixel_spacing == (double) FULL_REZ_PIXEL_SPACING)
      ...
      *picture_only_byte_data = set_up_complex_image(ceos_file_descriptor);
      create_histogram_from_complex_picture(*picture_only_byte_data);
   }
      ...
      sprintf(image_type_failure_announcement, "Image type %s unknown", 
 	       _format_type);
      make_announcement(image_type_failure_announcement, LOG);
