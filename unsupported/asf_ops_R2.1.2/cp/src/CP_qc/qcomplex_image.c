...
static char sccsid_qcomplex_image_c[] = "@(#)qcomplex_image.c	1.21 97/06/03 11:20:12";
   ...
   int   swath_number, location_in_swath, ceos_file_descriptor;
{
   ...
   if ((swath_number == 0) && (location_in_swath == 0)) {
 	 ...
 	 sprintf(malloc_announcement, "Cannot allocate %d bytes in memory for current_picture_data_swath", _image_size_x * _image_scale_factor);
 	 ...
 	 char malloc_announcement[200];
 	 sprintf(malloc_announcement, "Cannot allocate %d bytes in memory for current_raw_data_swath", _records_length * _image_scale_factor);
 	 make_announcement(malloc_announcement, LOG);
 	 exit(QC_MALLOC_ERROR);
      }
   }
   ...
   short *raw_complex_data_swath;
{
   ...
   else if (_complex_scale_parameter != 0.0)
      _complex_multiply_factor = _complex_scale_parameter;
   else if (_data_mean != 0.0)
      _complex_multiply_factor = 255.0 /(_data_mean + 2*_data_sdev)/1.4; 
   else 
   ...
   int   ceos_file_descriptor;
{
   ...
   if (!scaled_complex_image_byte_data) {
      ...
      sprintf(malloc_announcement, "Can't allocate %d bytes in memory for the image portion of the file", (scaled_image_size_x * scaled_image_size_y) );
      make_announcement(malloc_announcement, LOG);
