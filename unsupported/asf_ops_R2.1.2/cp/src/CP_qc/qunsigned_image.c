...
static char sccsid_qunsigned_image_c[] = "@(#)qunsigned_image.c	1.22 97/06/12 16:53:15";
...
***/
	...
	char problem[333];
	sprintf(problem, "CEOS error: image size x is %d, pixels per line is %d",
	...
	char problem[333];
	sprintf(problem, "CEOS error: image height is %d, lines per channel is %d",
   ...
   int   swath_number, location_in_swath, ceos_file_descriptor;
{
   ...
   if (location_in_swath == 0) {       
      if (_byte_factor == 1 || _avg_file_flag)
   ...
   for (row_index = start_row;
	row_index < end_row;
	row_index += _image_size_x)
     for (column_index = start_column;
...
read_averaged_file()
{
   ...
   if (number_of_bytes_read != average_file_size) {
      ...
      sprintf(read_announcement, 
             "Can't read image average file (%d bytes)", average_file_size);
      make_announcement(read_announcement, LOG);
      free(read_announcement);
      exit(QC_AVERAGE_FILE_UNEXPECTED_READ_RETURN_ERROR);
   }
