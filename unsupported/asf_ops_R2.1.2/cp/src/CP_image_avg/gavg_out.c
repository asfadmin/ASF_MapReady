...
static char sccsid_gavg_out_c[] = "@(#)gavg_out.c	1.4 97/02/12 09:25:54";
...
put_together_average_file()
{
   ...
   printfLLog(LOG_INFO, "Output file saved as %s", _output_to_file);
}
   ...
   byte *first_record_bytes;
{
   ...
   int new_recs_length = (_image_size_x / _image_scale_factor) 
                         + _left_border_pixels ;
/***
                         + _left_border_pixels + _right_border_pixels;
***/
...
***/
   ...
   for (row_index = 0; row_index < scaled_image_size_y; row_index++) {
 	 ...
 	 putc(_small_picture[image_index++], writeable_file_pointer);
      }
