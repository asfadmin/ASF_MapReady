 ...
 * 11/19/92 input a fudge factor for the xoffset 
 */


/* az4l(forfftsize,roffset,flag_azwgt,flag_deskew,prf,pbw,fzero2nul,h, -----
      azspacing, azsample, fdgrd, frgrd, skew_adj, fudge_fctr,
 ...
 INPUT:
   ...
   fudge_fctr	 int	   fudge factor as per Tom Bicknell for
			   computing xoffset. 250 for JERS, 0 for ERS-1.
...
extern RQST_PTR Cur_Rqst; /* CV 5/1/97 */
...
float     *err_offset,*totalskew,fudge_fctr;
...
/*  imin   = (float)(ceil((double)(maxnov))); */
  if (forfftsize >= maxnov)
  	imin = (float)(ceil(forfftsize/10.));
  else
  	imin = (float)(ceil((double)(maxnov)));
  delerror = 0.04;
...
/*  if( (skewfar - skewnear)  > 0 ) comment out by CV 5/1/97  
       total_offset_skew = total_skew;
*/
  if( Cur_Rqst->take_id[0]=='R' ) { /* for Ming to fix skew problem,CV 5/1/97 */
     total_offset_skew = total_skew;
     
/*     printf("VUU: TEST SKEW PROBLEM, t_skew=%g\n", total_offset_skew);*/
  }
  else {
  ...
  *totalskew = total_offset_skew;
  ...
  for(kline = kstart; kline < kend; kline++){
    ...
    if( (skewfar - skewnear)  > 0 ) /* CV for Ming, 5/2/97 */
        ioffset = forfftsize - (int)(total_offset_skew) + (int)(xskew);
    else 
        ioffset = forfftsize + (int)(xskew);
/*ioffset = forfftsize - (int)(total_offset_skew) + (int)(xskew);CV 5/2/97 */
