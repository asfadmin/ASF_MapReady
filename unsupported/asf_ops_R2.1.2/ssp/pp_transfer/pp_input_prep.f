...
c SccsId = @(#)pp_input_prep.f	2.41 3/24/98
	subroutine pp_input_prep(i_file_type,file_config,istatus,kids,mytask)
       include 'ssp2_const.inc'


        character*128 SccsId
        data SccsId /'@(#)PPpp_input_prep.f:2.41'/
...
c        if (mytask.eq.0) idummy=printfstat(file_status,1,0.0)

c ceos_leader_file,image_file,pmf_file,cali_file  are outputs
	write(6,*)'before call to pp_get_node istatus = ',istatus
        istatus= pp_get_node(i_file_type,echo_file,aux_file,rep_file,eph_file,
	...
	write(6,*)'after call to pp_get_node istatus = ',istatus
        if (istatus.ge.ierr_lower.and.istatus.le.ierr_upper) return
