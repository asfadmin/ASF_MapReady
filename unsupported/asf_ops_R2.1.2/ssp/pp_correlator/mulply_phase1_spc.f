c SccsId = @(#)mulply_phase1_spc.f	2.41 3/24/98

	
	subroutine mulply_phase1_spc(
     ...
     +  raw_file,image_file)
        ...
        character*128 SccsId_mulply_phase1_spc
        data SccsId_mulply_phase1_spc
     +  /'@(#)PPmulply_phase1_spc.f:2.41'/
	...
	real*8 f,a,k1,t_ref,r,dt,phase,fd_ref,fr_ref,scale_f,scale_fref
	real*8 fd_cnt
	...
	k1 = 1./(1./k_chirp - (f/fc)**2/abs(fr_ref))
	t_ref = 2/c * (r_ref - lambda/4*(f**2-fd_ref**2)/fr_ref) 
     1		+ plen/2
