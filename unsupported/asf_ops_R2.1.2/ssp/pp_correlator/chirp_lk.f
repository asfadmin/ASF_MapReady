c SccsId = @(#)chirp_lk.f	2.41 3/24/98


	subroutine chirp_lk(ir_st,ix_st,nadd_error,
     ...
     +  raw_file,image_file)
        ...
        character*128 SccsId_chirp_lk
        data SccsId_chirp_lk
     +  /'@(#)PPchirp_lk.f:2.41'/


c*******************************************************
c*     chirp scaling correlator written by m. jin      *
c*******************************************************
	...
	character*64 image_file	    !image data file name
        ...
        real t2,secnds,t1
        integer nbyte,iptr,ix_st,ir_st,iflag, nadd_error
...
c***************************************************
c** start range & azimuth compression             **
c***************************************************
...
d       call dtime(time)
d	print *, 'azimuth fft ', time(1), time(2)
...
d       call dtime(time)
d	print *, 'expand spectrum ', time(1), time(2)

c** multiply the first phase function *********

	call mulply_phase1_spc(
     +  rng_in,az_in,nadd,n_max,pi,c,pxl_unit,
     +  ers,jers,rsat_s12,rsat_s37,rsat_ws,rsat_fr,rsat_eh,rsat_el,
     +  buff_in,array,buff_in_rd,
     +  mode,
...
d       call dtime(time)
d	print *, 'multiply the first phase function ', 
d    +           time(1), time(2)
...
d       call dtime(time)
d	print *, 'range fft ', time(1), time(2)

c** multiply the second phase function *********

	call mulply_phase2_spc(
     +  rng_in,az_in,nadd,n_max,pi,c,pxl_unit,
     +  ers,jers,rsat_s12,rsat_s37,rsat_ws,rsat_fr,rsat_eh,rsat_el,
     +  buff_in,array,buff_in_rd,
     +  mode,
...
d       call dtime(time)
d	print *, 'multiply the second phase function ', 
d    +           time(1), time(2)
...
d       call dtime(time)
d	print *, 'range inverse fft ', time(1), time(2)

c** multiply the third phase function *********

	call mulply_phase3_spc(
     +  rng_in,az_in,nadd,n_max,pi,c,pxl_unit,
     +  ers,jers,rsat_s12,rsat_s37,rsat_ws,rsat_fr,rsat_eh,rsat_el,
     +  buff_in,array,buff_in_rd,
     +  mode,
...
d       call dtime(time)
d	print *, 'multiply the third phase function ', 
d    +           time(1), time(2)

c** select bandwidth centered at doppler centroid *********
...
d       call dtime(time)
d	print *, 'select bandwidth centered at doppler centroid ', 
d    +            time(1), time(2)
...
d       call dtime(time)
d	print *, 'reduce spectrum ', time(1), time(2)

c** azimuth inverse fft ***********************
...
d       call dtime(time)
d	print *, 'azimuth inverse fft ', time(1), time(2)

c** masking out the substandard pixels ********

	call truncate(ir_st,ix_st,
     +  rng_in,az_in,nadd,n_max,pi,c,pxl_unit,
     +  ers,jers,rsat_s12,rsat_s37,rsat_ws,rsat_fr,rsat_eh,rsat_el,
     +  buff_in,array,buff_in_rd,
     +  mode,
...
d       call dtime(time)
d	print *, 'masking out the substandard pixels ', 
d    +           time(1), time(2)

c** Circular Shift data azimuth dimension **********************

        call circ_shft(ir_st,ix_st,nadd_error,
     +  rng_in,az_in,nadd,n_max,pi,c,pxl_unit,
     +  ers,jers,rsat_s12,rsat_s37,rsat_ws,rsat_fr,rsat_eh,rsat_el,
     +  buff_in,array,buff_in_rd,
     +  mode,
...
d       call dtime(time)
d	print *, 'circular shift data azimuth dimension ', 
d    +           time(1), time(2)


c******* write output data *************************
 	iptr = 0
	nbyte=8*rng_in*az_in
c	call write_disk('rimage.dat',buff_in_rd,nbyte,iptr)
