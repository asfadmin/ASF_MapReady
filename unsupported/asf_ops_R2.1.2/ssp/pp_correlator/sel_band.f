c SccsId = @(#)sel_band.f	2.41 3/24/98
	...
	subroutine sel_band(
     ...
     +  raw_file,image_file)
        ...
        character*128 SccsId_sel_band
        data SccsId_sel_band
     +  /'@(#)PPsel_band.f:2.41'/
	...
	if(f.gt.fd+pbw/2.or.f.lt.fd-pbw/2) then
	   buff_in(i,j) = cmplx(0.,0.)
	else
	   ioffset = nint((f -fd)/df) + az_in
	   buff_in(i,j) = buff_in(i,j) * kw(az_in+ioffset)
	end if
