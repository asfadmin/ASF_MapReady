c SccsId = @(#)truncate.f	2.41 3/24/98


	
	subroutine truncate(ir_st,ix_st,
     ...
     +  raw_file,image_file)
        ...
        character*128 SccsId_truncate
        data SccsId_truncate
     +  /'@(#)PPtruncate.f:2.41'/
	...
	integer i,j,ir_st,ix_st,naz
	real*8 r, t, dt, t1,t2,f,fd,fr,fd_ref
	real*8 rw1, rw2, r_end, fd_end, fr_end, t_period, drw1, drw2

	real t_valid,t_shift

        integer index_valid_end,num_invalid,j0
        real r_far,fr_far,fr_min
	...
	fr_far = fr0_base+frd*(r_far-r0_base)
        ...
        dt = 1./prf
        r_end = (rng_in - 1) * dr + r_1st
        fd_end = (rng_in - 1) * dr * fdd + fd0
        fr_end = (rng_in - 1) * dr * frd + fr0
        do i = 1, rng_in
	   r = r_1st+(i-1)*dr
	   fd = (r - r_1st) * fdd + fd0
	   fr = (r - r_1st) * frd + fr0
	   call dop_ref(i_dop_mode,fd0,fdd,r,fd_const,r_1st,fd_ref)
	   drw1 = -lambda/4*(fd**2-fd_ref**2)/abs(fr)
	   if(drw1.lt.0.0) drw1 = 0
	   rw1 = lambda/2 * abs((fd - fr/2 * pbw/(2*fr))
	1	* pbw/(2*fr))
	   if(rw1.lt.r-r_1st) go to 111
        end do
 111	do i = rng_in, 1, -1
	   r = r_1st+(i-1)*dr
	   fd = (r - r_1st) * fdd + fd0
	   fr = (r - r_1st) * frd + fr0
	   call dop_ref(i_dop_mode,fd0,fdd,r,fd_const,r_1st,fd_ref)
	   drw2 = lambda/4*(fd**2-fd_ref**2)/abs(fr)
	   if(drw2.lt.0.0) drw2 = 0
	   rw2 = lambda/2 * abs((fd + fr/2 * pbw/(2*fr))
	1	* pbw/(2*fr))
	   if(rw2.lt.r_end-r) go to 222
        end do
...
c        if(ix_st+az_in-1.gt.nlines) naz = nlines-ix_st+1
	   ...
	   fr = (r - r_1st) * frd + fr0
	   call dop_ref(i_dop_mode,fd0,fdd,r,fd_const,r_1st,fd_ref)
	   t_shift = (fd - fd_ref)/abs(fr)
	   
c       t_valid = naz/prf-pbw/abs(fr)     !This retains as much good data
	   t_valid = naz/prf-pbw/abs(fr_min)
	   
	   t1 = -t_valid/2. + t_shift
	   t2 = t_valid/2. + t_shift
	   t_period = az_in/prf
	   index_valid_end = mod(t2+100.5*t_period,t_period)*prf+1
	   num_invalid = az_in - t_valid*prf 
	   
	   do j = 1, az_in
	      j0 = mod(index_valid_end+j-1,az_in)+1
	      if(j.le.num_invalid) buff_in_rd(i,j0) = cmplx(0.,0.)
	      if(r-r_1st.lt.rw1+drw1+4.*dr.or.r_end-r.lt.(plen*fs+6)*dr+
	1	   rw2+drw2)  buff_in_rd(i,j0) = cmplx(0.,0.)
	   end do
