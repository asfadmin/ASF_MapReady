c SccsId = @(#)reduce.f	2.41 3/24/98
	...
	integer i,j,jj
	real*8 f,fd_cnt,fd_start
	
	fd_cnt = fd0 + (rng_in-plen*fs)/2*dr* fdd
	fd_cnt = nint(fd_cnt/df)*df
	fd_start = fd_cnt - prf/2
	
	do i = 1, rng_in

	do j = 1, az_in
	array(j) = buff_in_rd(i,j) 
	end do

	if(nband.gt.az_in) then
	do j = 1, (nband-az_in)/2
	array(j) = array(j) +buff_in(i,az_in+nadd/2+j)
	array(az_in-j+1) = array(az_in-j+1) +buff_in(i,nadd/2-j+1)
	end do
	end if

	do j = 1, az_in
	f = (j-1)*df + fd_start
	jj = nint(mod(f+100*prf,prf)/df) + 1
	if(jj.le.az_in) buff_in_rd(i,jj)  = array(j)
	end do
