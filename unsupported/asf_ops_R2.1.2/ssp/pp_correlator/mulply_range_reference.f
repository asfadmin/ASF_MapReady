c SccsId = @(#)mulply_range_reference.f	2.41 3/24/98
	
	subroutine mulply_range_reference(
     ...
     +  raw_file,image_file)
        ...
        character*128 SccsId_mulply_range_reference
        data SccsId_mulply_range_reference
     +  /'@(#)PPmulply_range_reference.f:2.41'/
	...
	integer i,j
	real*8 k1,fd_ref,fr_ref,mu,mu0,phase,dphase
	...
	array(i) = cmplx(cos(phase),sin(phase))*kw(i)
	end if
	...
	buff_in_rd(i,j)=buff_in_rd(i,j)*array(i)

	end do
	buff_in_rd(1,j)=cmplx(0.,0.)
