c SccsId = @(#)expand.f	2.41 3/24/98
	...
	buff_in_rd(i,j)  = array(jj)
	end do
	
	do j = 1, (nband-az_in)/2
	buff_in(i,nadd/2-j+1) = buff_in(i,az_in+nadd/2-j+1)
	buff_in(i,j+az_in+nadd/2) = buff_in(i,j+nadd/2)
	end do
