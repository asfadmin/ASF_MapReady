c SccsId = @(#)r_resample_16.f	2.41 3/24/98




	subroutine r_resample_16(w,ir_st,ix_st,scale,image_buffer,
	...
	integer ierror
        ...
        character*128 SccsId_r_resample_16
        data SccsId_r_resample_16
     +  /'@(#)PPr_resample_16.f:2.41'/
	...
	real*8 rg_ref,r,rg
	real*8 r_center,rg_center,fdline
	real w(64,128)
	...
	integer*2 image_buffer(nfr*nfx)

	real*4 time(2)

	if(na_smp .gt. 8192) then
	   print *,'r_resample_16:na_smp > 8192, increase imag_line size'
	...
	do i = 1, nint((rg_end-rg_start)/dr)	!range resampling loop
	rg = (i-1)*dr + rg_start
	ir_loc = nint((rg - rg_ref)/dr) + 1
	if(ir_loc.gt.nfr) return

	k1 = i

	do j = 1, az_in+nadd
	   resam_buff(j) = buff_in(k1,j)
	end do

	fdline = (rg - r_1st)*fdd + fd0
	...
	call spat_avg_16(ia_loc_d2,na_smp,scale(ir_loc),
     +  rng_in,az_in,nadd,n_max,pi,c,pxl_unit,
     +  ers,jers,rsat_s12,rsat_s37,rsat_ws,rsat_fr,rsat_eh,rsat_el,
     +  buff_in,array,buff_in_rd,
     +  mode,
	...
	call overlay_16(ir_loc,ia_loc_d2,na_smp,0,image_buffer,
     +  rng_in,az_in,nadd,n_max,pi,c,pxl_unit,
     +  ers,jers,rsat_s12,rsat_s37,rsat_ws,rsat_fr,rsat_eh,rsat_el,
     +  buff_in,array,buff_in_rd,
     +  mode,
