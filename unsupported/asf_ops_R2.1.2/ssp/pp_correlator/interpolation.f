c SccsId = @(#)interpolation.f	2.41 3/24/98

	
	subroutine interpolation(w,
     ...
     +  raw_file,image_file)
        ...
        character*128 SccsId_interpolation
        data SccsId_interpolation
     +  /'@(#)PPinterpolation.f:2.41'/
	...
	integer i,j,jj,j_start,npt
	real*8 r,del_r,f,fd,fd_ref,f_start,fr
	real w(64,128),ai
	complex temp

c variables from inlined subroutine n_pt_intp
	integer ix,ix1,index
	real dx

	real*8 wght1(4),wght2(4),wght3(4),wght4(4)
	call weight(wght1, wght2, wght3, wght4, 4)


	npt = 4		!# of points for interpolation
	...
	do i = 1, rng_in-plen*fs
	   ...
	   do j = j_start, j_start+az_in-1
	      ...
	      del_r = -lambda/2 * .5 * (f**2-fd_ref**2)/fr
	      ai = del_r/dr + i
	      
c       inlining subroutine n_pt_intp 
	      dx = mod(10000.+ai,1.)
	      ix1 = nint(ai-dx)+plen*fs !r bias = plen*fs
	      if(ix1.lt.npt/2.or.ix1.ge.2048-npt/2) then
		 temp = cmplx(0.,0.)
	      else
		 index = 1+ int(dx*128.)
c       loop unrolling.  since npt=4, we have i=31...34
		 temp=w(31,index)*buff_in_rd(ix1-32+31,jj)+
	1	      w(32,index)*buff_in_rd(ix1-32+32,jj)+
	2	      w(33,index)*buff_in_rd(ix1-32+33,jj)+
	3	      w(34,index)*buff_in_rd(ix1-32+34,jj)
	      end if
c       end of inlining subroutine n_pt_intp
	      
c	call n_pt_intp(w,npt,temp,ai,jj,
c     +  rng_in,az_in,nadd,n_max,pi,c,pxl_unit,
c     +  ers,jers,rsat_s12,rsat_s37,rsat_ws,rsat_fr,rsat_eh,rsat_el,
c     +  buff_in,array,buff_in_rd,
c     +  mode,
c     +  raw_mode,i_dop_mode,nband,nlines,line_offset,
c     +  ns,nr_header,ns_header,i_squint,i_pt_sim,i_data_type,nlook,
c     +  nfr,nfx,icomplex,i_preproc,i_iqqi,nx_offset,i_dummy,
c     +  lambda,fc,bw,dmu,fs,plen,k_chirp,dr,
c     +  prf,prf_in,pbw,az_ang,az_res,df,
c     +  t_1st_echo,t_eph,t_gha,gha,xs,ys,zs,rsc,vxs,vys,vzs,vs,
c     +  re_mean,h,
c     +  rs_ref,r0_base,fd0_base,fr0_base,fd0,fr0,fdd,frd,
c     +  fd_const,r_ref,r_1st,
c     +  pxl_spacing,nx_spacing,
c     +  t_start_frm,t_end_frm,
c     +  lat1,lon1,rs_crn1,rg_crn1,lookang1,
c     +  lat2,lon2,rs_crn2,rg_crn2,lookang2,
c     +  lat3,lon3,rs_crn3,rg_crn3,lookang3,
c     +  lat4,lon4,rs_crn4,rg_crn4,lookang4,
c     +  lat5,lon5,rs_crn5,rg_crn5,lookang5,
c     +  raw_file,image_file)

	      array(jj) = temp
	   end do
	   
	   do j = 1, az_in
	      buff_in_rd(i,j) = array(j)	
	   end do
