c SccsId = @(#)overlay_16.f	2.41 3/24/98

	subroutine overlay_16(ir_loc,ia_loc_d2,na_smp,ireset,
     +  image_buffer,
	...
	integer ierror
        ...
        character*128 SccsId_overlay_16
        data SccsId_overlay_16
     +  /'@(#)PPoverlay_16.f:2.41'/
	...
	integer itemp,n_bytes,bytes1,dk_ptr,ival
	real*8 fd_far,fr_far
	real*8 fd_ref_base, fd_ref_far,r_far
	complex resam_buff(8192),resam_buff2(8192)
	integer*2 imag_line(8192)
	common /resbuf/resam_buff,resam_buff2,imag_line

	integer j,i,k,number2,number1,ia_loc_d2,ir_loc,na_smp
	integer ia_loc,ireset

	integer*2 image_buffer(nfr*nfx)

	if(na_smp .gt. 8192) then
	   print *,'overlay_16: na_smp > 8192, increase imag_line size'
	   call mpi_abort(mpi_comm_world,22,ierror)
           call exit(22)
        end if

	ia_loc = ia_loc_d2/nint(pxl_spacing/pxl_unit) + 1

	if(ia_loc+na_smp.gt.nfx) na_smp = nfx-ia_loc+1
	bytes1=na_smp
	dk_ptr=(ir_loc-1)*nfx+ia_loc-1
	do k = 1, na_smp
	   number1 = imag_line(k)
	   number2 = image_buffer(dk_ptr+k)
	   if(number1.ne.0.and.number2.eq.0) 
     +        image_buffer(dk_ptr+k) = number1
	end do
