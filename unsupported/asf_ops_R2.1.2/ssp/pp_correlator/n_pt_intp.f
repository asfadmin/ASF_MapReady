c SccsId = @(#)n_pt_intp.f	2.41 3/24/98
	

	subroutine n_pt_intp(w,npt,aout,x,jj,
     ...
     +  raw_file,image_file)
        ...
        character*128 SccsId_n_pt_intp
        data SccsId_n_pt_intp
     +  /'@(#)PPn_pt_intp.f:2.41'/
	...
	integer i,jj,ix,ix1,index,npt
	complex aout
	real w(64,128)
	real x,dx

	dx = mod(10000.+x,1.)
	ix1 = nint(x-dx)+plen*fs  	!r bias = plen*fs
	...
	index = 1+ int(dx*128.)
	aout = cmplx(0.,0.)
	do i = 33-npt/2,32+npt/2
	aout=aout+w(i,index)*buff_in_rd(ix1-32+i,jj)
	end do
	end if
	return
	end
