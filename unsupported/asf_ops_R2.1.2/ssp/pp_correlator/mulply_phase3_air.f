c SccsId = @(#)mulply_phase3_air.f	2.41 3/24/98


	subroutine mulply_phase3_air(
     ...
     +  raw_file,image_file)
        ...
        character*128 SccsId_mulply_phase3_air
        data SccsId_mulply_phase3_air
     +  /'@(#)PPmulply_phase3_air.f:2.41'/
	...
	k1 = 1./(1./k_chirp - lambda**3*r_ref*f**2/(2*c**2*vs**2))
	k2 = k1 * (1 + a)
	...
	dt = 2 * (r - r_ref)/c
	phi1 = -pi * k2 * a * dt**2
	phi2 = 4 * pi/lambda * (r - r_ref) * (1./(a + 1) - 1.)
	phase = phi1 + phi2
