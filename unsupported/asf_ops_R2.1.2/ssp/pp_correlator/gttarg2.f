c SccsId = @(#)gttarg2.f	2.41 3/24/98


        subroutine gttarg2(rst0,fd0,lambda,xs,ys,zs,vxs,vys,vzs,h,
     1             px,py,pz,xt,yt,zt,vxt,vyt,vzt,alkang,yaw,frame_mode)

        implicit none

        character*128 SccsId_gttarg2
        data SccsId_gttarg2
     +  /'@(#)PPgttarg2.f:2.41'/
...
c        print *, "gttarg2: initial quess for look angle: ", alkang
...
1111    continue
     ...
     +                frame_mode)
        ...
        if(n_iter.gt.n_iteration) then
           print *, 'gttarg2: cannot see target after ',
     +     n_iter,' iterations'
