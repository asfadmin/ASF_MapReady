c SccsId = @(#)interp2.f	2.39 3/24/98

	subroutine interp2(ir_st,ix_st,nadd_error,
     ...
     +  raw_file,image_file)
        ...
        character*128 SccsId_interp
        data SccsId_interp
     +  /'@(#)PPinterp2.f:2.39'/


c*******************************************************
c****  interpolation correlator written by m. jin      *
c*******************************************************
	...
	integer iflag
	real w(64,128),r_value
	
	integer ix_strng_in,iptr,nbyte,ir_st,ix_st,i, nadd_error
...
c** data is already decoded & range cfft & 
c** multiply range ref & inverse range cfft  *****

d	call dtime(time)
d	print *, 'decode,range compress ', time(1), time(2)

c** azimuth fft ******************************

	iflag = 1
	call azfft_intp(iflag,
     +  rng_in,az_in,nadd,n_max,pi,c,pxl_unit,
     +  ers,jers,rsat_s12,rsat_s37,rsat_ws,rsat_fr,rsat_eh,rsat_el,
     +  buff_in,array,buff_in_rd,
     +  mode,
...
d	call dtime(time)
d	print *, 'interpolation ', time(1), time(2)

c** multiply the azimuth phase function *********
	call mulply_azimuth_reference(
     +  rng_in,az_in,nadd,n_max,pi,c,pxl_unit,
     +  ers,jers,rsat_s12,rsat_s37,rsat_ws,rsat_fr,rsat_eh,rsat_el,
     +  buff_in,array,buff_in_rd,
     +  mode,
...
d	call dtime(time)
d	print *, 'multiply the azimuth phase function ', 
d    +           time(1), time(2)
...
c	call write_disk('r_image.dat',buff_in_rd,nbyte,iptr)
        ich_fd = initdk(idum,'/home/tmpdisk/r_image.dat'//char(0))
        call iowrit(ich_fd,buff_in_rd,nbyte)
        call closedk(idum,ich_fd)
