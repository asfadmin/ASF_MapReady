c SccsId = @(#)spat_avg_16.f	2.41 3/24/98



	subroutine spat_avg_16(ia_loc_d2,na_smp,scale,
	...
	integer ierror
        ...
        character*128 SccsId_spat_avg_16
        data SccsId_spat_avg_16
     +  /'@(#)PPspat_avg_16.f:2.41'/
	...
	real*8 scale                ! scale factor applied to image

	real temp,wavg(9)
	complex resam_buff(8192),resam_buff2(8192),w_kaiser(8192)
	integer*2 imag_line(8192)
	common /resbuf/resam_buff,resam_buff2,imag_line

	integer k,njump,kk,itemp,k1,na_smp,ia_loc_d2,npt,i,n_3db_1side
	integer i_from
	real phase,dis,alpha

	if(na_smp .gt. 8192) then
	   print *,'spat_avg_16:na_smp > 8192, increase imag_line size'
	...
	n_3db_1side = 64
	alpha = 0.8
	call kaiser(w_kaiser,64,alpha)
	call irgary(w_kaiser,64,8192)
	call cfft(w_kaiser,1,8192)

	if(nlook.ne.1) then
	   npt = nlook*2+1
	   do i = 1, npt
	      dis = (i-npt/2)*pxl_unit
	      i_from = nint(dis/az_res*n_3db_1side) + 1
	      wavg(i) = abs(w_kaiser(i_from))**2

c	      phase = pi * dis/az_res
c	      if(phase.eq.0) then
c		 wavg(i) = 1.
c	      else
c		 wavg(i) = (sin(phase)/phase)**2
c	      end if

	   end do
	   
	   njump = pxl_spacing/pxl_unit
	   do k = 1, na_smp
	      kk = njump*(k-1)+1
	      temp = 0.
	      do k1 = 1, npt
		 if(kk-npt/2+k1.lt.1.or.kk-npt/2+k1.gt.8192)then
		    temp = cmplx(0.,0.)
		    goto 1111
		 else if(abs(resam_buff2(kk-npt/2+k1)).eq.0) then
		    temp = cmplx(0.,0.)
		    goto 1111
		 else 
		    temp=temp+wavg(k1)*abs(resam_buff2(kk-npt/2+k1))**2
		 end if
	      end do
 1111	      itemp = sqrt(temp)*scale
c Since Fortran integer*2 is signed, the largest number it can store is 2^15-1=
c 32767.  We have to store numbers > 32767 as negative numbers which supposedly
c turns on the 16th bit.  The number would be treated as 32768 + number in 
c lower 15 bits.
	      if(itemp.gt.65535) itemp = 65535
	      if(itemp.gt.32767) itemp = itemp - 65536
	      imag_line(k) = itemp
	   end do
	end if
	...
	if(nlook.eq.1) then
	   do k = 1, na_smp
	      temp = abs(resam_buff2(k))
	      if(temp.gt.65535) temp = 65535
