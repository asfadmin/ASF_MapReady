c SccsId = @(#)pt_echo.f	2.41 3/24/98

	subroutine pt_echo(rt_center,h_offset,
	...
	real*8 rt_center,h_offset,mask_az,mask_rng
	real*8 dis,dta,r,t,dt,dtau,phase0,phase,fr,fd,fd_ref
	real*8 t_ref,az_thi,az_thi_d,fd_instan,t_bias,tp 
	integer i,j

	dt = 1./prf	
	fr = fr0 + (rt_center - r_1st)* frd
	call dop_ref(i_dop_mode,fd0,fdd,rt_center,fd_const,r_1st,fd_ref)
	fd = fd_ref
	t_bias = -(fd-fd_ref)/fr
	tp = (4*vs*sind(az_ang/2)/lambda)/abs(fr)

	t_ref = az_in/2*dt + h_offset/vs 

	do j = 1, az_in
	t = ( j - 1) * dt

c	if(j/256*256.eq.j) print *, 'aa', j

	dta = t - t_ref 
	fd_instan = fd_ref + fr * dta
	az_thi = atan2d(dta*vs,rt_center)
	mask_az=1.

	if(mode.eq.1) then
	if(abs(az_thi).gt.az_ang/2.) mask_az=0.
	dis = sqrt(rt_center**2 + (vs*dta)**2)
	else
	if(abs(dta+t_bias).gt.tp/2.) mask_az=0.
	dis = rt_center -lambda/2 * (fd_ref + fr/2 * dta) * dta 
	end if

	phase0 = -4. * pi/lambda * dis 
	phase0 = phase0 - nint(phase0/(2*pi))*2*pi
	if(phase0.lt.-pi) phase0 = phase0 +2*pi
	if(phase0.gt.pi) phase0 = phase0 -2*pi
c	if(abs(dta).lt.dt/10000) print *,'p=',phase0*180./pi,'dis= ', dis
c	if(abs(dta).lt.dt/10000) print *,'(c,s)=',cos(phase0),sin(phase0)
	
	do i = 1, rng_in
	r = r_1st + (i - 1)*dr
	dtau = 2 * (r - dis)/c
	mask_rng=1.
	if(dtau.gt.plen.or.dtau.lt.0) mask_rng=0.
	phase = phase0 + pi * k_chirp * (dtau- plen/2)**2
     1		+ 2 * pi * fd_instan * (dtau - plen/2)
	buff_in_rd(i,j) = buff_in_rd(i,j) + mask_rng * mask_az *
