c SccsId = @(#)mulply_phase2_spc.f	2.41 3/24/98



	subroutine mulply_phase2_spc(
     ...
     +  raw_file,image_file)
        ...
        character*128 SccsId_mulply_phase2_spc
        data SccsId_mulply_phase2_spc
     +  /'@(#)PPmulply_phase2_spc.f:2.41'/
	...
	integer i,j,jj,n_cntr
	real*8 mu,mu0,f,a,k1,t_ref,r,dt,phi1,phi2,phi3,phase,phi
	real*8 fd_ref,fr_ref,scale_f,scale_fref
	real*8 fd_cnt
	...
	alpha = 2.8/pi
	call kaiser(kw,rng_in,alpha)
	
	fd_cnt = fd0 + (rng_in-plen*fs)/2*dr* fdd
	fd_cnt = nint(fd_cnt/df)*df
	...
	fr_ref = fr0 + (r_ref - r_1st) * frd
	
	n_cntr = (az_in+nadd)/2 + 1	
	do j = n_cntr-nband/2, n_cntr+nband/2-1
	f = (j-n_cntr)*df +fd_cnt
	
	k1 = 1./(1./k_chirp + (f/fc)**2/fr_ref)
	scale_f = 1 + lambda/4* f**2/fr_ref**2 * frd
	scale_fref = 1 + lambda/4* fd_ref**2/fr_ref**2 * frd
	a = scale_f/scale_fref - 1

	do i = 1, rng_in
	mu = (i-1)*dmu+fc
	mu0 = mu
	if(mu.ge.fc+fd_ref+fs/2) mu = mu-fs
	if(mu0.ge.fc+fs/2) mu0 = mu0-fs

	phi1 = pi * (mu - fc)**2/(k1 * (1 + a)) + phi
	phi2 = pi * (f - (mu/fc*fd_ref))**2/(mu/fc*fr_ref) + pi/4
	phi3 = 2 * pi * (mu0 - fc) * plen/2.
