c SccsId = @(#)interp.f	2.41 3/24/98
	...
	subroutine interp(ir_st,ix_st,nadd_error,
...
c** range cfft *****************
	do i = 1, az_in
	   iflag = 1
	   call cfft(buff_in_rd(1,i),1,rng_in,iflag)
	   buff_in_rd(1,i) = cmplx(0.,0.)
	end do

d	call dtime(time)
d	print *, 'range fft ', time(1), time(2)

c** multiply the range phase function *********
	call mulply_range_reference(
     +  rng_in,az_in,nadd,n_max,pi,c,pxl_unit,
     +  ers,jers,rsat_s12,rsat_s37,rsat_ws,rsat_fr,rsat_eh,rsat_el,
     +  buff_in,array,buff_in_rd,
     +  mode,
...
d	call dtime(time)
d	print *, 'multiply the range phase function ', 
d    +           time(1),time(2)

c** range inverse fft *************************
	iflag = -1
	call rngfft_intp(iflag,
     +  rng_in,az_in,nadd,n_max,pi,c,pxl_unit,
     +  ers,jers,rsat_s12,rsat_s37,rsat_ws,rsat_fr,rsat_eh,rsat_el,
     +  buff_in,array,buff_in_rd,
     +  mode,
