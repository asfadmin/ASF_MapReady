c SccsId = @(#)azfft_intp.f	2.41 3/24/98

	subroutine azfft_intp(iflag,
     ...
     +  raw_file,image_file)
        ...
        character*128 SccsId_azfft_intp
        data SccsId_azfft_intp
     +  /'@(#)PPazfft_intp.f:2.41'/
	...
	if(iflag.eq.1) then
     ...
     +  raw_file,image_file)
	   ...
	   call cfft(array,increa,az_in,iflag)

	   call put_az_line(i+n,
     +  rng_in,az_in,nadd,n_max,pi,c,pxl_unit,
     +  ers,jers,rsat_s12,rsat_s37,rsat_ws,rsat_fr,rsat_eh,rsat_el,
     +  buff_in,array,buff_in_rd,
     +  mode,
