c SccsId = @(#)az_resample.f	2.41 3/24/98




	subroutine az_resample(w,ir_st,ix_st,rg,ia_loc,na_smp,fdline,
	...
	integer ierror
        ...
        character*128 SccsId_az_resample
        data SccsId_az_resample
     +  /'@(#)PPaz_resample.f:2.41'/
	...
	real*8 r_start,rg_start,r_end,rg_end,xg_start,xg_end
	real*8 xg_ref
	real*8 r_center,rg_center
	real*8 ax
	real w(64,128)
	complex resam_buff(8192),resam_buff2(8192)
	byte imag_line(8192)
	common /resbuf/resam_buff,resam_buff2,imag_line
  
	real*8 fdline, phase

        real xg,vsg,rg
	integer j,k,k1,index,nr_total,ix_st,ir_st,npt,na_smp,ia_loc

c       inlining sr2gr introduced variables
	real*8 in,out,alpha
	...
	1    rng_in,az_in,nadd,n_max,pi,c,pxl_unit,
	2    ers,jers,rsat_s12,rsat_s37,rsat_ws,rsat_fr,rsat_eh,rsat_el,
	3    buff_in,array,buff_in_rd,
	4    mode,
	5    raw_mode,i_dop_mode,nband,nlines,line_offset,
	6    ns,nr_header,ns_header,i_squint,i_pt_sim,i_data_type,nlook,
	7    nfr,nfx,icomplex,i_preproc,i_iqqi,nx_offset,i_dummy,
	8    lambda,fc,bw,dmu,fs,plen,k_chirp,dr,
	9    prf,prf_in,pbw,az_ang,az_res,df,
	1    t_1st_echo,t_eph,t_gha,gha,xs,ys,zs,rsc,vxs,vys,vzs,vs,
	2    re_mean,h,
	3    rs_ref,r0_base,fd0_base,fr0_base,fd0,fr0,fdd,frd,
	4    fd_const,r_ref,r_1st,
	5    pxl_spacing,nx_spacing,
	6    t_start_frm,t_end_frm,
	7    lat1,lon1,rs_crn1,rg_crn1,lookang1,
	8    lat2,lon2,rs_crn2,rg_crn2,lookang2,
	9    lat3,lon3,rs_crn3,rg_crn3,lookang3,
	1    lat4,lon4,rs_crn4,rg_crn4,lookang4,
	2    lat5,lon5,rs_crn5,rg_crn5,lookang5,
	3    raw_file,image_file)
	
	vsg = vs * re_mean*cos(rg_center/re_mean)/rsc
	nx_spacing = vsg/prf	!natural spacing in azimuth 
	
	xg_ref = 0.
	xg_start = int(((ix_st-1)*nx_spacing)/(4*pxl_unit))*(4*pxl_unit)
	ia_loc = nint((xg_start-xg_ref)/pxl_unit)+1
        xg_end = int(((ix_st-1+az_in+nadd-8)*nx_spacing)/pxl_unit)
	1    *pxl_unit
	
	na_smp = nint((xg_end-xg_start)/pxl_unit)
	
	if(na_smp .gt. 8192) then
	   print *,'az_resample: na_smp > 8192, increase imag_line size'
	   call mpi_abort(mpi_comm_world,22,ierror)
           call exit(22)
        end if
	
	do j = 1, nint((xg_end-xg_start)/pxl_unit) !azimuth resampling loop
	   xg = (j-1)*pxl_unit + xg_start
	   ax = (xg - (ix_st-1)*nx_spacing)/nx_spacing
	   index = int((ax-int(ax))*128) + 1
	   k1 = int(ax)
	   phase = -2.0*pi*fdline/prf
	   resam_buff2(j) = cmplx(0.,0.)
	   
	   if(k1-1.lt.1.or.k1+2.gt.8192) then
	      resam_buff2(j) = cmplx(0.,0.)
	   else
	      if(abs(resam_buff(k1-1)*resam_buff(k1)*
	2	   resam_buff(k1+1)*resam_buff(k1+2)).eq.0) then
		 resam_buff2(j) = cmplx(0.,0.)
	      else
		 resam_buff2(j) = 
	1	      w(31,index)*resam_buff(k1-1)*
	2	      cmplx(cos(phase*(k1-1-ax)),sin(phase*(k1-1-ax))) +
	3	      w(32,index)*resam_buff(k1)*
	4	      cmplx(cos(phase*(k1-ax)),sin(phase*(k1-ax))) +
	5	      w(33,index)*resam_buff(k1+1)*
	6	      cmplx(cos(phase*(k1+1-ax)),sin(phase*(k1+1-ax))) +
	7	      w(34,index)*resam_buff(k1+2)*
	8	      cmplx(cos(phase*(k1+2-ax)),sin(phase*(k1+2-ax)))
		 
	      endif
	   endif
	   
c	do k = 33-npt/2,32+npt/2
c        
c           if(k1-32+k.lt.1.or.k1-32+k.gt.8192)then
c              resam_buff2(j) = cmplx(0.,0.)
c              goto 1111
c           else if (abs(resam_buff(k1-32+k)).eq.0) then
c	       resam_buff2(j) = cmplx(0.,0.)
c               goto 1111
c           else
c	       resam_buff2(j)=resam_buff2(j) 
c     .                        + w(k,index)*resam_buff(k1-32+k)
c           end if
c	end do
