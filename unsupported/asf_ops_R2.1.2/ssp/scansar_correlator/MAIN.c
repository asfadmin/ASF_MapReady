...
static char sccsid_MAIN[]= "@(#)SSP2MAIN.c:2.41";     
...
static
int SendAckToServer (char* ack_type, int errno, char* text, ODL msg, int itype)
{
    ODL ack;
...
static
void  ssp_scan (ODL msg, void *arg)
{
...
static
void ssp_heartbeat(ODL msg, void *arg) 
{
...
report_percent=calculate_percent(time_mark,istage,ipercent,NUM_NODE);
 if (istage==0)
   sprintf(wayne_state_str,"%s","SSP2 IDLING");
 if (istage==1)
   sprintf(wayne_state_str,"DATA TRANSFER: %3.1f %% completion",ipercent*100.0);
 if (istage==2)
   sprintf(wayne_state_str,"PROCESSING: %3.1f %% completion",report_percent);
 if (istage==3)
   sprintf(wayne_state_str,"PROCESSING: %3.1f %% completion",report_percent);
 if (istage==4)
   sprintf(wayne_state_str,"PROCESSING: %3.1f %% completion",report_percent);
...
static
void ssp_cleanup(ODL msg, void *arg)
{
 char save_flag[60];
 char save_dir[60];
 int ierr;
...
static
void ssp_reset(ODL msg, void *arg)
{
 int icode;
 char cmd[500];
 char id_char[10];
...
int main(int argc, char *argv[])
{
    ...
    if (FILE_ENG_CONFIG == NULL) {
       dump_error_and_exit("NOT VALID SSP2 CONFIGURATION FILE",1);
    }


    printf("Hello, this is ssp2 \n");
    fflush(stdout);
...
/*  strcpy(instr_mode , ODLGetString(odl, INSTRUMENT_MODE, &ierr));*/
  ...
  if (strcmp(instr_mode,"SWB")==0) *i_instr= 4;
  if (strcmp(instr_mode,"SNA")==0) *i_instr= 2;
  if (strcmp(instr_mode,"SNB")==0) *i_instr= 3;
