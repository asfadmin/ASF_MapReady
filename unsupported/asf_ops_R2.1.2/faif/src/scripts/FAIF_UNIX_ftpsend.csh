#!/bin/csh -f
#===============================================================================
# Filename:	FAIF_UNIX_ftpsend.csh
# Description:
...
###############################################################
if ($#argv < 3) then
   echo "Usage: FAIF_UNIX_ftpsend.csh RemoteHost RemoteDir FileToSend Mode FAIFRootpath Username Password"
   exit 1
endif
...
set sentfile_remote = $sentfile

# CSA rrp files are a special case. Check for them, and if the current
# file is one, strip the trailing index value off its name while ftp-ing.

set special_case = `echo $sentfile | cut -c 1-1,9-12`
if ($special_case == "f.rrp") set sentfile_remote = `echo $sentfile | cut -c 1-12`
...
$mode
cd $destdir
put $sentpath $sentfile_remote
lcd $copydir 
get $sentfile_remote $sentfile"_copy"
bye
.
