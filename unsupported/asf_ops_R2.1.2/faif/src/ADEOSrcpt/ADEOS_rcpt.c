...
/*==============================================================================
Filename:       ADEOS_rcpt.c

Description:    Contains the functions to parse the ADEOS receipt mail message.
...
Notes:	borrowed from ADEOS_msg.c (Phil Yurchuk)
...
Description:	Usage banner	
Parameters:	name of program
Returns:	None	
Creator:	Cameron Cooper
Creation Date:	Thu Sep  7 14:49:49 PDT 1995
Notes:		
==============================================================================*/
static 
...
Description:    Pulls out the subject of the ADEOS mail message, and the 
		...
		Then creates a linked list, and passes it to check_ADEOS_msg. 
...
/* after <BOM> has been found, start reading in the messages */
  ...
  if (message[0])
    head = node1 = parse_ADEOS_msg(message) ;    /* send messages to parse_ADEOS_msg */
  ...
  check_ADEOS_msg(head, subject, configfile) ; 
  
   syslog(LOG_NOTICE, "Completed.");
