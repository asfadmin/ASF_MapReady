char *sccs = "@(#)ims_mapMessageUpdate.c	1.1 07/17/97";
/******************************************************************************
**
** File:        ims_mapMessageUpdate.c
**
** Function:    Checks downlink map and update downlink messages to map
**              to downlink side.
**
** Author:      Sean Hardman
**
...
int *, int *, char *, char *);
static int checkDownlink(IMS_MSG_STRUCT *, char *, char *, int, int, char *,
int *, char *, int, int *);
	...
	char *argv[])
{
	...
	*/
	...
	{
		(void) ims_msg (msgDesc, IMS_INFO,
			"Updating rows has been disabled.");
	}
	...
	for (granule_idx = startIdx; granule_idx < endIdx+1; granule_idx++)
	{
		...
		else
		{
			...
			{
				(void) ims_msg (msgDesc, IMS_INFO,
					"Skipping this file because the sensor value is 'Z'.");
			}
			else
			{
				if (checkDownlink(msgDesc,
					platform, sensor, rev, seq, new_sensor, &new_sequence, granules_table,
					granule_idx, &count) < IMS_OK)
				{
					(void) ims_msg(msgDesc, IMS_ERROR,
				...
				if (count > 0)
				{
					(void) sprintf(log_msg,
						"%s %s %s %d %d %s %s %s %d\n", platform, sensor,
						mode, rev, seq, activity_id, new_sensor, mode, new_sequence);
	...
	(void) ims_msg (msgDesc, IMS_INFO,
		"Ending the ims_mapMessageUpdate at index '%d' "
		"in granules table '%s' for dataset '%s'.",
	...
	int *count)
{
	...
	*/
	...
	/*
	** Query the datatake_entry table and look for the record.
	*/
	(void) sprintf (cmdBuf,
		"select SENSOR, SEQUENCE from datatake_entry "
		"where DT_PLATFORM = '%s' "
		"and DT_SENSOR = '%s' "
		"and DT_REVOLUTION = %d "
		"and DT_SEQUENCE = %d",
	...
	{
		(void) ims_msg (msgDesc, IMS_WARNING,
			"Could not find corresponding entry in the datatake_entry table: %s",
			 cmdBuf);
		return (IMS_OK);
	}

	/* SENSOR */
	(void) memcpy((char *) new_sensor, qDesc->valAddr[0],
		qDesc->valLength[0]);
	new_sensor[qDesc->valLength[0]] = '\0';

	/* SEQUENCE */
	(void) memcpy((char *) &short_seq, qDesc->valAddr[1],
		qDesc->valLength[1]);
	*new_sequence = (int) short_seq;
	...
	*/
	...
	/*
	** Update granule table entry.
	*/
	(void) sprintf(cmdBuf,
		"update %s set SENSOR = '%s', SEQUENCE = %d "
		"where granule_idx = %d",
		granules_table, new_sensor, *new_sequence, granule_idx);
	...
	if (updateFlag == IMS_TRUE)
	{
		while ((status = ims_qiNextRow(qDesc)) != IMS_ENDOFTRANSACTION)
		{
			...
			{
				(void) ims_msg(msgDesc, IMS_ERROR,
					   "Could not update table %s for granule_idx %d.",
		...
		*count = IMS_AFFECTED (qDesc);
		...
		(void) ims_msg (msgDesc, IMS_INFO,
			"Updated %d rows in table %s: %s",
			IMS_AFFECTED (qDesc), granules_table, cmdBuf);
	}
	else
	{
		(void) ims_msg (msgDesc, IMS_INFO,
			"Would have updated granule information accordingly: %s",
			cmdBuf);
	}
