static char *sccs = "@(#)ims_sequenceFix.c	1.1 07/17/97";
/******************************************************************************
**
** File:        ims_sequenceFix
**
...
static int updateMapEntry(IMS_MSG_STRUCT *, char *, char *, int, int, int, int, char *, char *, char *);
	...
	char *argv[])
{
	...
	*/
	...
	while (!feof(inpDesc))
	{
		...
		if (strcmp (activity_id, "DMP") == 0)
		{
			status = updateMapEntry(msgDesc, 
		...
		{
			(void) ims_msg (msgDesc, IMS_INFO,
				"Skipping this record because the activity_id is not 'DMP'."); 
		}
	...
	*/
	...
	(void) ims_msg (msgDesc, IMS_INFO,
		"ims_sequenceFix processing completed successfully.");
	...
	char *dataset)
{
	...
	*/
	...
	(void) sprintf (cmdBuf,
		"select DT_SEQUENCE from datatake_entry "
	...
	{
		(void) ims_msg(msgDesc, IMS_ERROR,
			"Could not find sequence information for "
			"platform = '%s' sensor = '%s' rev = '%d' seq = '%d'",
			platform, sensor, rev, seq);
		return(IMS_ERROR);
	}
