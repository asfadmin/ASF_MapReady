...
/*****************************************************************************
  ...
  Routine: ODLWriteStmtBuf		<Nadia.Adhami@Jpl.Nasa.Gov>
 
  Description: Writes an ODL label statement to the specified buffer.
 
  Input:
          buffer - Pointer to buffer to which statement is to be written.
          statement - Character string with statement to be written.
          buffer_size - size of buffer
 
  Output: The statement is printed to the output buffer.

*****************************************************************************/


void ODLWriteStmtBuf (buffer,statement,buffer_size)
     char buffer[];
     char statement[];
     int  buffer_size;
{

  if ((strlen(buffer) + strlen(statement)) >= buffer_size)
  {
    buffer[0] = '\0';
#ifdef DEBUG
    fprintf(stderr, "ODLWriteStmtBuf(): buffer not big enough\n");
#endif
    return;
  }
