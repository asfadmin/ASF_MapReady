/*****************************************************************************
  ...
  WriteLabelBuf:  Writes a PDS label in the Object Description Language
               to the specified buffer.

  Input:
         output_buffer  - pointer to a buffer.
         ...
         buffer_size  - size of output_buffer

  Output:  There are no output parameters and no function value is returned.
           The label is written to the file using output routine ODLWriteStmtBuf.
  ...
  9/28/95  - Nadia Adhami nadia.adhami@jpl.nasa.gov
    Modified a version of WriteLabel so that it writes to a buffer
...
void WriteLabelBuf (output_buffer, base_node, buffer_size)
       char       output_buffer[];
       AGGREGATE  base_node;
       int	  buffer_size;
{
         ...
         while (node->parent != old_node)
           {
             ...
             ODLWriteStmtBuf (output_buffer, stmt, buffer_size);
...
#ifdef PDS_TOOLBOX
   if (pds_finish_label)
      ODLWriteStmtBuf (output_buffer, "END\n", buffer_size);
#else
   ODLWriteStmtBuf (output_buffer, "END\n", buffer_size);
#endif
