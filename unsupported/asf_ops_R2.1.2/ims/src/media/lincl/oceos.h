...
/* SAR Leader File - Facitlity Related Data Record */
...
/* SAR Leader File - Facitlity Related Data Record ICE MOTION */

#define LF1ASUB       10
#define LIMTYPE       201

/* SAR Leader File - Facitlity Related Data Record ICE CLASS */

#define LICTYPE       202


/* SAR Leader File - Facitlity Related Data Record WAVE MOTION */
...
typedef struct descp_rec  *pnt_desc_rec_t;
...
typedef struct  vol_desc_rec {
    ...
    char  phyvol_id[17];        /* physical tape ID */
    ...
    long  first_file;           /* first reference file in volume */
    long  volset_log;           /* logical volume in set */
    long  phyvol_log;           /* logical volume in physical volume */
    ...
    long  n_filepoint;          /* number of pointer records */
    long  n_voldir;             /* number of records */
    long  n_tot_logvol;         /* number of tot, logical vols */
    char  spare2[89];           /* spare */
    char  spare3[101];          /* spare */
...
typedef struct file_ptr_rec {
    ...
    char  spare2[101];          /* spare */
    char  orderline[3];
    char  mediaid[17];          /* media id  */
    char  proccode[4];          /* product code  */
    char  spare3[80];           /* spare */
...
typedef struct sarl_desc_rec {
    ...
    short file_num;             /* file number */
    char  file_name[17];        /* filename */
    char  rec_seq[5];           /* record sequence and location type flag */
    ...
    char  spare4[2];            /* spare */
    char  spare5[2];            /* spare */
    char  spare6[65];           /* OPS Reader s/w version */
    ...
    long  spare7[11];           /* spare */
    long  n_faci_data;          /* number of facility data records */
    long  l_faci_data;          /* record length */
    char  spare8[289];          /* spare */
...
typedef struct {
    char  sensid[7];            /* SIR-C */
    char  hyphen1[2];           /* hyphen */
    char  freq[3];              /* SAR band: L, C, or X */
    char  hyphen2[2];           /* hyphen */
    char  res[3];               /* resolution mode: HI, LO, SUrvey */
    char  acq[3];               /* radar data acquisiton mode */
    char  hyphen3[2];           /* hyphen */
    char  tx[3];                /* transmit polarization */
    char  rx[3];                /* receive polarization */
    char  spare1[15];           /* pad structure to length 32 */
} sensor_id_t;

typedef struct dataset_sum_rec {
    desc_rec desc;              /* header info */
    short seq_num;              /* Data Set Summary: record sequence number */
    short sar_chn;              /* SAR channel indicator */
    char  scene_id[17];         /* site ID (3-char ID) */
    ...
    char  spare5[8];            /* spare */
    float fr;                   /* range complex sampling rate */
    ...
    float fa;                   /* nominal PRF */
    float elev_beam;            /* antenna elevation 3dB beam width */
    float azim_beam;            /* antenna azimuth 3dB beam width */
    ...
    char  prod_type[33];        /* product type */
    ...
    char  clutter_lock[5];      /* clutter lock flag */
    char  auto_focus[5];        /* autofocussing flag */
    float line_spacing;         /* line spacing (m) */
    float pix_spacing;          /* pixel spacing (m) */
...
typedef struct map_proj_rec {
    ...
    char  proj_desc[33];        /* Map projection descriptor */
...
typedef struct pos_data_rec {
    ...
    Pos_Vect_Rec pos_vect[64];  /* Data point position/velocity */
    char  spare1[126];          /* spare for word boundary padding */
...
typedef struct att_vect_rec {
    ...
    long  gmt_sec;              /* millisecond of the day (GMT) */
...
typedef struct rad_comp_set {
    ...
    short req_records;          /* number of required records */
...
typedef struct radi_comp_rec {
    ...
    short    chan_ind;          /* sar channel indicator */
    long     n_dset;            /* number of data sets */
    long     dset_size;         /* data set size */
    char     spare1[5];         /* padding for link list */
...
typedef struct        /* radiometric uncertainty record */
{
    float db;                    /* units of db */
    float deg;                   /* units of deg */
} rad_unc_rec;

typedef struct        /* misregistration record */
{
    float alt_m;                 /* nominal along track misregistration */
    float crt_m;                 /* nominal cross track misregistration */
}  misreg_rec;

typedef struct qual_sum_rec {
    desc_rec desc;              /* header info */
    short seq_num;              /* dqs record sequence number */
    char  sar_chn[5];           /* sar channel indicator */
    ...
    float rad_unc_db;           /* nominal radiometric uncertainty, db */
    float rad_unc_deg;          /* nominal radiometric uncertainty, deg */
    rad_unc_rec  rad_unc[16];   /* rel short term radio calibr uncertainty */
    ...
    misreg_rec  misreg[16];     /* misregistration error */
...
typedef struct histogram_dset {
    ...
    long  *data_values;        /* table values 1-256 */
...
typedef struct rng_spec_rec {
    ...
    float data_values[256];     /* spectral data values 1-256 */
...
typedef struct corner_pts_rec {
    float latitude;              /* polygon corner pt. latitude */
    float longitude;             /* polygon corner pt. longitude */
...
typedef struct dig_elev_rec {
    ...
    Dem_Desc*  set;             /* pointer to list of DEM data descriptor data sets */
...
typedef struct fac_related_rec {
    ...
    float  near_sta_LAT;         /* near start Latitude */
    float  near_sta_LON;         /* near start Longitude */
    float  near_end_LAT;         /* near end Latitude */
    float  near_end_LON;         /* near end Longitude */
    float  far_sta_LAT;          /* far start Latitude */
    float  far_sta_LON;          /* far start Longitude */
    ...
    long   start_block;          /* start block */
    long   end_block;            /* end block */
    ...
    char   ASC_DESC[2];          /* Ascending-Descending flag */
    float  S_C_altitude;         /* Spacecraft altitude */
    double X_pos;                /* spacecraft x position */
    double Y_pos;                /* spacecraft y position */
    double Z_pos;                /* spacecraft z position */
    double X_vel;                /* spacecraft x velocity */
    double Y_vel;                /* spacecraft y velocity */
    double Z_vel;                /* spacecraft z velocity */
    ...
    short  type_ephemeris;       /* type of ephemeris */
    float  looks_azimuth;        /* effective looks azimuth */
    float  looks_range;          /* effective looks range */
    float  WPH_azimuth;          /* weighing pedestal height azimuth */
    float  WPH_range;            /* weighing pedestal height range */
    ...
    float  avg_ter_height;       /* average terrain height */
    short  proc_gain;            /* processor gain */
    char   deskew_flag[5];       /* deskew applied flag */
    ...
    float  resol_azimuth;        /* resol azimuth */
    float  resol_range;          /* resol range */
...
typedef struct  img_desc_rec_t {
    ...
    short rlen_len;             /* record length field length */
    char  spare2[5];            /* spare */
    char  spare3[65];           /* OPS Reader s/w version */
    long  n_dataset;            /* number of data set summary records */
    long  l_dataset;            /* record length */
    char  spare4[25];           /* spare */
    ...
    short nchn;                 /* number of SAR channels */
    long  nlins;                /* number of lines per data set */
    ...
    short nbott;                /* number of bottom border scan lines */
    ...
    short nprefix;              /* length of prefix data per line */
    ...
    long  pix_rng;              /* maximum data range of pixel */
    char  spare7[193];          /* reserved */
...
typedef struct sart_desc_rec {
    ...
    char  ascii_flag[3];        /* ASCII flag */
    char  spare1[3];            /* spare */
    char  doc_format[13];       /* document format */
    char  format_rev[3];        /* format revision ("A") */
    char  rec_format_rev[3];    /* record format revision level */
    char  software_id[13];      /* software ID */
    short file_no;              /* file number */
    char  filename[17];         /* filename */
    char  seq_flag[5];          /* record sequence and location type flag */
    long  seq_location;         /* sequence number location */
    short seq_field_len;        /* sequence number field length */
    char  code_flag[5];         /* record code and location type flag */
    long  code_location;        /* record code location */
    short code_field_len;       /* record code field length */
    char  len_flag[5];          /* record length and location type flag */
    long  len_location;         /* record length location */
    short len_field_len;        /* record length field length */
    char  spare2[2];            /* spare */
    char  spare3[2];            /* spare */
    char  spare4[2];            /* spare */
    char  spare5[65];           /* spare */
    long  n_data_set_recs;      /* number of data set summary records */
    long  len_data_set;         /* record length */
    long  n_map_proj_recs;      /* number of map projection data records */
    long  len_map_proj;         /* record length */
    long  n_platf_recs;         /* number of platform position records */
    long  len_platf;            /* record length */
    long  n_att_recs;           /* number of attitude data records */
    long  len_att;              /* record length */
    long  n_radio_recs;         /* number of radiometric data records */
    long  len_radio;            /* record length */
    long  n_radio_comp_recs;    /* number of radio compensation records */
    long  len_radio_comp;       /* record length */
    long  n_qual_recs;          /* number of data quality summary records */
    long  len_qual;             /* record length */
    long  n_hist_recs;          /* number of data histogram records */
    long  len_hist;             /* record length */
    long  n_spectra_recs;       /* number of range spectra records */
    long  len_spectra;          /* record length */
    long  n_elev_recs;          /* number of digital elevation model recs */
    long  len_elev;             /* record length */
    long  n_update_recs;        /* number of radar parameter update recs */
    long  len_update;           /* record length */
    long  n_annot_recs;         /* number of annotation data records */
    long  len_annot;            /* record length */
    long  n_proc_recs;          /* number of detailed processing records */
    long  len_proc;             /* record length */
    long  n_calib_recs;         /* number of calibration data records */
    long  len_calib;            /* record length */
    long  n_ground;             /* number of ground control pts records */
    long  len_ground;           /* record length */
    long  spare6[10];           /* spare */
    long  n_facil_recs;         /* number of facility data records */
    long  len_facil;            /* record length */
    char  spare16[289];         /* spare */
...
/* ************************************
    changed by David Pass  :  5/24/95 for vdf processing: for
    reader program   */
/* *********************
    This is the structures saved in the vdf file for each granule:
    as far as I know, only the File_Ptr_Rec and the Tex_Rec.  There
    may be more.  Note:  granules are the data files extrancted from
    the Alaska Sar Facility raw data as requested by users.
    The File_Ptr_Rec can be the leader file, the trailer file and/or
    the imagery options file (data file) descriptions.
*********************** */
typedef struct file_ptr_rec       *pnt_file_ptr_rec_t;
typedef struct text_rec           *pnt_text_rec_t;
typedef struct granule_data__t    *pnt_granule_data_t;
typedef struct granule_data__t {
    pnt_file_ptr_rec_t      pnt_file_rec;  /*  file ptr for leader
        file  */
    pnt_file_ptr_rec_t      pnt_tfile_rec; /*  file ptr for trailer
        file  */
    pnt_file_ptr_rec_t      pnt_ifile_rec; /*  file ptr for imagery
        options file */
    pnt_text_rec_t          pnt_text_rec; /* text record for data */
    pnt_granule_data_t      pnt_next; /*  pntr to next granule info */
} granule_data_t;

/* ************************
    This is the structure that is returned to the user: there are
    num_granules set of data for the tape info.
****************************** */
typedef struct recvdr  *pnt_recvdr_t;
typedef struct vol_desc_rec  *pnt_vol_desc_rec_t;
typedef struct vdf_file__t   *pnt_vdf_file_t;
typedef struct vdf_file__t {
    pnt_vol_desc_rec_t      pnt_descript;
    pnt_recvdr_t            pnt_vdr;
    pnt_granule_data_t      pnt_gdata; /* ptr to granule data files */
} vdf_file_t;
