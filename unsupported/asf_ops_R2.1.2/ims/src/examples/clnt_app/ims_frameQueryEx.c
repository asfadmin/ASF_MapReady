static char *sccs = "@(#)ims_frameQueryEx.c	5.3  01/07/97";
/* *************************************************************
**
** File:        ims_frameQueryEx.c
**
** Function:    An client program that uses the ims_frameQuery
**              routine.
**
** Author:      David Pass
**
...
static IMS_CMD_CLASS cmdLineElm [] =
{
	...
	{"-M",              &commands.mode},
	{"+mode",           &commands.mode},
        {"-F",              &commands.frameMode},
        {"+frameMode",      &commands.frameMode},
	...
	char *argv[])
{
	...
	int rev;
	int sequence;
	IMS_FRAME_STRUCT * pnt_retbuf;
	IMS_FRAME_STRUCT * pnt_retbuf2;
	...
	*/
	...
	if ((status = ims_frameQuery( query, commands.platform,
		rev, sequence, commands.mediaId, commands.mode, commands.frameMode )  ) < IMS_WARNING)
	{
		goto ERROR;
	}

	pnt_retbuf = &retbuf;
	while( pnt_retbuf  !=  (IMS_FRAME_STRUCT *) NULL  )
	{
		(void) printf ("\nQuery Results:\n");
		(void) printf ("  frame_id = %ld\n", pnt_retbuf->frame_id );
		(void) printf ("  station_id = %s\n", pnt_retbuf->station_id );
		(void) printf ("  start_time = %s\n", pnt_retbuf->start_time );
		(void) printf ("  end_time = %s\n", pnt_retbuf->end_time );
		(void) printf ("  center_time = %s\n", pnt_retbuf->center_time);
		(void) printf ("  data_direction = %s\n", 
				pnt_retbuf->data_direction);
		pnt_retbuf = pnt_retbuf->next;
	}

	ims_frameFree ((IMS_FRAME_STRUCT *) query->retPtr);
...
ERROR:
	...
	*/
	(void) ims_msg (msgDesc, IMS_ERROR,
		"The ims_frameQuery select failed.");
	...
	IMS_CMN_QUERY *query, int * rev, int * sequence )
{
   ...
   "Numerical input is not valid. Try again.\n" );
	...
	if (commands.mode ==  (char *) NULL)
	{
        ...
        /* frame mode */
        if (commands.frameMode ==  (char *) NULL)
        {
                if (ims_getString (IMS_TRUE, inputBuffer, IMS_INPUT_BUF_LEN,
                        "Frame Mode: ") == (char *) NULL)
                {
                        (void) ims_msg (msgDesc, IMS_FATAL,
                                "Error detected while reading input string.");
                        return (IMS_FATAL);
                ...
                i = strlen( inputBuffer )+1;
                commands.frameMode = (char *) malloc( i * sizeof( char ));
                (void) strcpy (commands.frameMode , inputBuffer);
        }
