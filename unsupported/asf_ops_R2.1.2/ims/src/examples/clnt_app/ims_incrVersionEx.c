static char *sccs = "@(#)ims_incrVersionEx.c	5.1  03/17/96";
/* *************************************************************
**
** File:        ims_incrVersionEx.c
**
** Function:    An example program for the ims_incrVersion routine.
**
** Author:      Alin Tilden
**
** Date:        01/11/96
...
** main ()
**
** This is the driver for the ims_incrVersion routine.
**
**************************************************************** */
	   ...
	   char *argv[])
{
    ...
    */
    if ((status = ims_incrVersion (query, name)) < IMS_OK)
    {
    ...
    (void) ims_msg (msgDesc, IMS_INFO,
        "The ims_incVersion call was successful.");
...
ERROR:
    ...
    (void) ims_msg (msgDesc, status, "Could not increment version.");
    (void) ims_msgStructFree (msgDesc);
			...
			IMS_CMN_QUERY  *query)
{
	...
	name = malloc ((strlen (inputBuffer) + 1) * sizeof (char));
	(void) strcpy (name, inputBuffer);
    }
