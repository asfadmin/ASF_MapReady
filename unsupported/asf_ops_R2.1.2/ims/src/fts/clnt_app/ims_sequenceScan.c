static char *sccs = "@(#)ims_sequenceScan.c	1.1  03/07/97";
/******************************************************************************
**
** File:        ims_sequenceScan.c
**
** Function:    Issue a scan job for the given data sequence message
**              by executing the ims_downlinkAux application.
**
** Author:      Sean Hardman
**
** Date:        3/3/97
**
** Note:        Based on the original ims_startScan written by Dan Crichton.
**
**
	...
	char *argv[])
{
	...
	*/
	...
	/*
	** Set-up global dataset value.
	*/
	(void) sprintf (glbl_dataset, "%s RAW SIGNAL SEGMENT", userSpec.platform);
	...
	*/
	...
	(void) ims_msg (msgDesc, IMS_INFO,
		"Starting scan job submission for granule '%s'.", userSpec.filename);

	/*
	** Run the auxiliary program to submit the scan job.
	*/
	if ((status = runAuxProgram (msgDesc, "ims_downlinkAux")) < IMS_OK)
	{
		(void) ims_msg (msgDesc, status,
			"Scan job submission failed.");
		(void) ims_msgStructFree (msgDesc);
		exit (1);
	}
	...
	(void) ims_msg (msgDesc, IMS_INFO,
		...
		"given granule did not meet the criteria for issuing a scan job.");
	...
	IMS_MSG_STRUCT *msgDesc)
{
	...
	*/
	...
	else
	{
		...
		(void) strcpy (userSpec.platform, inputBuffer);
	}
