static char sccsid_rawchk_c[] =
    "@(#)rawchk.c	1.2 96/04/09 19:13:31";
...
/*  float erf_334(x)   --------------------------------
(used to be just erf, but it conflicted with an intrinsic erf function
of DEC ULTRIX)
...
float x;
{ float erf_334();
   return(0.5*(1.0 + erf_334(x/sqrt(2.0))));
}
