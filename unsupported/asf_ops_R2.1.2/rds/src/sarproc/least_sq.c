static char sccsid_least_sq_c[] =
    "@(#)least_sq.c	1.2 96/04/09 19:13:28";

/*  least_sq(f_vec,n,num_smpl,f_coff)  ----------------
...
determnt=(n*sum_x2*sum_x4)+(sum_x*sum_x3*sum_x2)+
	 (sum_x2*sum_x3*sum_x)-(sum_x2*sum_x2*sum_x2)-
	 (sum_x3*sum_x3*n)-(sum_x4*sum_x*sum_x);
...
matrix    =(n*sum_xy*sum_x4)+(sum_y*sum_x3*sum_x2)+
	   (sum_x2*sum_x2y*sum_x)-(sum_x2*sum_xy*sum_x2)-
	   (sum_x2y*sum_x3*n)-(sum_x4*sum_y*sum_x);

f_coff[1] = matrix/determnt;

matrix     =(n*sum_x2*sum_x2y)+(sum_x*sum_xy*sum_x2)+
	    (sum_y*sum_x3*sum_x)-(sum_x2*sum_x2*sum_y)-
	    (sum_x3*sum_xy*n)-(sum_x2y*sum_x*sum_x);
