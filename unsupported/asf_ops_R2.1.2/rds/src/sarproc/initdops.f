	subroutine initdops (leftlook,sv,roll,yaw,pitch,xlambda,re,f,
     .range,gamma,fd,fdot,v_swath,v_orbital,v_target,v_relative,istat)
...
c	for a SAR.
c
c	INPUT:		leftlook  TRUE = left looking SAR, FALSE = right
c			sv	6 element, double precision, statevector.
...
c*/
	...
	data choice /'USEA'/
      character*80 sccsid
      data sccsid /'@(#)initdops.f	1.5 96/07/29 12:06:48\0'/
	if (istat.eq.1) then 
		verbose=.true.
	else
		verbose=.false.
	end if
	if (leftlook .eq. 0) then
	    side='r'       ! looking toward right of flight path
	else
	    side='l'       ! looking toward left of flight path
	end if
	xsc(1)=sv(1)
	xsc(2)=sv(2)
	xsc(3)=sv(3)
...
c
	...
	if (g1.ge.pi/2.) then ! error in getlook
		istat=-1
	...
	if (gamma_d.ge.pi/2.) then ! error in getlook
		istat=-2
