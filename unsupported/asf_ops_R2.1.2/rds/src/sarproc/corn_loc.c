...
static char sccsid_corn_loc_c[] =
	"@(#)corn_loc.c	1.4 96/07/25 17:45:51";
...
extern add_seconds(struct gmt*, double);

/* corn_loc(sv_ref,t_ref,t_start,delta,nl,r3points, --------------------
        fd3points, fr3points,re,rp, lambda,zero_dop,fd_adj,leftlook,
	a,b,c,d,e,
	a_propvec, b_propvec, c_propvec, d_propvec, e_propvec,istat)
...
variable:	nl
type:		integer
description:	total number of range lines (not counting 't_start' line)
...
variable:	leftlook
type:		integer
description:	FALSE => right looking SAR
...
corn_loc(leftlook, sv_ref, t_refp, t_startp, r3points, fd3points, fr3points,
	nl, zero_dop, re, rp, lambda, fd_adj, delta,
	a, b, c, d, e, a_propvec, b_propvec, c_propvec, d_propvec, e_propvec,
	istat)

struct  gmt *t_refp,*t_startp;

double	*sv_ref,re,rp,lambda, *r3points,fd_adj,
	*a,*b,*c,*d,*e,
	*a_propvec, *b_propvec, *c_propvec, *d_propvec, *e_propvec;

double	delta;
float   *fd3points, *fr3points;

int	leftlook, nl,zero_dop,*istat;
...
struct gmt t_ab,t_cc,t_de,t_a,t_b,t_c,t_d,t_e,t_out, t_ref, t_start;
...
getlocc_(&leftlook,a_propvec,&r_near,&fd_a,&re,&rp,&lambda,&years,&days,&hrs,
...
fd_b += fd_adj * fdot_b;
getlocc_(&leftlook,b_propvec,&r_far,&fd_b,&re,&rp,&lambda,&years,&days,&hrs,
...
getlocc_(&leftlook,c_propvec,&r_mid,&fd_c,&re,&rp,&lambda,&years,&days,&hrs,
...
getlocc_(&leftlook,d_propvec,&r_near,&fd_d,&re,&rp,&lambda,&years,&days,&hrs,
...
printf("fd_a=%lf\nfd_b=%lf\nfd_c=%lf\nfd_d=%lf\nfd_e=%lf\n",fd_a,fd_b,
	fd_c,fd_d,fd_e);
*/
getlocc_(&leftlook,e_propvec,&r_far,&fd_e,&re,&rp,&lambda,&years,&days,&hrs,
