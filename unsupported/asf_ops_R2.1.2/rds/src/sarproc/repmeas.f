...
c*/
#include "libmp.h"
        character*1     ans,replica(1)      !pointer to replica  2k in length
	logical verbose
...
c          the bigfft.f routine required.
	real*4		coeftable(8192+1)
        ...
        data pi /3.14159/       
      character*80 sccsid
      data sccsid /'@(#)repmeas.f	1.4 96/04/09 22:51:56\0'/
c
...
c
	if(verbose)write(6,*)'converting replica to complex'
        do i=1,npts
        rep(i+1024)=cmplx(float(ichar(replica( (i-1)*2 +1)) -32),
     . float(ichar(replica(i*2))-32)  )
        end do  
c
c   calculate:   Gain
c                bias
...
c
	...
	if (verbose) then ! ask about plot
	write(6,*)'enter y for plot of replica spectrum'
	read(5,101)ans
	if (ans.eq.'y') then
	call mpinit (gls)
ccall mpdevice(gls,"tek",2,0)
        call mpdevice(gls,"mcd -xl600 -xh1150 -yl10 -yh500",2,0)
	call mplotsrcy(gls,1,2048,0,plot,"F",1,1,NULL,NULL)
...
c
	...
	if (verbose) then
	write(6,*)'enter y for plot of correlation peak'
	read(5,101)ans
	if (ans.eq.'y') then
	write(6,*)'ploting interpolated response peak'
	call mpinit (gls)
ccall mpdevice(gls,"tek",2,0)
        call mpdevice(gls,"mcd -xl600 -xh1150 -yl10 -yh500",2,0)
	call mplotsrcy(gls,1,2048,0,plot,"F",1,1,NULL,NULL)
	call mpaxvals(gls,2,-50.,0.,10.,0)
	call mptitle(gls,2,-1,-1,'dB Power ')
	call mptitle(gls,4,-1,-1,"RETURN TO CONTINUE...")
 	call mplot(gls,0,0,0)
	call mpend(gls)
	read(5,101)ans
	write(6,*)'ploting close-up of peak:'
	call mpinit(gls)
ccall mpdevice(gls,"tek",2,0)
        call mpdevice(gls,"mcd -xl600 -xh1150 -yl10 -yh500",2,0)
	call mplotsrcy(gls,1,256,0,plot(896),"F",1,1,NULL,NULL)
	call mpaxvals(gls,2,-50.,0.,10.,0)
	call mptitle(gls,2,-1,-1,'dB Power ')
	call mptitle(gls,4,-1,-1,"RETURN TO CONTINUE...")
	call mplot(gls,0,0,0)
