...
static char sccsid_odl_api_c[] =  "@(#)odl_api.c	1.16 96/10/31 11:25:23";

#ifndef __sgi              
#define mkTime mktime      /* IRIX mktime() doesn't do leap year properly */
#endif
...
#endif
{
    ...
    if (!odl) printfLLog(LOG_ERR, "%s", err);
    return odl;
}
...
#endif
{
	...
	return NULL;
    if (!(new = (ODL) ODLparse(Val(&s), Len(&s), err)))
	printfLLog(LOG_ERR, "%s", err);
    Destroy(&s);
    return new;
}
...
#endif
{
    ...
    *errPtr = 0;
	...
	type == (Method_t) Symbol)
    {
    	if ((p = (char*) malloc(n = (size_t) Len(odl)+1)) == NULL) {
	    *errPtr = -1;
	    return NULL;
	}
