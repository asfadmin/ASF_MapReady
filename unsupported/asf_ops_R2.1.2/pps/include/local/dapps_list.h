...
Revisions:	Nadia Adhami  12/12/95
		DEL_LIST, DEL_ALL, NUMELTS macros :
			added safegaurd against null ptr
==============================================================================*/

#ifndef _DAPPSLLIST_
#define _DAPPSLLIST_

#pragma ident "@(#)dapps_list.h	1.1  11/21/96"
...
void        *(*unlink_by_cursor)( llist *list_p, cursor *u_cursor_p)
...
#define DEL_AT_CURSOR( list_p, cursor ) \
		((list_p)->del_by_cursor)( list_p, &(cursor) )

#define UNLINK_AT_CURSOR( list_p, cursor ) \
		((list_p)->unlink_by_cursor)( list_p, &(cursor) )

#define DEL_ALL( list_p )  \
		if (list_p) ((list_p)->rm_all_entries( (list_p), (cursor *)NULL ))

#define DEL_LIST( list_p )  \
                if (list_p) ((list_p)->destroy_list( (list_p), (cursor *)NULL ))
 
#define NUMELTS( list_p )        (((list_p)?((list_p)->num_elts):0))
