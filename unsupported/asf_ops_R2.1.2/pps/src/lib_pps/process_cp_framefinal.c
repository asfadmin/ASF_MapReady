...
/*==============================================================================
Filename:	process_cp_framefinal.c

Description:
	This module contains the routines used for processing CP Frame Status. 

External Functions:
	process_CP_FrameFinal
...
static char SccsFileId[] = "@(#)process_cp_framefinal.c	1.4    04/23/97";
...
#endif
{
	...
	{
		(void)sprintf(buf, " %s : 'process_CP_FrameFinal' : %s",
...
==============================================================================*/
#ifdef __STDC__
int process_CP_FrameFinal( DBPROCESS *dbproc_server,
	DBPROCESS		*dbproc_commit, 
	IMS_CMN_QUERY		**ims_query,
	CP_JobStatus_Record 	*cp_status
	)
#else
int process_CP_FrameFinal(dbproc_server, dbproc_commit, ims_query, cp_status)
	DBPROCESS		*dbproc_server, 
	DBPROCESS		*dbproc_commit, 
	IMS_CMN_QUERY		**ims_query,
   	CP_JobStatus_Record 	*cp_status
...
#endif
{
	...
	cursor            ptr;
        ...
        (void)sprintf(errormsg, " %s : %s [job_id = %d, status = %s]",
                        pps_err_msgs[ER_DEADLOCK], "process_CP_FrameFinal", 
	...
	/* if failed to update job state because the job is not in
	   SUBMITTED or RETRY state, report the error and return error status 
 	   code to CP */
	if (ret != 1)
	{
