...
/*Fields used during processing*/
	...
	FILE *out;			/* Binary file pointer (write)*/
	...
	int curFormat;			/* The current format count we're looking at */
	int prevFormat;			/* The last format count we looked at */	
...
/*Fields filled out/used by lz/ceos2raw during processing*/
	...
	FILE *fpFrame;			/* Output frame list file pointer (write) */
...
typedef void (*readPulseFunc)(bin_state *s,iqType *iqBuf);
...
iqType *ERS_fillZeroBytes(int nIn,iqType *out);
