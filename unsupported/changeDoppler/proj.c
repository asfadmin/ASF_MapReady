...
double dtr = M_PI/180.0;  /* Degrees to Radians */
double rtd = 180.0/M_PI;  /* Radinas to Degrees */
...
extern double slat;            /* reference latitude for polar stereo */
extern double alon0;           /* reference longitud for polar stereo */
extern double plat1;           /* the first standard arallel for Lambert */
extern double plat2;           /* the second standard arallel for Lambert */
extern double lat0;            /* original lat for Lambert */
extern double lon0;            /* original long for Lambert */
extern char   proj_id[15];     /* the id number of the projection used */
extern int    zone;            /* the UTM zone number used */
extern char   hem;             /* sourth or north hemsphere */
extern double rlocal;

cross(double x1, double y1, double z1, double x2, double y2, double z2,
...
ll_to_proj(double lat_d,double lon,double *op1,double *op2)
{
  ...
  else ll_ac(lat_d,lon,op1,op2);
}

proj_to_ll(double p1, double p2, double *lat, double *lat_d, double *lon)
{
  ...
  else ac_ll(p1,p2,lat,lat_d,lon);
}
...
***********************************************************************/
lambert_ll(double x,double y,double *lat,double *lat_d,double *lon)
{
...
utm_ll(double x,double y,double *lat,double *lat_d,double *lon)
{
  ...
  rm0 = 0.0;
  if ((*lat_d)>=0.0) yadj=0.0; else yadj=1.0E7; /* Ref pt Southern Hemisphere */
...
ssmill(double xx,double yy,double *alat,double *alon)
{
  ...
  e=sqrt(e2);      
  slatr=slat*dtr;
  if (*alat>=0.0) { sn=1.0; xlam=-45.0; } else { sn=-1.0; xlam=0.0;  }
...
*****************************************************************************/
llssmi(double lat, double lon, double *x, double *y)
{
  double re=6378.273;     /* radius of earth = 6378.273 km (huges ellipsoid) */
  double e2=0.006693883;  /* eccentricity of the earth (huges ellipsoid) */
  double ta[2];
  double sn,e,xlam,rlat,t,cm,rho;
  double alon, alat;
  int    i;
...
ll_utm(double tlat, double tlon, double *op1, double *op2)
{
  double   k0 = 0.9996;       /* Center meridian scale factor */
 ...
 ******************************************************************/
ll_lambert(double lat,double lon,double *x,double *y)
{
  double a = 6378206.4;  /* mean equatorial radius in meters */
  double e = 0.0822719;  /* earth eccentricity */
  double m1, m2, t1, t2, t0;
...
alpha123(double x, double y, double z, double vx, double vy, double vz)
 {
...
ac_ll(double c1, double c2, double *lat, double *lat_d, double *lon)
 {
   double qlat, qlon;
   double x, y, z;
...
ll_ac(double lat, double lon, double *c1, double *c2)
 {
  double x,y,z;
  double x_t,y_t,z_t;
  double qlat, qlon;
...
rotate_z(double *x0,double *y0,double *z0,double theta0,double sign)
 {
...
rotate_y(double *x0, double *y0, double *z0,double theta0, double sign)
 {
...
ll_to_ebf(double lat, double lon,double *x,double *y,double *z)
 {
  double r_local;
...
ebf_to_ll(double x,double y,double z,double *lat,double *lat_d,double *lon)
 {
