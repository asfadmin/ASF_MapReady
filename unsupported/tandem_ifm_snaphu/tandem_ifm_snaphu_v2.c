...
*/
		...
		sprintf(cmd,"snaphu_v2 --tile 3 3 400 400 --nproc 8  ml.phase 4800 -m ml.amp \
...
/*	DISPLAY("Tandem Interferometric DEM Generation Program\n");*/
	...
	seeds=argv[3];
        ...
        printf("\nDate: ");
        system("date");
        printf("Program: tandem_ifm_snaphu (version 2)\n\n");
	...
	while (optind<argc)
	{
	  if (argv[optind][0]=='-')
		...
		case 'l':
			if (!(++optind<argc)) give_usage(argv[0]);
			else sprintf(logFile, "%s", argv[optind++]);
			logflag=1;
			break;
	...
	/* Done processing command line arguments.*/
        ...
        if (logflag) {
          fLog = FOPEN(logFile, "a");
          StartWatchLog(fLog);
          printLog("Program: tandem_ifm_snaphu (version 2)\n\n");
	  FCLOSE(fLog);
        }
...
/*	DISPLAY("    Remapping the Amplitude to Byte\n");*/
	sprintf(cmd,"amp2img -l 1x1 -s 1x1 -q amplitude_float.img amplitude.img\n");
        if (logflag) {
          fLog = FOPEN(logFile, "a");
	  sprintf(cmd,"amp2img -l 1x1 -s 1x1 -x %s -q amplitude_float.img amplitude.img\n", logFile);
          sprintf(logbuf,"\nCommand line: %s\n", cmd);
          printLog(logbuf);
        }
	execute(cmd);
