...
int arcticFlag=+1;/*+1-> arctic (North Pole); -1->antarctic (South Pole).*/
...
void drawLatLon(void)
{
	...
	{
		lon=-90;
		psfor(D2R*lon,D2R*arcticFlag*lat,&p.x,&p.y);
		psfor(D2R*(lon+1),D2R*arcticFlag*lat,&old_p.x,&old_p.y);
		postScriptRotate(p,old_p);
		postScriptF("%.0f lat,-90 lon",(float)arcticFlag*lat);
		postScriptRotateEnd();
...
main(int argc,char *argv[])
{
	...
	int argStart=1;
	if (argc<2)
		{printf("Usage dopPlot [ -a] <files...>\n"); exit(1);}
	if (0==strcmp("-a",argv[argStart]))
		arcticFlag=-1,argStart++;
	projectionInit();
	postScriptInit("output.ps");
	drawing=0;
	for (imageNo=argStart;imageNo<argc;imageNo++)
