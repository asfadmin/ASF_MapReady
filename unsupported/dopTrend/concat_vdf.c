/***************************************************************************
      ...
      SYNTAX:  concat_vdf outVDF [-inVdfList | [vdf1 [vdf2 [...]]]
  PARAMETERS:  
  	outVDF: freshly-created, new VDF.
  	-inVdfList: a file containing paths to VDF files to be read.
  	vdf1...: VDF files to be read.
  	 
 DESCRIPTION:  This program concatenates several Volume Descriptor
               Files (VDFs).  This is necessary before writing a CEOS-
               format tape which contains several orders.  The resulting
               VDF is written out, the referenced files are checked for
               existence, and a list of good files is reported to
	       stdout.
      ...
      AUTHOR:  Shusun Li, GI at UAF, Ak.
         ...
         1.0   7/1/97 - converted from get_names to concat_vdf. O.Lawlor
...
/*Read and chain product names from VDF's.*/
	...
	{/*For each input VDF:*/
		...
		{
			/*Extract each referenced product name.*/
			char *fileCode,*productName;
			int blockSize;
			switch((subType1Code)p->fpr.subType1)
			{
			case filePointer:
				/*If it's a file pointer, add it to the list.*/
		...
		/* Get the next VDF name, or break if none.*/
		if (vdfFile)
...
/*Now we output the chain of file descriptors, creating the output VDF.*/
/*Write VDF Header.*/
...
/*Write and print chained file descriptors.*/
	num_filePointers=1;
	while (listHead!=NULL)
	{
