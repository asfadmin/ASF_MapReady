...
printf("(This version compiled from /home/fsjec/cal/src/randi)\n");
}
...
/* now if more than one pulse */
    ...
    for (count = 1; ; count++) {
	printf("Analysing block %lu\r",count);
	/* test the asumption */
	if (buflen != drdGetCurrentBlockLength(pFH) ) {
	    printf("The buffer length of the first and the %lu. pulse differ\n",
                                g_cmdLine.luPulseNumber);
	...
	for (i = 0; i < NEXT_POW2(nValues); i++) 
	  { 
	  acFrequency[i].real=0.0;
	  acFrequency[i].imag=0.0;
	  }
...
if ((*pfFsampl<=0) || (*pfFc<=0) || (fMixShift<=0))
   {
   printf("I'm so sorry, but I couldn't find the Information in the .aux File\n");
   Bye();
   exit();
   } 
...
/* Jason hack - added some code right here to measure the power for
   a pulse.  I stole the algorithm in as much as I could from a CSA
   In Flight Calibration document */
