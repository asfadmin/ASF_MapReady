...
#define OUTFILE_S_EXT   ".saturation"
#define IMGFILE_I_EXT	".stddev.i.image"
#define IMGFILE_Q_EXT	".stddev.q.image"
#define IMGFILE_A_EXT	".stddev.a.image"
#define IMGFILE_S_EXT   ".saturation.image"

#define maxLineLength 80     /*max length read in aux file*/
...
/* structure for IQA (I Q Abs) values */
struct IQA {
	double I,Q,A;
	};
...
printf("\t		  each pixel in the image consists of <samples>\n");
printf("\t	          values in width and <scans> values in height\n");
...
char *pszAimage;
register FILE *fileSout = NULL,*fileSimageOut = NULL;
char *pszSimage;
DRDFILE *echo,*aux;
ushort uLength,uBlockPerLine;
char *apSample,*pTmp,*searchTmp,*pszTmp;
register int	*abValid;
register struct IQA *alfMean,*alfStdDev;
register double *alfSat;
...
if (g_cmdLine.bCalculateS) {
    fileSout = fopendef(NULL,g_cmdLine.pszInFile,OUTFILE_S_EXT,"w+");
    if (NULL == fileSout) {
        fprintf(stderr,"cannot open output file for S-part\n");
        g_cmdLine.bCalculateS = 0;
        } /* no calculation of S part because cannot open file */
    else {
        fileSimageOut=fopendef(NULL,g_cmdLine.pszInFile,IMGFILE_S_EXT,"w+");
        pszSimage = g_hjw_filename;
        if (NULL == fileSimageOut) {
            fprintf(stderr,"cannot open output image file for S-part\n");
            g_cmdLine.bCalculateS = 0;
            }
        }
    }
...
printf("Searching in the matching .aux file for the Bit quantization\n");
printf("of the satellite\n");
/*default*/
numBits = 4;

if (NULL == (aux=drdOpen(strcat(g_cmdLine.pszInFile,AUX_FILEEXT))))
   {
   printf("Cannot open matching auxiliary file\n");
   printf("SDEVF uses default quantization of 4 Bits (RADARSAT) !!!\n");
   }
else {
   rewind(aux->file);
   pTmp=malloc (maxLineLength);
   NotFound=1;
   while (!feof(aux->file))
   {
     fgets(pTmp,maxLineLength,aux->file);
     /*search only for the beginning of QUANTIZATION, because in some
      *files there's written QUANTISATION*/
     if (!(NULL==strstr(pTmp,"QUANTI")))
       {   /*found*/
       NotFound=0;
       searchTmp=(char *)strstr(pTmp,"=");
       searchTmp=(char *)strpbrk(searchTmp,"0123456789");
       numBits=(short int) atoi(searchTmp);
       printf("SDEVF found a quantization of %d Bits\n",numBits);
       break;
       }
   }
   auxClose(aux->file);  
   if (NotFound)
   {
   printf("Cannot find Quantization in the auxiliary file\n");
   printf("SDEVF uses default quantization of 4 Bits (RADARSAT) !!!\n");
   }
   free(pTmp);   
}
   
highBin = (short int) (pow(2.0,(double)numBits-1) - 1);
lowBin  = (short int) (-pow(2.0,(double)numBits-1));
...
/* only calculation if there is something to calculate */
	...
	if (CalculateCluster(highBin,lowBin,alfMean,alfStdDev,alfSat,apSample,abValid,uLength) ) {
		...
		if (g_cmdLine.bCalculateS)
		    fprintf(fileSout," %lf",alfSat[count]);
		if (g_cmdLine.bCalculateAbs)
		...
		if (g_cmdLine.bCalculateS) fprintf(fileSout," ERROR");
    ...
    */
    ...
    if (g_cmdLine.bCalculateS) rewind(fileSout);
    ...
    if (g_cmdLine.bCalculateS) {
        /* hardcode so DN 0 is 0% and DN 100 is 100% */
	TransformToImageFile(fileSout,fileSimageOut,0.0,100.0);
        }
    if (g_cmdLine.bCalculateAbs) {
...
if (NULL != fileSout) close(fileSout);
if (NULL != fileSimageOut) close(fileSimageOut);
...
if (g_cmdLine.bCalculateS)
        CallDx(pszSimage,uBlockPerLine,luRows);
if (g_cmdLine.bCalculateAbs)
...
for(count2 = 0;count2 < cl/(g_cmdLine.sampls);count2++) { /*every part of line*/
	...
	if (apValid[count1]) {
	    /*pTmp gives the actual position in the buffer apSample*/
		...
		if (g_cmdLine.bCalculateS)
                    if (pTmp[count3].I == hiBin || pTmp[count3].I == loBin) 
                        apSat[count2]++;
		if (g_cmdLine.bCalculateQ)
    ...
    apSat[count2] = apSat[count2] * 100.0 / ((double)nValids * (double)g_cmdLine.sampls);
