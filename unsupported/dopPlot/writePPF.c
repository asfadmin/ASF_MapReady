...
#if STANDALONE
	#define PPF_DESTINATION_DIRECTORY ""
#else
...
/*Find ceos extension (.L)*/
	i=strlen(ppfBase);
        ...
        ppfBase[i-1]=ppfBase[i+1];/*Copy over the product type.*/
...
void writeDopplerDelta(char *ceosFile, double dopplerOffsetPRF)
{
	static FILE *mail_message=NULL;/*Informative list of changed files/dopplers.*/
	char ppfFile[256];
...
{
	int argNo=1;
	if (argc<2) {printf("Usage: writePPF <ceos>\n");exit(1);}
	while (argv[argNo])
	{
		float dop=1;
		if ((argNo%2) == 0)
