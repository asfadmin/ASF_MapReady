...
public class Locater {
	...
	try {
	   ...
	   {
		System.err.println ( "ERROR:Could not convert field \""+field_name + "\" with a value of \"" +
			field +"\"");
		return 0;
	   }
	...
	lon = new double[1];
         ...
         /* 
		Compute the "inverse flattening" required by the 
		ellipsoid computations 
	  */

        double f83  = (m.ellipsoid_major_axis-m.ellipsoid_minor_axis)/m.ellipsoid_major_axis;
   ...
   private void en2normxy ( double E[], double N[], Target t, double samples, double lines)
     {
      ...
      //System.err.println("Target" + t.cs_dev_id + " at (" + t.cs_lat + "," +t.cs_long +" is at "+ Ei +","+ Ni);
	...
	{
	    System.err.println("ERROR:Targeti \"" + t.cs_dev_id + "\" is not in image");
	    return;
	}
    ...
    public Locater ( Vector targets, Metadata m) 
      {
			/* Compute the "inverse flattening" as required by the 
			   converters */
 ...
 */
		...
		utmc(Lon[0], 4, Lat, Lon, Eastings, Northings, m.ellipsoid_major_axis, f, zone);
...
/*  Notes from fortran code:
  ...
  */
		...
		try { 
		  ...
		  {
			System.err.println("ERROR:\tA error occured while writing to the file \"" + 
...
!
! 04/21/97 - Devin Lando - Added new parameter to contain the base longitude.
!  Previously, utmc used the first element in the tlon parameter to find the
!  base zone.
!
! *************************************************************************
!
    ...
    for ( int i = 0; i < number ; i++ )
     {
      ...
      a3     = (5.0 - 18.0*t + Math.pow(t,2) + 72.0*c - 58.0*epsq) * Math.pow(a1,5) / 120.0;
      xadj   = 5E5;
      ...
      c4     = 35.0*e6/3072.0;
      rm     = a*(c1*lat - c2*Math.sin(2.0*lat) + c3*Math.sin(4.0*lat) - c4*Math.sin(6.0*lat));
      rm0    = 0.0;
      b1     = Math.pow(a1,2)/2.0+ (5.0 - t + 9.0*c + 4.0 * c*c)*Math.pow(a1,4)/24.0;
      b2     = (61.0 - 58.0*t + t*t + 600.0*c - 330.0*epsq) * Math.pow(a1,6) / 720.0;
      yadj   = 1E7;
