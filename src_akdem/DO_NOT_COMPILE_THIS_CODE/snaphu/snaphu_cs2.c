...
/* Use this to print the DIMACS format flow answer to standard out */
/* Added by CWC 4/98 (could also specify with -DPRINT_ANS at compile time) */
	      ...
	      long cs2scalefactor, short ***flowsptr)
{
  ...
  for ( i = ndp; i < ndp + n; i ++ ){
	...
	if(flow>LARGESHORT || flow<-LARGESHORT){
	  fprintf(sp0,"ERROR: flow will overflow short data type\n");
	  exit(ABNORMAL_EXIT);
	}
	...
	if(from==(to+1)){    
	  ...
	  if(!((to-1) % nNrow)){
	    ...
	    fprintf(sp0,"ERROR: unassigned ground arc (trying to continue)\n");
	    fprintf(sp0,"       to: %ld, from: %ld, flow: %ld\n",to,from,flow);
	    fprintf(sp0,"       (arc may have been flipped)\n");
	  }        
	}else{
	  fprintf(sp0,"ERROR: non-grid arc (trying to continue)\n");
	  fprintf(sp0,"       to: %ld, from: %ld, flow: %ld\n",to,from,flow);
	}
      } /* end if flow on arc */
