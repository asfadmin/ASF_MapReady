	...
	meta_parameters *meta)
{
	...
	int ignored;
	proj_parameters *p=meta->geo->proj=(proj_parameters *)MALLOC(sizeof(proj_parameters));
	meta->geo->type='P';	
	meta->geo->xPix=pixSize;
	meta->geo->yPix=pixSize;
	switch(proj->proj)
	...
	default:/*Unrecognized Map projection!*/
		...
		if (logflag) {
		  sprintf(logbuf, "   WARNING: Unrecognized map projection code %d passed to \n"
		       "            geocode/proj.c:write_proj_meta!\n"
		       "            NOT creating .meta file...\n",proj->proj);
		  printLog(logbuf);
		}
