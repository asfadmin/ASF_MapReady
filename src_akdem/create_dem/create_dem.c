...
{	
        printf("\n    Usage: create_dem [-c] <configuration file>\n\n");
	printf("           -c	creates only the configuration file and exits.\n\n");
	printf("    create_dem will run the complete SAR interferometric\n");
...
main(int argc, char *argv[])
{
	...
	if (argc<2 || argc>3) usage("create_dem");
...
/*		    if (datatype == 0) strcat(options, " -swath");*/
	...
	if (datatype==2) {
		...
		/* Ingest CEOS SLC data */
		  ...
		  check_return(c2p("a", "a"), "converting complex master image into phase and amplitude (c2p)");
          	  ...
          	  check_return(amp2img("a.amp", "a_amp.img", meta->ifm->nLooks, 1), 
		  ...
		  check_return(c2p("b", "b"), "converting complex slave image into phase and amplitude (c2p)");
          	  ...
          	  check_return(amp2img("b.amp", "b_amp.img", meta->ifm->nLooks, 1), 
		...
		/* Coregister slave image */
		  ...
		  sprintf(tmp, "%s_b_amp.img", cfg->general->base);
          	  check_return(amp2img("b_corr.amp", tmp, meta->ifm->nLooks, 1), 
					"creating byte amplitude slave image (amp2img)");
	...
	/* Refine the offset */
	  ...
	  while (off) {
	    ...
	    check_return(amp2img(tmp, "sar_byte", meta->ifm->nLooks, 1), "creating byte amplitude image (amp2img)");
