...
{	
 ...
 printf("\n"
	"   -c   creates only the configuration file and exits.\n");
 printf("\n"
	"DESCRIPTION:\n"
	"   create_dem will run the complete SAR interferometric\n"
	"   processing chain, from ingesting the level zero STF data\n"
 ...
 printf("\n"
	"Version %1.2f, ASF InSAR TOOLS, 2001\n"
	"\n", VERSION);
 exit(1);
}
...
main(int argc, char *argv[])
{
	...
	struct DDR ddr;
	FILE *fCorr, *fCoh;
	char configFile[255], cmd[255], path[255], data[255], par[255], tmp[255], options[255];
	char *veryoldBase=NULL, *oldBase=NULL, *newBase=NULL;
	int i, delta, shiftX, shiftY, cFlag=0;
	float xshift, yshift, avg;
	double lat1=0.0, lat2=0.0, lon=0.0;
	...
	if (strncmp(cfg->general->status, "new", 3)==0 && !cFlag) {
	  ...
	  sprintf(tmp, "%s/%s", path, par);
	  if (!fileExists(tmp)) check_return(1, "master image par file does not exist");
	  sprintf(cmd, "ln -s %s/%s .", path, par); system(cmd);
	}
	sscanf(cfg->slave->path, "%s", path);
	sscanf(cfg->slave->data, "%s", data);
	sscanf(cfg->slave->par, "%s", par);
	if (strncmp(cfg->general->status, "new", 3)==0 && !cFlag) {
	  sprintf(tmp, "%s/%s", path, data);
	  if (!fileExists(tmp)) check_return(1, "slave image data file does not exist");
	  sprintf(cmd, "ln -s %s/%s .", path, data); system(cmd);
	  sprintf(tmp, "%s/%s", path, par);
	  if (!fileExists(tmp)) check_return(1, "slave image par file does not exist");
	  sprintf(cmd, "ln -s %s/%s .", path, par); system(cmd);
	  system("mkdir reg");
	}
	...
	if (strncmp(cfg->lz2raw->status, "new", 3)==0) {
	  ...
	  if (!fileExists(cfg->master->par)) check_return(1, "master image par file does not exist");
	  check_return(lz2raw_flywheel(cfg->master->data, "a", cfg->lz2raw->prc_e2, cfg->lz2raw->prcFlag), 
			"ingesting master image(lz2raw_flywheel)");
	  if (!fileExists(cfg->slave->data)) check_return(1, "slave image data file does not exist");
	  if (!fileExists(cfg->slave->par)) check_return(1, "slave image par file does not exist");
          check_return(lz2raw_flywheel(cfg->slave->data, "b", cfg->lz2raw->prc_e1, cfg->lz2raw->prcFlag), 
			"ingesting slave image (lz2raw_flywheel)");
	  sprintf(cfg->lz2raw->status, "success");
	  check_return(write_config(configFile, cfg), "Could not update configuration file");
	}

        /* Calculate the Doppler polynomial for each swath */
        if (strncmp(cfg->fix_in_fromraw->status, "new", 3)==0) {
          check_return(fix_in_fromraw(cfg->master->data, "a"), 
			"calculate Doppler for master image (fix_in_fromraw)");
          check_return(fix_in_fromraw(cfg->slave->data, "b"), 
			"calculate Doppler for slave image (fix_in_fromraw)");
          sprintf(cfg->fix_in_fromraw->status, "success");
          check_return(write_config(configFile, cfg), "Could not update configuration file"); 
	}

        /* Constrain swaths to specified latitudes */
        if (strncmp(cfg->swath_offset->status, "new", 3)==0) {
	  if (cfg->general->lat_begin < -90.0 || cfg->general->lat_begin > 90.0) {
	    printf("\n   WARNING: latitude constraint (begin) out of range - value ignored\n\n");
	    cfg->general->lat_begin = -99.0;
	    cfg->general->lat_end = -99.0;
            check_return(write_config(configFile, cfg), "Could not update configuration file"); 
	  }
	  if (cfg->general->lat_end < -90.0 || cfg->general->lat_end > 90.0) {
	    printf("\n   WARNING: latitude constraint (end) out of range - value ignored\n\n");
	    cfg->general->lat_begin = -99.0;
	    cfg->general->lat_end = -99.0;
            check_return(write_config(configFile, cfg), "Could not update configuration file"); 
	  }
	  if (cfg->general->lat_begin != -99.0) {
	    sprintf(tmp, " -lat %.1f %.1f", cfg->general->lat_begin, cfg->general->lat_end);
	    sprintf(cfg->water_mask->status, "skipped");
	  }
	  else sprintf(tmp, "");
	  check_return(swath_offset("a", "b", cfg->master->data, cfg->slave->data, tmp), 
			"offset calculation (swath_offset)");
	  read_bound_file("boundlines", &sB);
          cfg->aisp_master->start_offset = sB.start_offset1;
          cfg->aisp_master->end_offset = sB.masked_end_offset1;
          cfg->aisp_master->patches = sB.aisp_patches;
	  cfg->coreg_p1->start_master = sB.masked_start_offset1;
	  cfg->coreg_p1->start_slave = sB.masked_start_offset2;
	  cfg->coreg_pL->start_master = sB.masked_end_offset1-4096;
	  cfg->coreg_pL->start_slave = sB.masked_end_offset2-4096;
          cfg->aisp_slave->start_offset = sB.start_offset2;
          cfg->aisp_slave->patches = sB.aisp_patches;

          sprintf(cfg->swath_offset->status, "success");
          check_return(write_config(configFile, cfg), "Could not update configuration file"); 
	}

	/* Check for water */
        if (strncmp(cfg->water_mask->status, "new", 3)==0) {
	  if (!fileExists(cfg->general->dem)) check_return(1, "reference DEM file does not exist");
          strncpy(tmp, cfg->general->dem, strlen(cfg->general->dem)-4);
          check_return(water_mask(tmp, "boundlines", cfg->water_mask->percent), "applying the water mask (water_mask)");
	  read_bound_file("boundlines", &sB);
          cfg->aisp_master->start_offset = sB.start_offset1;
          cfg->aisp_master->end_offset = sB.masked_end_offset1;
          cfg->aisp_master->patches = sB.aisp_patches;
	  ...
	  cfg->coreg_pL->start_slave = sB.masked_end_offset2-4096;
          ...
          sprintf(cfg->water_mask->status, "success");
          check_return(write_config(configFile, cfg), "Could not update configuration file"); 
	}
	...
	/* Processing the master image */
	  ...
	  sprintf(tmp, "-d 1 -c reg/avedop ");
	    ...
	    if (cfg->aisp_master->power == 1) sprintf(options, "%s -z 1", tmp);
	...
	/* Coregister last patch */
	  ...
	  check_return(fico("reg/a_pL", "reg/b_pL", "reg/ctrlL", "reg/ficoL", cfg->coreg_pL->grid, cfg->coreg_pL->fft), 
				"fine coregistration last patch (fico)");
	...
	/* Calculate the interferogram and coherence */
	  ...
	  if (fCoh = FOPEN(logFile, "r")) {
	    ...
	    if (avg < cfg->igram_coh->min) {
	      sprintf(tmp, "average coherence level below minimum of %.1f", avg);
	      check_return(1, tmp);
	    }
	    FCLOSE(fCoh);
	  }
	...
	/* Create subset of reference DEM */
        if (strncmp(cfg->dem_sim->status, "new", 3)==0) {
	  sprintf(tmp, "%s.amp", cfg->igram_coh->igram);
	  check_return(amp2img(tmp, "sar_byte", 5, 1), "creating byte amplitude image (amp2img)");
	  ...
	  check_return(reskew_dem("dem_big.dem", "a.meta", "dem_slant.ht", "dem_sim.amp"), 
          ...
          sprintf(cfg->dem_sim->status, "success");
          check_return(write_config(configFile, cfg), "Could not update configuration file");
	}

	/* Perform pixel offset matching */
        if (strncmp(cfg->offset_match->status, "new", 3)==0) { 
	  if (cfg->offset_match->offset < 0 || cfg->offset_match->offset > 1) {
	    printf("\n   WARNING: offset flag set to invalid value - set to value of 1\n\n");
	    cfg->offset_match->offset = 1;
            check_return(write_config(configFile, cfg), "Could not update configuration file"); 
	  }
	  if (!cfg->offset_match->offset) {
	  ...
	  else
	  {
	    ...
	    FCLOSE(fCorr);
	    shiftX = (int) (xshift+0.5); shiftX *= -1;
	    shiftY = (int) (yshift+0.5); shiftY *= -1;

	    check_return(trim("dem_slant.ht", "dem_lined.ht", shiftY, shiftX, ddr.nl, ddr.ns), 
				"shifting and re-sizing slant range DEM (trim)");
	    check_return(trim("dem_simbyte.img", "dem_lined_amp.img", shiftY, shiftX, ddr.nl, ddr.ns), 
				"shifting and re-sizing simulated byte amplitude (trim)");
	  }
	...
	/* Simulated phase image and seed points */
	  ...
	  check_return(dem2seeds("dem_lined.ht", "sar_byte.img" , cfg->sim_phase->seeds, cfg->sim_phase->fft), 
	...
	/* Phase unwrapping */
	  ...
	  if (strncmp(cfg->unwrap->algorithm, "snaphu_v2", 9)==0) {
	    ...
	    cfg->unwrap->tiles_azimuth = (int) (fabs(lat1-lat2)*3);
            check_return(snaphu_v2("ml.phase", "ml.amp", cfg->aisp_master->power_img, cfg->aisp_slave->power_img,
					"snaphu.conf", "unwrap.phase", cfg->unwrap->tiles_azimuth, cfg->unwrap->tiles_range,
					cfg->general->procs), "phase unwrapping (snaphu_v2)");
	...
	/* Elevation and elevation error */
	  ...
	  if (strcmp(cfg->unwrap->algorithm, "escher")==0) {
	    check_return(eleverr(cfg->igram_coh->coh, newBase, "a.meta", "unwrap.phase.mask" , cfg->elevation->error), 
	...
	/* Remapping to ground range */
	  ...
	  check_return(deskew_dem(cfg->elevation->dem, "a.meta", "elevation.dem", ""), 
					"remapping elevation to ground range DEM (deskew_dem)");
	  check_return(deskew_dem("elevation.dem", "a.meta", "amplitude_float.img", "a_amp.img"), 
					"remapping amplitude to ground range (deskew_dem)");
	  check_return(deskew_dem("elevation.dem", "a.meta", "error.img", cfg->elevation->error), 
	...
	/* Geocoding */
	  ...
	  sprintf(tmp, "grep -c %s %s > /dev/null", uc(cfg->geocode->key), cfg->geocode->proj);
