...
lasErr c_getddr(const char *hname,struct DDR *ddr)
{
	...
	/* otherwise we've got old style meta info, so get ddr info from ddr file  */
		...
		if ( (clen != ctemp) || (dlen != dtemp) ) {
		   c_errmsg("Error reading record 1 of associated DDR file","getddr-read",NON_FATAL);
		   return(E_FAIL);
		}
		...
		if ( (clen != ctemp) || (dlen != dtemp) ) {
		   c_errmsg("Error reading record 2 of associated DDR file","getddr-read",NON_FATAL);
		   return(E_FAIL);
		}

		if (strcmp(c_getsys(),ddr->system) != 0)	  /* if the DDR system and the host   */
		  convflg = TRUE;			  /* are not the same then set the    */
							  /* conversion flag 		      */
