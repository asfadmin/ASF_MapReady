...
R. White, PSU/ESSC	Oct, 1994	Included sincos() for sun SYS5
O. Lawlor, ASF          March, 1998     Always use C version of sincos(),
                                        except on sunos, where it's already defined.
...
-----------------------------------------------------------------*/
	 ...
	 sgna = '-';
      con = fabs (pak);
      degs = (int) ((con / 10000.0) + .001);
      con =  con  - degs * 10000;
      mins = (int) ((con / 100.0) + .001);
      secs = con  - mins * 100;
...
/* Function to calculate UTM zone number--NOTE Longitude entered in DEGREES!!!
  ---------------------------------------------------------------------------*/
int calc_utm_zone(double lon) { return((int)(((lon + 180.0) / 6.0) + 1.0)); }
