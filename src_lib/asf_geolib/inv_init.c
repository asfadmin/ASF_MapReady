...
void inv_init(
	...
	inverse_transform inv_trans[]/* inverse function pointers			*/
	)
{
int zone;		/* zone number					*/
double azimuth=0;	/* azimuth*/
double angle=0;		/* rotation anlge				*/
double alf=0;		/* SOM angle					*/
double lon1=0;		/* longitude point in utm scene			*/
double lon2=0;		/* 2nd longitude point 				*/
...
int   start=0;		/* start of SOM Beginning or end		*/
double time=0;		/* SOM time					*/
int path;		/* SOM path number				*/
int satnum;		/* SOM satellite number				*/
int mode;		/* which format is used	A or B			*/
...
*/
  ...
  if (insys == OBEQA)
    ...
    *iflg = obleqinvint(radius,center_long,center_lat,shape_m, shape_n,
    ...
    {
	inv_trans[insys]=geographinv;
    }
    else {
fprintf(stderr,"Unrecognized projection passed to asf_geolib/inv_init!\n"
   "Projection code is %d\n",insys);
    }
return;
}
