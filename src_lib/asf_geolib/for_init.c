...
void for_init(
	...
	forward_transform for_trans[]	/* forward function pointers			*/
	)
{
...
int   start=0;		/* where SOM starts beginning or end		*/
double time=0;		/* SOM time					*/
double radius;		/* radius of sphere				*/
int path;		/* SOM path number				*/
int satnum;		/* SOM satellite number				*/
int mode;		/* which initialization method  to use A or B	*/
...
*/
  ...
  if (outsys==GEO)
  {
      for_trans[outsys]=geographfor;
  } 
  else
  {
    fprintf(stderr,"Error in asf_geolib/for_init:\n"
	"Unrecognized Map Projection code %d!\n",outsys);
    exit(1);
  }   
return;
}
