...
#endif
#ifdef M10
void display_idx__(i_w, i_x, i_y, i_width, i_height, i_bpl, i_data)
#endif
#ifdef HP
	...
	
{
	...
	for (i = 0 ; i < i_width[0] ; i++) for(j = 0; j < i_height[0] ; j++) {
#ifdef M4
             ...
             if (visualList[0].depth == 8) {
               ...
               i_bpp = 16;
               i_nbits[(i*i_width[0]+j)*2+0] = indices[i_val]*4+indices[i_val]/8;  
               i_nbits[(i*i_width[0]+j)*2+1] = (indices[i_val]-8*(indices[i_val]/8))*32 + indices[i_val];  
               ...
               i_bpp = 32;
               i_nbits[(i*i_width[0]+j)*4+0] = 0;  
               i_nbits[(i*i_width[0]+j)*4+1] = indices[i_val];  
               i_nbits[(i*i_width[0]+j)*4+2] = indices[i_val];  
               i_nbits[(i*i_width[0]+j)*4+3] = indices[i_val];  
             } else {
	...
	
{
	...
	for (i = 0 ; i < i_height[0] ; i++) for(j = 0; j < i_width[0] ; j++) {
             ...
             k = (int)((float)(i_ctble)*r_dat[i*i_bpl[0]+j]);
             if (k < 0) { 
               printf("rdat < 0 %d \n",k);
               k = 0;
             } /* endif */
             if (k > i_ctble-1) { 
               printf("rdat => i_ctble   %d %d \n",k,i_ctble);
               k = i_ctble-1;
             } /* endif */
             ...
             if (visualList[0].depth == 8) {
               ...
               i_bpp = 16;
               i_nbits[(i*i_width[0]+j)*2+0] = indices[k]/256; 
               i_nbits[(i*i_width[0]+j)*2+1] = indices[k] - 256*(indices[k]/256); 
             } else if (visualList[0].depth == 16) {
               i_bpp = 16;
               i_nbits[(i*i_width[0]+j)*2+0] = indices[k]/256; 
               i_nbits[(i*i_width[0]+j)*2+1] = indices[k] - 256*(indices[k]/256); 
               ...
               i_bpp = 32;
               i_nbits[(i*i_width[0]+j)*4+0] = 0;  
               i_nbits[(i*i_width[0]+j)*4+1] = indices[k];  
               i_nbits[(i*i_width[0]+j)*4+2] = indices[k];  
               i_nbits[(i*i_width[0]+j)*4+3] = indices[k];  
             } else {
               printf("depth not supported \n");
             }
             }
...
#endif
#ifdef M10
void display_rmg__(i_w, i_x, i_y, i_width, i_height, i_bpl, r_dat1,r_dat2)
#endif
#ifdef HP
	...
	
{
...
#else
    if (i_width[0]*i_height[0] >  100000) {
      printf("Error - viewport too big ( 100000 max) -  %d %d %d\n",i_width[0],i_height[0],
            i_width[0]*i_height[0]);
      /* exit(0); */
    } /* endif */
#endif
	...
	for (i = 0 ; i < i_height[0] ; i++) for(j = 0; j < i_width[0] ; j++) {
             ...
             if (visualList[0].depth == 8) {
               ...
               i_nbits[(i*i_width[0]+j)*2+0] = indices[i_val]/256; 
               i_nbits[(i*i_width[0]+j)*2+1] = indices[i_val] - 256*(indices[i_val]/256); 
...
#endif
#ifdef M10
void get_wininfo__(i_w, i_vx, i_vy, i_vw, i_vh, i_cw, i_ch,i_widget)
#endif
#ifdef HP
...
#endif
#ifdef M10
void move_scroll__(i_w,i_x,i_y)
#endif
#ifdef HP
    ...
    int i_c[];
{
      ...
      /* printf("value of i_c in setcolor = %d %d\n",i_c[0],allocated); */
      ...
      if (i_c[0] > 0) { 
        if (i_clrs == 0) {
          /* printf("freeing colors in setcolor\n"); */
          free_colors(); 
		  allocated = 0;  
        } /* endif */
      } /* endif */
      /* printf("exiting setcolor\n");*/
}


#ifdef SGI
void settable_(i_ntble,i_ctble)
#endif
#ifdef SUN
void settable_(i_ntble,i_ctble)
#endif
    ...
    int i_ctble[768];
{
    int j;
      ...
      for (j = 0; j < 256 ; j++) {      /* for better results, this should be interpolated, not nearest neighbor */
        rred [j]   = i_ctble[(j*i_ntble[0])/256];
        ggreen [j] = i_ctble[(j*i_ntble[0])/256+256];
        bblue [j]  = i_ctble[(j*i_ntble[0])/256+512];
        /* printf("%d) rgb = %d %d %d\n",j,rred [j],ggreen [j],bblue [j]); */
      } /* enddo */
...
#endif
#ifdef M10
void get_dialog_test__(a_msg,a_rsp)
#endif
#ifdef HP
...
/*******************************************/
#ifdef SGI
int init_gx_(i_gxi,i_typ,a_labl,i_wxs,i_wys,i_frx,i_fry,i_cin,i_din)
#endif
#ifdef SUN
int init_gx_(i_gxi,i_typ,a_labl,i_wxs,i_wys,i_frx,i_fry,i_cin,i_din)
#endif
#ifdef M10
int init_gx__(i_gxi,i_typ,a_labl,i_wxs,i_wys,i_frx,i_fry,i_cin,i_din)
#endif
#ifdef HP
int init_gx(i_gxi,i_typ,a_labl,i_wxs,i_wys,i_frx,i_fry,i_cin,i_din)
#endif
...
{ 
    ...
    static char *visual_class[] = { 
      ...
      ix = 0;
      if (i_db > 5-1) printf("i = %d \n",i); 
      if (i_db > 5-1) printf("i_gxi = %d \n",i_gxi[0]);
      for (i=1; i < i_gxi[0]+1; i++) {
        ...
        if (ix == 0) {
          ...
          if (i_db > 4-1) printf("creating formy\n");
        ...
        XtSetArg(args[n], XmNhorizontalSpacing,                0); n++;
        XtSetArg(args[n], XmNverticalSpacing,                  0); n++;
        if (i_db > 4-1) printf("creating form\n");
        ...
        if (i_db > 6-1) printf("i_cnt = %d %d \n",i_cnt[i],i);

        if (i_db > 6-1) printf("i_typ = %d \n",i_typ[i]); 
        ...
        switch (i_tttt) {
          ...
          case 3:
            ...
            XtSetArg(args[n], XmNrightAttachment,     XmATTACH_FORM); n++;
            XtSetArg(args[n], XmNleftAttachment,      XmATTACH_FORM); n++;  
            if (i_db > 4-1) printf("creating lable\n");
          ...
          case 4:
            ...
            XtSetArg(args[n], XmNleftAttachment,             XmATTACH_FORM); n++;
            if (i_db > 4-1) printf("creating scroll\n");
    ...
    XGetWindowAttributes(dgx,top[i_app],&xwa);
    ...
    screen  	= DefaultScreen(dgx);    
    /*    gc	        = DefaultGC(dgx, screen); */
    gc = XCreateGC(dgx,top[i_app],0,NULL);
...
/*    if (0 == 0) { */
    if (allocated == 0) {
      if (i_db > 5-1) printf("calling alo_colors\n");
      alo_colors();
      allocated=1; 
      if (i_ctble < 9) {
      ...
      if (i_db > 2-1) printf("Allocating %d colors\n",i_ctble);
      ...
      if (i_db > 5-1) printf("calling put_colors from i_gx\n"); 
      put_colors();
      if (i_clrs == 0) {
        if (i_db > 5-1) printf("calling free_colors from i_gx\n"); 
        free_colors();
        allocated=0;
      } /* endif */    
    } /* endif */
...
static void grey_4bita()
{
    ...
    for (i = 0 ; i < i_magn ; i++) {
       for (j = 0; j < i_colr ; j++) 
         ...
         /* printf("%d) 4ba = %d %d %d\n",i*i_colr+j,red [i*i_colr+j],green [i*i_colr+j],blue [i*i_colr+j]); */
    } /* enddo */
 ...
 static void colr_8bita()
{
    ...
    i_colr = i_ctble;
       ...
       for (j = 0; j < 256 ; j++) {
         red [j*i_colr/256]   = rred[(j)];
         green [j*i_colr/256] = ggreen[(j)];
         blue [j*i_colr/256]  = bblue[(j)];
...
static void colr_4bita()
{
    ...
    for (i = 0 ; i < i_magn ; i++) {
       for (j = 0; j < i_colr ; j++) {
         red [i*i_colr+j]   = rred[(j*255)/(i_colr-1)];
         green [i*i_colr+j] = ggreen[(j*255)/(i_colr-1)];
         blue [i*i_colr+j]  = bblue[(j*255)/(i_colr-1)];
       } /* enddo */
    } /* enddo */
    for (i = i_magn*i_colr ; i < i_ctble ; i++) {
         red [i] =   rred[0];
...
static void colr_4bitb()
{
    ...
    for (j = 0 ; j < i_colr ; j++) {
       for (i = 0; i < i_magn ; i++) {
         red [i*i_colr+j]   = rred[(i*255)/(i_magn-1)];
         green [i*i_colr+j] = ggreen[(i*255)/(i_magn-1)];
         blue [i*i_colr+j]  = bblue[(i*255)/(i_magn-1)];
       } /* enddo */
    } /* enddo */
    for (i = i_magn*i_colr ; i < i_ctble ; i++) {
         red [i] =   rred[0];
...
static void mixr_8bita()
{
    ...
    for (i = 0 ; i < i_magn ; i++) {
       ...
       for (j = 0; j < i_colr ; j++) {
         red [i*i_colr+j]   = (int)(r_fact*rred[(j*255)/(i_colr-1)]);
         green [i*i_colr+j] = (int)(r_fact*ggreen[(j*255)/(i_colr-1)]);
         blue [i*i_colr+j]  = (int)(r_fact*bblue[(j*255)/(i_colr-1)]);
       } /* enddo */
    } /* enddo */
}
...
static void alo_colors()
{
    ...
    unsigned int  nplanes = 0, nentries;
      ...
      if (i_clrs != 0 && i_clrs < i_ctble) i_ctble = i_clrs;

      /* printf("Number of Color-Cells = %d\n",i_ctble); */

      signed_nentries = nentries = i_ctble;
      ...
      if (signed_nentries < 0) {
        fprintf(stderr, "dgx: Couldn't allocate any colors\n");
        nentries = 0;
      }
...
static void free_colors()
{
    ...
    XFreeColors(dgx, cmap, indices,i_ctble, nplanes); 
    XFlush(dgx);    
}
...
static void put_colors()
{
    ...
    if ((xcolors = (XColor *) malloc ((long) 
	  ...
	  fprintf(stderr, "put_colors: Couldn't allocate space for colors\n");
	  return;
    }
    
    if (i_db > 4-1) printf(" \n");
    if (i_db > 4-1) printf("New Color Table \n");
    for (i = 0; i < i_ctble; i++) {
      ...
      if (i_db > 4-1) printf("  i,ind,rgb = %d %d %d %d %d\n",i,indices[i],red [i],green [i],blue [i]);
    }
    ...
    for (i = 1; i < i_gx+1; i++) {
	  if (fgx[i] != 0) XSetWindowColormap(dgx,fgx[i],cmap);
	  if (wgx[i] != 0) XSetWindowColormap(dgx,wgx[i],cmap);
    }
    XFlush(dgx);
}
...
#endif
#ifdef HP
off64_t ioseek64(chan, loc_byte)
#endif
int *chan;
off64_t *loc_byte;
{  
