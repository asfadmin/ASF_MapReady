...
DESCRIPTION:
	...
	coherence in interferograms
	Complex data that is processed at a doppler different that the optimal
	doppler will exhibit a decreased
	coherence in interferometry.  This coherence can be improved by
	filtering the data in azimuth and 
	retaining only the part of the two dataset's spectra that overlap.  The
	two datasets are then time-domain
	demodulated to baseband by the mean of their two doppler centroids.
	Another program 'cpx_spectrum' generates
	an azimuthal spectrum of the data.  
	
	The frequency parameter file indicates which frequencies should be
	filtered.  This can be generated using the program
...
/* Define all the variables that we'll be using */
	...
	int compensate_for_last_chunk=1;		/* if last chunk is 0 don't add loop for last chunk*/
...
/* Usage is shown if the user doesn't give 3 arguements */
	if(argc!=4)
	{
		printf("\nUsage: %s <.cpx in> <.cpx out> <frequency parameter"
			" file>\n\n",argv[0]);
		printf( "                 <.cpx in>   Input Complex Image\n"
		        "                <.cpx out>   Output Complex Image\n"
			"<frequency parameter file>   File containing"
			" passband information.\n\n"
			"cpx_filter - Bandpass filters complex image data"
			" in azimuth\n");
...
/* Get the number of lines and samples from the input DDR */
	...
	if( (2*toss_size) > (last_chunk=(int)((chunk_float-(float)chunk_int)*(float)block_size+0.5)))
...
/*		printf("Current time is %f seconds\n",cur_time);	*/
	...
	/* Write out the data file */
		...
		printf("Writing the output lines %d to %d in file %s\n",start_line,
				start_line+chunk_size-toss_size, outfile);
