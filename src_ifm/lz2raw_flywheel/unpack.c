...
iqType *ERS_unpackBytes(signalType *in,int nIn,iqType *out)
{
	int i,len=nIn/EnSig;
	if (len*EnSig!=nIn) {
	  sprintf(errbuf,"   ERROR: Asked to convert %d bytes, which is not divisble by %d!\n",nIn,EnSig);
	  printErr(errbuf);
	}
	for (i=0;i<len;i++)
...
/* Fill zero bytes in if we find an aux code before we expect one */
