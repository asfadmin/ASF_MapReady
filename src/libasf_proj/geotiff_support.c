...
void get_tiff_type(TIFF *tif, tiff_type_t *tiffInfo)
{
  ...
  do {
    t->imageCount++;
    // Makes next image directory the current directory until no more are left
  ...
  if (t->imageCount > 0 && !t->volume_tiff) {
    // Length of a scanline in bytes (regardless of data type or rgb/gray)
    tsize_t ss = TIFFScanlineSize(tif); 
    uint32 nstrips = TIFFNumberOfStrips(tif); // Number of strips in file
    uint32 rowsperstrip = 0;
    // Rows per strip ...zero for scanline and tile files
    rc[0] = TIFFGetField(tif, TIFFTAG_ROWSPERSTRIP, &rowsperstrip); 
    ...
    // Determine TIFF type
    if (rc[1] && rc[2] && tile_width > 1 && tile_length > 1 && 
	TIFFIsTiled(tif)) {
      uint32 tiledepth;
...
int get_bands_from_citation(int *num_bands, char **band_str, int *empty, 
			    char *citation, int expected_count)
{
  char *s;
  int band_no;
  *num_bands = 0;
  ...
  if (s != NULL) {
    ...
    if (s && strlen(s) && 
	strncmp(s, MAGIC_UNSET_STRING, strlen(MAGIC_UNSET_STRING)) == 0) {
      *num_bands = 0;
      strcpy(*band_str, "");
    }
    else {
      ...
      int num_empty, i;
      for (i=0, num_empty = 0; i<expected_count; i++) 
	num_empty += empty[i] ? 1 : 0;
      if (expected_count > 0 && *num_bands != expected_count - num_empty) {
        // The correct number of bands was not found, so zero everything out 
	// as a failure
        *num_bands = 0;
        strcpy(*band_str, "");
      }
