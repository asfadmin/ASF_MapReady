...
// Lock events significant to the deadlock detector.
typedef enum {
  ATTEMPT_READER_LOCK,
  ACHIEVE_READER_LOCK,
  ATTEMPT_WRITER_LOCK,
  ACHIEVE_WRITER_LOCK,
  FAIL_TO_ACHIEVE_WRITER_LOCK,   // For trylock.
...
// Is the lock with lock_id currently writer_locked or in the process
// of being writer_locked (conservatively)?  IMPROVEME: this is
// inefficient, we ought to maintain a seperate map from locks to
...
request_type_as_gstring (request_type_t request_type)
{
  ...
  case ACHIEVE_WRITER_LOCK:
    ...
    break;
  case FAIL_TO_ACHIEVE_WRITER_LOCK:
    g_string_append (result, "fail_to_achieve_writer");
    break;
	...
	Graph *pending_graph, Graph *block_graph, op_query_t *query)
{
  ...
  if ( query->request_type == ATTEMPT_READER_LOCK ) {
	...
	ce->contents = NULL;
      ...
      // Dump graphs of state before request granted.
      /*
      static int nn = 1;
      g_print ("about to give nod number %d to requestor %s for %s\n", nn,
      	       requestor->str, lock->str);
      GString *pending_dump_name
	= my_g_string_new_printf ("/tmp/dumpage/pg_%d.dot", nn);
      GString *block_dump_name
	= my_g_string_new_printf ("/tmp/dumpage/bg_%d.dot", nn);
      graph_dump_by_edges (pending_graph, pending_dump_name->str, node_names);
      graph_dump_by_edges (block_graph, block_dump_name->str, node_names);
      g_string_free (pending_dump_name, TRUE);
      g_string_free (block_dump_name, TRUE);
      nn++;
      */
  ...
  else if ( query->request_type == FAIL_TO_ACHIEVE_WRITER_LOCK ) {
    // Note that from the perspective of this point in the code,
    // failing to achieve a writer lock is exactly equivalent to
    // achieving a lock and them immediately releasing it.  This is
    // perhaps easier to see when you consider that
    // ATTEMPT_READER_LOCK is used to get permission for
    // writer_trylock attempts as well as regular writer_lock
    // attempts.

    // IMPROVEME: this is cut and paste from ACHIEVE_WRITER_LOCK query
    // type handling code above.  Factor it into a routing and decide
    // what to call it for yourself :)
    g_assert (is_writer_locked (lock, held_writer_locks));
    guint ii;
    for ( ii = 0 ; ii < requestor_locks->len ; ii++ ) {
      GString *lock_id = g_ptr_array_index (requestor_locks, ii);
    ...
    // IMPROVEME: this is cut and past from ACHIEVE_WRITER_UNLOCK
    // query type handling code below.
...
deadlock_preventer (void)
{
  ...
  for ( ; ; ) {
    ...
    if ( query->shutdown_request ) {
      // IMPROVEME: For now we leak the graph and other paraphenalia,
      // it won't matter.
      break;
    }
...
const int task_type_count = 13;
typedef enum {
  TASK_TYPE_ADD_FIELD = 0,
...
// Return a pointer to a new GString instance describing task_type.
//
...
// Actually it currently is used.
static GString *
task_type_as_gstring (task_type_t task_type)
{
  switch ( task_type ) {
  case TASK_TYPE_ADD_FIELD:
    return g_string_new ("ADD_FIELD");
    break;
  case TASK_TYPE_REMOVE_FIELD:
    return g_string_new ("REMOVE_FIELD");
    break;
  case TASK_TYPE_READER_LOCK_FIELD:
    return g_string_new ("READER_LOCK_FIELD");
    break;
  case TASK_TYPE_READER_UNLOCK_FIELD:
    return g_string_new ("READER_UNLOCK_FIELD");
    break;
  case TASK_TYPE_WRITER_LOCK_FIELD:
    return g_string_new ("WRITER_LOCK_FIELD");
    break;
  case TASK_TYPE_WRITER_TRYLOCK_FIELD:
    return g_string_new ("WRITER_TRYLOCK_FIELD");
    break;
  case TASK_TYPE_WRITER_UNLOCK_FIELD:
    return g_string_new ("WRITER_UNLOCK_FIELD");
    break;
  case TASK_TYPE_SET_FIELD_VALUE:
    return g_string_new ("SET_FIELD_VALUE");
    break;
  case TASK_TYPE_GET_FIELD_VALUE:
    return g_string_new ("GET_FIELD_VALUE");
    break;
  case TASK_TYPE_READER_LOCK_TABLE:
    return g_string_new ("READER_LOCK_TABLE");
    break;
  case TASK_TYPE_READER_UNLOCK_TABLE:
    return g_string_new ("READER_UNLOCK_TABLE");
    break;
  case TASK_TYPE_WRITER_LOCK_TABLE:
    return g_string_new ("WRITER_LOCK_TABLE");
    break;
  case TASK_TYPE_WRITER_UNLOCK_TABLE:
    return g_string_new ("WRITER_UNLOCK_TABLE");
    break;
  default:
    g_assert_not_reached ();
    break;
...
random_op (void)
{
  ...
  g_assert (TASK_TYPE_WRITER_UNLOCK_TABLE == 12);
  ...
  while ( !selected ) {
    ...
    g_assert (result < task_type_count);
...
test_thread_func (gpointer data)
{
  ...
  while ( g_timer_elapsed (timer, &junk) < thread_test_run_time
    ...
    // Comment bits of this out to selectively enable or disable
    // testing of particular methods.  Note that disabling unlock
    // tasks may result in test runs that don't terminate (since we
    // close all locks before exiting).
    if ( ! ( FALSE
	     || task_type == TASK_TYPE_ADD_FIELD
	     || task_type == TASK_TYPE_REMOVE_FIELD
	     || task_type == TASK_TYPE_READER_LOCK_FIELD
	     || task_type == TASK_TYPE_READER_UNLOCK_FIELD
	     || task_type == TASK_TYPE_WRITER_LOCK_FIELD
	     || task_type == TASK_TYPE_WRITER_TRYLOCK_FIELD
	     || task_type == TASK_TYPE_WRITER_UNLOCK_FIELD
	     || task_type == TASK_TYPE_SET_FIELD_VALUE
	     || task_type == TASK_TYPE_GET_FIELD_VALUE
	     || task_type == TASK_TYPE_READER_LOCK_TABLE
	     || task_type == TASK_TYPE_READER_UNLOCK_TABLE
	     || task_type == TASK_TYPE_WRITER_LOCK_TABLE
	     || task_type == TASK_TYPE_WRITER_UNLOCK_TABLE
	     ) ) {
      continue;
    }

    GString *task_string = task_type_as_gstring (task_type);
    g_string_free (task_string, TRUE);
	...
	if ( have_table_writer ) {
	  ...
	  if ( fields->len < field_count ) {
	    ...
	    if ( ! attempt_result ) {
	      GString *pts = pid_thread_string ();
	      g_error ("%s: unexpected writer_lock refusal from "
		       "deadlock_preventer", pts->str);
	...
	if ( have_table_writer || have_table_reader ) {
	  GPtrArray *fields = table_file_catalog (tf);
	  if ( fields->len > 0 ) {
	    guint field_to_lock = floor (my_random (fields->len - 0.00000001));
	    GString *field_name = g_ptr_array_index (fields, field_to_lock);
	    if ( !my_g_hash_table_entry_exists (reader_locks, field_name)
		 && !my_g_hash_table_entry_exists (writer_locks, field_name)
		 ) {
	      ...
	      gboolean attempt_result = receive_message (response_queue);
	      if ( attempt_result ) {
		gboolean trylock_result
		  = table_file_field_writer_trylock (tf, field_name->str,
						     NULL);
		if ( trylock_result ) {
		  send_message (rqs, requestor_id->str, *response_queue,
				ACHIEVE_WRITER_LOCK, field_name->str);
		  ...
		  send_message (rqs, requestor_id->str, *response_queue,
				FAIL_TO_ACHIEVE_WRITER_LOCK, field_name->str);
		  gboolean fail_result = receive_message (response_queue);
		  g_assert (fail_result);
		}
	      }
	    }
	  }
	...
	GPtrArray *choices = my_g_ptr_array_sum (reader_choices,
						 writer_choices);
	// FIIXME: should also include writer locked fields in choices.
	if ( choices->len > 0 ) {
