...
cal_params *create_cal_params(const char *inSAR, meta_parameters *meta)
{
           ...
           strncmp(processorStr, "FOCUS", 5) != 0)
  {
    ...
    printf("Using calibration factor %0.2f\n", alos->cf);
  }
  else
...
float get_cal_dn(cal_params *cal, int line, int sample, float inDn, int dbFlag)
{
  ...
  float noise_floor = 0.001;
  if (scaledPower > noise_floor && inDn > 0.0) {
    if (dbFlag) {
      calValue = 10.0 * log10(scaledPower);
        //calValue = 10.0 * log10(scaledPower) - 83.0 - 32.0; // Times 500.0 ? // Franz
    }
    else {
    ...
    if (dbFlag) {
      calValue = 10.0 * log10(noise_floor); // Produces -30 dB (if noise floor is 0.001)
      bad_count++;
//      printf("Bad count: %d\n", bad_count);
    }
    else {
      calValue = noise_floor;
    }
}
