/****************************************************************
FUNCTION NAME: equidckeyset

SYNTAX:  equidckeyset (ddr, gtif); 
...
DESCRIPTION:
    Set the geoTIFF keys for Equidistant Conic projection.
...

{
  ...
  GTIFKeySet(gtif, ProjCoordTransGeoKey, TYPE_SHORT, 1, CT_EquidistantConic);
  GTIFKeySet(gtif, PCSCitationGeoKey, TYPE_ASCII, 0,
                   "Equidistant Conic converted from LAS");

  if (ddr.valid[3])
  {
    /* Set the longitude of the standard parallel
       ------------------------------------------ */
    lon = ddr.proj_coef[2];
    c_decdeg (&lon, "DMS", "LON");
    GTIFKeySet(gtif, ProjStdParallel1GeoKey, TYPE_DOUBLE, 1, lon);
    if (ddr.proj_coef[8] == 1);
    {
      /* Set the longitude of the second standard parallel
         ------------------------------------------------- */
      lon = ddr.proj_coef[3];
      c_decdeg (&lon, "DMS", "LON");
      GTIFKeySet(gtif, ProjStdParallel2GeoKey, TYPE_DOUBLE, 1, lon);
    }
    ...
    GTIFKeySet(gtif, ProjNatOriginLatGeoKey, TYPE_DOUBLE, 1, lat);
