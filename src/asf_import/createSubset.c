...
void lat2time(stateVector locVec, double loc_sec, double targetLat, double range, double doppler, double *targetTime)
{
	...
	slantRange = range;
	  ...
	  vecAdd(sarPos,relPos,&targetPos);
          lat=atan2(targetPos.z, sqrt(targetPos.x*targetPos.x+targetPos.y*targetPos.y));
