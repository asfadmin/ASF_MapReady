...
PROGRAM HISTORY:
    ...
    1.6     2/02   R. Gens	Removed FACDR dependency
...
void usage(char *name)
{
  printf("\n"); 
  printf("Usage: %s\n", name);
  printf("  trim_slc <inSLC> <outCPX> <top line> <left samp> [<trim ht> "
	 "<trim wth>]\n\n"); 
...
{
  int era;
  int in_bytes_per_line,line_header,data_header;
...
/*Process CLA's*/
  ...
  if (argc!=7 && argc!=5) usage("trim_slc");
   /*Compute filenames*/
  
  era=set_era(argv[1],infileData,0);
  era=set_era(infileData,infile,1);

  get_ifiledr(infile,&vfdr); /* get image dimensions from file descriptor record */
  get_dssr(infile,&dssr); /* get line and pixel spacing from dataset summary record */

  outfileData=appendExt(argv[2],".cpx");
  outfileDDR=appendExt(argv[2],".ddr");
  StartWatch();
  system("date");
  printf("Program: trim_slc\n\n");
  ...
  inMaxX=vfdr.lbrdrpxl+vfdr.datgroup+vfdr.rbrdrpxl;
  inMaxY=vfdr.numofrec;

  if (era==0) /*pre-RADARSAT data-- vfdr invalid, 12 byte header per line always.*/
  	line_header=12;
  else line_header=192; /*post-RADARSAT data-- 192 byte header */
  in_bytes_per_line=inMaxX*vfdr.bytgroup;
...
/*Write out our outDDR*/
  create_good_ddr(outfileDDR,outMaxY,outMaxX,startY+1,startX+1,dssr.line_spacing,dssr.pixel_spacing,4);
...
/* Read size of data header */
     if (fread(&bufhdr,sizeof(bufhdr),1,in)!=1) 
	printErr("   ERROR: End of file detected.\n");

     data_header = bigInt32(bufhdr.recsiz);

     if ((fread(inbuffer, data_header, 1, in)) == NULL) 
	printErr("   ERROR: Problem reading data portion of record.\n");
  ...
  for (;y<lastReadY;y++)
  {
	...
	FSEEK64(in,data_header+inputY*(in_bytes_per_line+line_header)+line_header+SLCPixSize*inputX,0);
