...
*    oversampled_data_write -> write the oversampled date out to the file
*        "file_name"
*
*Private Member Functions
*    compress -> converts the image data from a matrix format to a linear format 
*        where the first item starts at 1. This is done just so the fourn
*        routine will work without extensive modifications.
...
*    rotate -> Rotate the elements in an array along the first dimension so
*        that the specified bin is moved to bin 1.
*    copy_corners -> Copy each corner of a square array into the corresponding
*        corner of a larger (square) array. Both arrays are assumed to be
*        complex.
*    baseband -> This subroutine shifts a complex 2-D spectrum to baseband along
*        its first dimension. The array is assumed to be square.
*******************************************************************************/
import java.util.* ;
import java.awt.* ;
import java.io.* ;
...
public class image_data
{
    ...
    /*********************************************************************
     * compress:
     * converts a i/q matrix format to a linear format where i follows q
     * and the first element starts at 1.
     * (This is just used to feed data into the fourn function below )  */
    ...
    /************************************************************
     * uncompress_q:
     * undo's a compress (see above) and returns the Q values. */
    ...
    /************************************************************
     * uncompress_i:
     * undo's a compress (see above) and returns the I values. */
    ...
    /*******************************************
     * cabs:
     * return the 'power' (amplitude squared) */
    ...
    /************************************************************************
     * baseband:
     * "Basebands" spectra. This is done by moving the cols with the largest 
     * values and the rotating them to [0] and then rotating the largest rows
     * to [0]. This stratigy is not the best - should instead use the
     * "windowing" techique. */
    ...
    /******************************************************
     * image_data_write:
     * write the image data out to the file "file_name". */
    ...
    private void data_write ( double [][]one, double [][]two, int sz, String file_name, boolean power )
    {
        ...
        if ( power ) {
            ...
            // Chop off at -40 ---> used to make plot more understandable.
            if ( min < -40) { min = -40; }
            delta = 256.0 /(min) ;
            fiddle = 256;
        }
    ...
    /***************************************************
     * oversampled_data_write:
     * write the oversampled data out to "file_name". */
    ...
    *       Converted with no modifications to program structure - JC */
    ...
    /***************************************
     * xpose1d:
     * Transpose a complex square matrix. */
    ...
    /**************************************************
     * findmax_cmpx2d_win:
     * finds the largest value in the subseted data. */
    ...
    /*****************************************************
     * findmax_cmpx2d_win:
     * finds the largest value in the oversampled data. */
    ...
    /***********************************************************
     * find_peak:
     * find the largest value in the oversampled data and update
     * peak_x, peak_y.                                        
     c
    ...
    /*******************************************************************
     * rotate:
     * Rotate the elements in an array along the first dimension so that
     * the specified bin is moved to bin 1.
     * Note: Used in basebanding.
     * NOTE: I REMOVED THE ';' AFTER ALL THE J FOR LOOPS BECAUSE I
     *       THOUGHT IT WAS A MISTAKE, BUT I'M NOT SURE  --Patrick    */
     ...
     * copy_corners:
     ...
     c     i, j              Indexing variables
     c*/
    private void copy_corners ( double ii[][], double qq[][], double II[][],
                                double QQ[][], int s, int S) {
        int i, j;
    ...
    private int get_centroid(double []dataset, int size, int window_len) {
        ...
        for (start_bin=1; start_bin<size; start_bin ++ ) {
            ...
            sum = previous_sum - dataset[start_bin-1] + dataset[right_edge];
            if (sum > peak_sum) {
                peak_sum = sum;
                peak_bin = start_bin;
            }
