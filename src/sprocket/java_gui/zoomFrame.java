...
class zoomFrame extends Frame implements ActionListener, ItemListener,
   ...
   // Have mainCanvas call this to get the width of the zoom box.
   public int getZoomWidth() {
      int width  = zoomedCanvas.getWidth() * zoomedCanvas.im.imagesize;
      width /= zoomedCanvas.im.width;
      width /= zoomedCanvas.ratio;
      return width;
   }
   
   // Have mainCanvas call this to get the height of the zoom box.
   public int getZoomHeight() {
      int height = zoomedCanvas.getHeight() * zoomedCanvas.im.imagesize;
   ...
   // OPTION MENU ITEMS *****************
      if (actEvent.getSource() == exportImageMenuItem) {
         FileDialog f = new FileDialog(this, "export image as...",
                                       FileDialog.SAVE);
      ...
      if (zoomedCanvas.im != null) {
         ...
         if (actEvent.getSource() == enterMenuItem ) {
            if (zoomedCanvas.simagemask == null) {
               FileDialog f = new FileDialog(this, "new smask filename",
                                             FileDialog.SAVE);
               ...
               if (filename != null) {
                  ...
                  filename = f.getDirectory() + filename;
                  zoomedCanvas.simagemask = new smask(filename,
                                                      mainCanvas.im.imagesize,
                                                      zoomedCanvas.im.width,
                                                      zoomedCanvas.im.height);
   ...
   public void itemStateChanged (ItemEvent itemEvt) {
      if (itemEvt.getSource() == autoMenuItem) {
         if (!zoomedCanvas.auto) {// Do this first because when the file dialog
                                  // goes away, SProCKET will repaint the zoom
                                  // canvas. If auto is on, your first filename
                                  // will be null.
          FileDialog f = new FileDialog(zoomFrame.this, "base export name",
                                        FileDialog.SAVE);
