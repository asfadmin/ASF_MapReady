...
// the forked child.  The behavior of an instance in which locks are
// held accross a g_thread_new or the like is also undefined.  In
// other words, besides the arrangement described in the previous
// paragraph, the supported paradigm for use of this class in a
// multithread or multiprocess program is:
// 
...
// the thread that created it, and when a field is removed, the lock
// the removing thread is required to hold is automaticly released.  A
// thread may therefore safely create a field, release the table level
// lock required to create that field, do other work, and then modify
// the value of the field, all without fear of interference by other
// threads.
//
...
//   table_writer_unlock (t);
//
// Where the class name method prefix has been omitted for clarity.
//
...
#define TABLE_FILE_MAX_KEY_LENGTH 200
#define TABLE_FILE_MAX_VALUE_LENGTH 1000
