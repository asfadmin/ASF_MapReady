...
getRec * fillOutGetRec(char file[])
{
  ...
  if (fp_ASF!=NULL)
	...
	{ 
	  ...
	  r->nLines=FTELL64(r->fp_in)/r->lineSize;/*Automatically figure out number 
						    of lines in file.*/
...
*/
void getSignalLine(const getRec *r,long long lineNo,complexFloat *destArr,
		   int readStart,int readLen)
{
  ...
  if (rightClip-leftClip!=
	...
	fprintf(stderr,"Lines = %lld Samples = %lld, rightClip = %d, leftClip = %d "
