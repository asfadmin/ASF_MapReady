#define ASF_NAME_STRING "make_sr_dem"
...
"     using data in the given DEM.  DEM will be in slant range.\n"
...
static void make_sr_dem_usage(const char *name)
{
  asfPrintStatus("\n"
      "Usage:\n"
      ASF_USAGE_STRING
...
#define usage make_sr_dem_usage
...
main (int argc, char *argv[])
{
  ...
  if (argc >= 2 && strmatches(argv[1],"-help","--help",NULL))
    print_help();
  if (argc<3)
    make_sr_dem_usage(ASF_NAME_STRING);
  ...
  int fail = make_sr_dem_ext(meta, demImg, demMeta, pad, tolerance, out_name, 1);
  int ok = !fail;
