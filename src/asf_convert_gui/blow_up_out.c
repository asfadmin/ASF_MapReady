...
in_output_thumbnail (GtkWidget *widget, GdkEventMotion *event)
{
    ...
    assertions).  */
    GdkRectangle otn_rect;	/* Output thumbnail rectangle.  */
    /* FIXME: fix this border hackery to be precise somehow.  */
    otn_rect.x = otnc_rect.x + 1;	/* There is probably a small border so +1.  */
    otn_rect.y = otnc_rect.y + 1;
    ...
    if ( gdk_region_point_in (otn_region, (int) event->x, (int) event->y) ) {
        result = TRUE;
        //    g_message ("Over output thumbnail!");
    }
...
{
    g_assert (GTK_IS_TREE_VIEW (widget));
    return in_output_thumbnail (widget, event);
}
...
thumbnail_region (GtkWidget *widget, GdkEventMotion *event)
{
    ...
    /* If over the output image thumbnail, return the output thumbnail region, */
    GtkTreeViewColumn *tc = NULL;	/* Thumbnail column we are over.  */
    if ( in_output_thumbnail (widget, event) ) {
    ...
    GdkRectangle otn_rect;		/* Image thumbnail rectangle.  */
    /* FIXME: fix this border hackery to be precise somehow.  */
    otn_rect.x = tn_rect.x + 1;	/* There is probably a small border so +1.  */
    otn_rect.y = tn_rect.y + 1;
    otn_rect.width = THUMB_SIZE;
    otn_rect.height = THUMB_SIZE;

    return gdk_region_rectangle (&otn_rect);
}
                  ...
                  GtkTreeViewColumn *column, GdkRegion *tr)
{
    ...
    gtk_tree_model_get (GTK_TREE_MODEL (completed_list_store), &iter,
        COMP_COL_OUTPUT_THUMBNAIL_BIG, &popup_image_pixbuf, -1);
                         ...
                         maybe_clear_popup_image_args_t *args)
{
    ...
    if ( !gdk_region_point_in (tn_region, (int) event->x, (int) event->y) ) {
        ...
        gulong unblock_count 
            ...
            (tree_view, completed_files_list_motion_notify_event_handler, NULL);
...
gboolean
completed_files_list_leave_notify_event_handler (GtkWidget *widget,
...
gboolean    
completed_files_list_scroll_event_handler (GtkWidget *widget, GdkEventScroll *event,
                                           gpointer user_data)
{
    GdkEventMotion motion_event;
    fake_motion_signal_args_t *fmsa = g_new (fake_motion_signal_args_t, 1);
