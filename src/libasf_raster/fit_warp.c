...
/*************************************
fill point offset array, and call
...
int fit_warp(const char *offsetsFile, const char *imageName, const char *outName)
{
	...
	in=FOPEN(offsetsFile,"r");
	meta = meta_read(imageName);
	if (!meta) return 1;
...
/*Create offset images*/
	...
	asfPrintStatus("Output warping images will be %d lines by %d samples\n",
