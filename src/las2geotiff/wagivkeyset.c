...
PURPOSE:  Set the geoTIFF keys for Wagner IV projection.
...

{
  ...
  /* Wagner IV is not a known projection in the current version of geoTIFF.
     ---------------------------------------------------------------------- */
  GTIFKeySet(gtif, ProjectedCSTypeGeoKey, TYPE_SHORT, 1, 32767);
  GTIFKeySet(gtif, PCSCitationGeoKey, TYPE_ASCII, 0,
            "Wagner IV--incomplete implementation in geoTIFF");
  if (ddr.valid[3])
  {
