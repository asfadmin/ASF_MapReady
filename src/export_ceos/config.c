...
int init_config(char *configFile)
{
  ...
  fprintf(fConfig, "export_ceos configuration file\n\n");
  ...
  fprintf(fConfig, "output format = < GEOTIFF | JPEG | ENVI | ESRI >\n");
...
#define newStruct(type) (type *)MALLOC(sizeof(type))
  ...
  strcpy(cfg->comment, "export_ceos configuration file");
  ...
  while (fgets(line, 255, fConfig) != NULL) {
	...
	cfg->general->in_data_name = read_str(line, "input data file"); 
      if (strncmp(test, "input metadata file", 19)==0) 
	cfg->general->in_meta_name = read_str(line, "input metadata file"); 
	...
	cfg->general->out_format = read_str(line, "output format");
      if (strncmp(test, "resampling", 10)==0) 
	cfg->general->resample = read_int(line, "resampling");
      if (strncmp(test, "browse image", 12)==0) 
	cfg->general->browse = read_int(line, "browse image");
      if (strncmp(test, "geocoding", 9)==0)
	cfg->general->geocoding = read_int(line, "geocoding");
      if (strncmp(test, "log file", 8)==0) 
	cfg->general->logFile = read_str(line, "log file");
  ...
  while (fgets(line, 255, fConfig) != NULL) {
    ...
    if (strcmp(params, "general")==0) {
      ...
      if (strncmp(test, "batch mode", 10)==0)
        cfg->general->batch = read_int(line, "batch mode");
    ...
    if (strcmp(params, "geocoding")==0) {
      ...
      if (strncmp(test, "background fill", 15)==0)
        cfg->geocoding->background = read_int(line, "background fill");
    ...
    if (strcmp(params, "polar")==0) {
      test = read_param(line);
      if (strncmp(test, "datum", 5)==0) 
	cfg->polar->datum = read_int(line, "datum");
      if (strncmp(test, "center latitude", 16)==0) 
	cfg->polar->center_lat = read_double(line, "center latitude"); 
      if (strncmp(test, "center longitude", 17)==0) 
	cfg->polar->center_lon = read_double(line, "center longitude");
      if (strncmp(test, "units", 5)==0) 
	cfg->polar->units = read_str(line, "units");
    }
    
    if (strncmp(line, "[UTM]", 5)==0) strcpy(params, "utm");
    if (strcmp(params, "utm")==0) {
      test = read_param(line);
      if (strncmp(test, "datum", 5)==0) cfg->utm->datum = read_int(line, "datum");
      if (strncmp(test, "zone number", 11)==0) 
	cfg->utm->zone = read_int(line, "zone number");
      if (strncmp(test, "units", 5)==0) 
	cfg->utm->units = read_str(line, "units");
    }
    
    if (strncmp(line, "[Albers Conic Equal Area]", 25)==0) strcpy(params, "albers");
    if (strcmp(params, "albers")==0) {
      test = read_param(line);
      if (strncmp(test, "datum", 5)==0) 
	cfg->albers->datum = read_int(line, "datum");
      if (strncmp(test, "first standard parallel", 23)==0) 
	cfg->albers->first_parallel = 
	  read_double(line, "first standard parallel");
      if (strncmp(test, "second standard parallel", 24)==0) 
	cfg->albers->second_parallel = 
	  read_double(line, "second standard parallel");
      if (strncmp(test, "central meridian", 16)==0) 
	cfg->albers->center_meridian = read_double(line, "central meridian");
      if (strncmp(test, "latitude of origin", 18)==0) 
	cfg->albers->orig_latitude = read_double(line, "latitude of origin");
      if (strncmp(test, "units", 5)==0) 
	cfg->polar->units = read_str(line, "units");
    }
    
    if (strncmp(line, "[Lambert Azimuthal Equal Area]", 30)==0) 
      strcpy(params, "lambert_az");
    if (strcmp(params, "lambert_az")==0) {
      test = read_param(line);
      if (strncmp(test, "datum", 5)==0) 
	cfg->lambert_az->datum = read_int(line, "datum");
      if (strncmp(test, "center latitude", 15)==0) 
	cfg->lambert_az->center_lat = read_double(line, "center latitude");
      if (strncmp(test, "center longitude", 16)==0) 
	cfg->lambert_az->center_lon = read_double(line, "center longitude");
      if (strncmp(test, "units", 5)==0) 
	cfg->lambert_az->units = read_str(line, "units");
    }
    
    if (strncmp(line, "[Lambert Conformal Conic]", 25)==0) 
      strcpy(params, "lambert_cc");
    if (strcmp(params, "lambert_cc")==0) {
      test = read_param(line);
      if (strncmp(test, "datum", 5)==0) 
	cfg->lambert_cc->datum = read_int(line, "datum");
      if (strncmp(test, "first standard parallel", 23)==0) 
	...
	cfg->lambert_cc->second_parallel = 
	  read_double(line, "second standard parallel");
      if (strncmp(test, "central meridian", 16)==0) 
	cfg->lambert_cc->center_meridian = read_double(line, "central meridian");
      if (strncmp(test, "latitude of origin", 18)==0) 
	cfg->lambert_cc->orig_latitude = read_double(line, "latitude of origin");
      if (strncmp(test, "units", 5)==0) 
	cfg->lambert_cc->units = read_str(line, "units");
    }
    
    if (strncmp(line, "[Resampling]", 12)==0) strcpy(params, "resampling");
    if (strcmp(params, "resampling")==0) {
      test = read_param(line);
