...
main(int argc, char *argv[])
{
  ...
  if (cfg->general->batch) {
    ...
    /* Ingest CEOS image */
    if (strncmp(cfg->import->status, "new", 3)==0) {
      sprintf(in, "tmp");
	...
	sprintf(cfg->geocoding->status, "success");
      check_return(write_config(configFile, cfg), 
		   "Could not update configuration file");
      }
    }
    ...
    /* Resampling */
    if (strncmp(cfg->resampling->status, "new", 3)==0) {
      if (cfg->general->resample || cfg->general->browse) {
	sprintf(in, "%s", out);
	sprintf(out, "tmp_small");
	if (cfg->general->browse) 
	...
	sprintf(cfg->resampling->status, "success");
	check_return(write_config(configFile, cfg), 
		     "Could not update configuration file");
      }
    }  
