...
int check_meta_block(const char *block, dbf_header_t *dbf, int nCols)
{
  ...
  for (ii=0; ii<nCols; ii++) {
    if (strncmp(dbf[ii].meta, search, strlen(search)) == 0 &&
        dbf[ii].visible)
      visible = TRUE;
  }
  return visible;
			   ...
			   c2v_config *cfg)
{
  ...
  if (cfg && strlen(cfg->header_file) > 0) 
    read_header_config_ext("META", &dbf, &nCols, NULL, cfg->header_file);
  else
  ...
  */
  ...
  for (ii=0; ii<nCols; ii++) {
    ...
    else if (strncmp(dbf[ii].meta, "meta.general.image_data_type", 28) == 0) {
    ...
    else if (strncmp(dbf[ii].meta, "meta.general.radiometry", 23) == 0) {
    ...
    else if (strncmp(dbf[ii].meta, "meta.general.orbit_direction", 28) == 0) {
    ...
    else if (strncmp(dbf[ii].meta, "meta.general.orbit", 18) == 0)
  ...
  for (ii=0; ii<nCols; ii++) {
    ...
    if (meta->sar) {
      ...
      else if (strncmp(dbf[ii].meta, "meta.transform.incid_a", 24) == 0) {
        for (kk=0; kk<6; kk++)
  ...
  for (ii=0; ii<nCols; ii++) {
    ...
    if (meta->stats) {
      ...
      for (kk=0; kk<meta->stats->band_count; kk++) {
        ...
        if (strncmp(dbf[ii].meta, "meta.stats.band_stats.mask", 26) == 0)
  ...
  for (ii=0; ii<nCols; ii++) {
    ...
    if (meta->state_vectors) {
      ...
      for (kk=0; kk<meta->state_vectors->vector_count; kk++) {
        if (strncmp(dbf[ii].meta, "meta.state.vectors", 18) == 0) {
        ...
        strcmp(value, MAGIC_UNSET_STRING) != 0)
    {
      if (expect_coords) {
        ...
        for (ll=0; ll<nCols; ll++) {
          int len = strlen(dbf[ll].meta);
          if (strncmp_case(dbf[ll].meta, param, len) == 0 && dbf[ll].visible)
          {
  ...
  c2v_config *cfg)
{
  ...
  double *lat = NULL, *lon = NULL, center_lat = -99, center_lon = -99;
