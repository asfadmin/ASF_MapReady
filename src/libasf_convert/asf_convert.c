...
int asf_convert_ext(int createflag, char *configFileName, int saveDEM)
{
  ...
  // Regular processing
    ...
    if (cfg->general->import) {
      ...
      // Verify that polarimetric matrix is only set for POLSARPRO input format
      if (strncmp_case(cfg->import->format, "POLSARPRO", 9) != 0 &&
	  cfg->import->matrix)
	asfPrintError("Only matrices generated by PolSARPro can be "
		      "ingested.\n");
      
      // When input data is a matrix, make sure this is passed along
      if (cfg->import->matrix)
	strcpy(cfg->import->image_data_type, "POLARIMETRIC_MATRIX");
