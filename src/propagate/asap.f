...
C
      OPEN(5,FILE='./temp_file_formerly_known_as_5')
...
C
  500 CONTINUE
      ...
      TOUT=T+STEP
C
C  In order to do reverse propagation, needed to modify
C  the line "IF (TOUT.GT.TF) TOUT=TF" to following:
C  * T. Logan 8/98 - ASF
C
      IF (STEP.GT.0.AND.TOUT.GT.TF) TOUT=TF   
      IF (STEP.LT.0.AND.TOUT.LT.TF) TOUT=TF
