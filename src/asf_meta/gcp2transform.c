...
meta_transform *gcp2transform(gcp_location *gcp, int gcp_count, char *type)
{
  ...
  double *ls2lat = (double *) MALLOC(sizeof(double)*50);
  double *ls2lon = (double *) MALLOC(sizeof(double)*50);
  double *ll2samp = (double *) MALLOC(sizeof(double)*50); 
  double *ll2line = (double *) MALLOC(sizeof(double)*50);
...
double value_at_pixel25(double inval1, double inval2, double *coef, int NUM_COEFS)
{
  ...
  if (NUM_COEFS>=27)
    temp += coef[25]*inval1*inval1*inval1*inval1*inval1 +
            coef[26]*inval2*inval2*inval2*inval2*inval2;
  return(temp);
}
  ...
  double *yin, double *vals, int cnt, double *coefs, int use_latlon_objective)
{
  ...
  int sizes[8] = {3,6,10,15,19,22,25,27};
