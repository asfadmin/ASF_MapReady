...
size_t FREAD(void *ptr,size_t size,size_t nitems,FILE *stream)
{
        size_t ret;
        char error_message[1024];
        char *errno_message;   /* this points to a static string within strerror */
        ...
        if (ret!=nitems)
        {
                ...
                sprintf(error_message,
                        ...
                        (int)stream, (errno) ? errno_message : "Unknown");
                fprintf(stderr,error_message);
                if (fLog!=NULL) fprintf(fLog,error_message);

                sprintf(error_message,
                        "*    Note that this was NOT a read-past end of file error.\n"
                        "*    Attempted to read %ld bytes from the file, but only\n"
                        "*    %ld bytes were read\n", (long) nitems, (long) ret);

                if (caplib_behavior_on_error == BEHAVIOR_ON_ERROR_ABORT)
                    strcat(error_message,
                        "**\n**   Program terminating... Error encountered while reading.\n");
