...
float read_header(char* fname, char* record);
...
char **argv;
{
  ...
  FILE *fp_in, *fp_out;
  float *img;
  int c_ns, c_nl, i, k, line_count, sample_count;
  ...
  height = read_header(in_header, "GPS ALTITUDE, M");
  lat_peg = read_header(in_header, "LATITUDE OF PEG POINT");
  lon_peg = read_header(in_header, "LONGITUDE OF PEG POINT");
  head_peg = read_header(in_header, "HEADING AT PEG POINT");
  lat_peg = lat_peg*PI/180.0;
  lon_peg = lon_peg*PI/180.0;
  head_peg = head_peg*PI/180.0;
  
  /* read offset parameters */
  s0 = read_header(in_header, "ALONG-TRACK OFFSET S0  (M)");	
  c0 = read_header(in_header, "CROSS-TRACK OFFSET C0  (M)");
  ...
  img = (float *) MALLOC(sizeof(float) * sample_count * line_count);
  fp_in = FOPEN(dem_img, "rb");
  get_float_lines(fp_in, meta, 0, line_count, img);
  fp_out = FOPEN(out_file, "w");
  
  for (k=1; k<=line_count; k=k+c_nl)
    for (i=1; i<=sample_count; i=i+c_ns) { 
      
      /* radar coordinates */
      c_lat = (i*meta->general->y_pixel_size + c0) / ra;
      s_lon = (k*meta->general->x_pixel_size + s0) / ra;
      height = img[k*sample_count+i];
...
{
  FILE *fp;
  char header[5000], line[50], *tmp;
  float value;
  int ii;

  fp = FOPEN(fname, "r");

  while(fgets(header, 5000, fp)) {
    if(strcmp(record, "GPS ALTITUDE, M") == 0)
      if(tmp = strstr(header, "GPS ALTITUDE, M")) {
	for (ii=0; ii<50; ii++)
	  line[ii] = tmp[ii];
	sscanf(tmp, "GPS ALTITUDE, M %f", &value);
	return value;
      }
    if (strcmp(record, "LATITUDE OF PEG POINT") == 0)
      if (tmp = strstr(header, "LATITUDE OF PEG POINT")) {
	for (ii=0; ii<50; ii++)
	  line[ii] = tmp[ii];
	sscanf(line, "LATITUDE OF PEG POINT %f", &value);
      }
    if (strcmp(record, "LONGITUDE OF PEG POINT") == 0)
      if (tmp = strstr(header, "LONGITUDE OF PEG POINT")) {
	for (ii=0; ii<50; ii++)
	  line[ii] = tmp[ii];
	sscanf(line, "LONGITUDE OF PEG POINT %f", &value);
      }
    if (strcmp(record, "HEADING AT PEG POINT") == 0)
      if (tmp = strstr(header, "HEADING AT PEG POINT")) {
	for (ii=0; ii<50; ii++)
	  line[ii] = tmp[ii];
	sscanf(line, "HEADING AT PEG POINT %f", &value);
      }
    if (strcmp(record, "ALONG-TRACK OFFSET S0  (M)") == 0)
      if (tmp = strstr(header, "ALONG-TRACK OFFSET S0  (M)")) {
	for (ii=0; ii<50; ii++)
	  line[ii] = tmp[ii];
	sscanf(line, "ALONG-TRACK OFFSET S0  (M) %f", &value);
      }
    if (strcmp(record, "CROSS-TRACK OFFSET C0  (M)") == 0)
      if (tmp = strstr(header, "CROSS-TRACK OFFSET C0  (M)")) {
	for (ii=0; ii<50; ii++)
	  line[ii] = tmp[ii];
	sscanf(line, "CROSS-TRACK OFFSET C0  (M) %f", &value);
      }
  }
  FCLOSE(fp);
  return value;
