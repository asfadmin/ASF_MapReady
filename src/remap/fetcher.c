...
typedef struct {
/*Original Image Info:*/
	struct DDR *inDDR;/*Input image DDR*/
	FILE *inFile;/*File pointer to input image*/
	int inBand;/*Input band number*/
	int inSize;/*Input pixel size*/
	int getRealCpx;/*Fetch the real portion of complex image?*/
...
/*Open the freshly-created blocking store file for read.*/
	g->blockIn=FOPEN(g->blockName,"rb");
...
/*Set input fields*/
	...
	g->inFile=in;
	g->inBand=bandNo;
	g->inSize=dtype2dsize(g->inDDR->dtype,NULL);
	g->getRealCpx=(outDtype==DTYPE_COMPLEXREAL);
	
/*Set Block Store fields*/
	/*Round *up* to nearest integral number of blocks*/
	g->blockWid=(g->inDDR->ns+BLOCK_SIDE-1)/BLOCK_SIDE;
	g->blockHt=(g->inDDR->nl+BLOCK_SIDE-1)/BLOCK_SIDE;
